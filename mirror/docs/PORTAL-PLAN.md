...
| 2 | `MENU_INVOKE` | shortcut-less menu items — the largest hole in the act plane | **done**, guard closed 2026-07-31 |
| 3 | `CONTROL_INVOKE` | controls by identity with no coordinate math | **done** (both halves, 20/20 each) |
| 4 | `WINDOW_ACT` | move/resize/zoom/close without a drag, so metal stops needing QMP | **done** (20/20 each, all four ops) |
| 5 | `TEXT_GET`/`TEXT_SET` | field contents read and written in-context | **done** (20/20 each, three kinds) |

The catalog numbers are the plan's ordering, not the wire's. `WINDOW_ACT` and the
text ops were built in parallel and both claimed `PT_OP_* = 5`; `WINDOW_ACT`
landed first, so on the wire the text ops are **ops 6 and 7**. The rows above
keep their plan numbers so the document's own cross-references stay true;
`ptshared.h` is the authority on what a request actually carries.
...
### 4. `WINDOW_ACT` — done, all four ops (2026-07-31)

This section used to predict that `DragWindow` had `TrackControl`'s shape. It
does not, and the difference is worth keeping rather than editing away:
**`DragWindow` is `pascal void`.** It performs the move itself and hands the
application nothing, so there is no question to answer and no application code
that runs after it. `move` is therefore served by calling `MoveWindow` in the
target's own context — the same call `DragWindow` would have made, minus the
tracking loop and the motion the loop needs. No patch and no click at all.

The other three DO have the shape, because the application has work to do
afterwards: `FindWindow` is answered with the part, and then `GrowWindow`,
`TrackBox` or `TrackGoAway` is answered in turn, and the application calls
`SizeWindow`/`ZoomWindow`/its own close handler itself. `FindWindow` is patched
so that no chrome geometry enters the op — locating a grow box or a close box
means inventing a title-bar height and a corner size no guest structure
reports.

20/20 on each of move, resize, zoom and close against the window's own rect
re-read from the guest; `close` verified by the window's absence from the
guest's window list. **No QMP anywhere in the path**, which is what this op
existed to achieve. Details, the mutation proof, and what is not claimed are in
`docs/STATUS.md`.

`close` is deliberate about its contract: it answers the application's own
`TrackGoAway`, so the application's own close path runs — save-changes dialog
and all. It does not promise the window closes, and a dialog that keeps the
window open is a correct outcome, not a failure.

### 5. `TEXT_GET` / `TEXT_SET` — done (2026-07-31, lane P2)

The difference between driving an app and filling in a form. Measured on mac99,
`tests/textops-probe.py`, N=20 independent trials per kind, the dialog closed
and reopened every trial and a value unique to that trial written.

**These ops install no trap patch, and that is a design decision, not an
omission.** `MenuSelect` and `TrackControl` are questions the application is
about to ask, so the way to act is to answer them. A TextEdit record and a
dialog's item list are not questions — they are per-process roots, unreachable
from outside and ordinary memory from inside. So the hook serves the request
directly, which also means there is no armed window at all.

Three ways to name the text, all requiring the window:

| `kind` | Names | Mechanism |
|---|---|---|
| `ditem` | a dialog item, 1-based | `GetDialogItem` → `Get`/`SetDialogItemText` |
| `dialogte` | the dialog's own live TERec (`DialogRecord.textH`) | `TEGetText`-equivalent / `TESetText` |
| `te` | a `TEHandle` the caller supplies | the same, on any TE the caller can name |

#### The rates, and the oracles

Two oracles, and the second one is the point. `textset`'s reply already carries
a read-back — the guest re-reads the object after writing it — but that travels
the same code, in the same call, through the same hook. So every trial is also
checked against `axtree`'s per-window `textEdit.text`, which `axtext.c` reads
from **outside** the process over the `ax_memory` seam. No shared code, no
shared call, no shared moment.

| Kind | read correct | write actuated | outside path agrees |
|---|---|---|---|
| `ditem` | 20/20 | 20/20 | 20/20 |
| `dialogte` | 20/20 | 20/20 | 20/20 |
| `te` (explicit handle) | 20/20 | 20/20 | 20/20 |

The read half is seeded through the guest's own **key plane**, never through
`textset` — a read test that writes with the thing it is testing proves nothing.

#### Does the APPLICATION agree?

That is a different claim from "the object holds our string", and it is the one
that a shadow copy would break: an app that keeps its own copy of a field would
read its copy and ignore ours, and no object-level oracle could tell. So ask
the application to act. SimpleText's **Save As** is a Dialog Manager dialog
whose filename is `editText` item 10. Write a name, press Return, and the
oracle is **a file on disk**, read back through the anchor.

**8/8 retitled the document window, and 8/8 produced a real file in
`Macintosh HD:Desktop Folder:` under exactly the name written.** SimpleText
reads the field.

#### No hijack — 0/20

`tests/nohijack-probe.py --case text`. With no armed window there is no
user's-call-answered hijack to have; the question takes the form that can lose
data instead — **does a write land on an object the request did not name?**
Two stimuli per trial, both aimed at the object NOT named:

| Stimulus | Result |
|---|---|
| the user's own typing in the `editText` field, while a `textset` is aimed at the `statText` label beside it | field untouched **0/20 hijacked**; the named label got the write 20/20 |
| a request naming a window outside the target's window list | refused **20/20** |

#### The identity check, and the wild read it used to allow

The guard is the same shape as `pt_trackcontrol_answer`'s `ControlHandle` test,
which is the one that holds:

1. The caller names the window **twice** — by window-list index and by title —
   and the guest refuses unless both agree, before anything is written. An index
   alone goes stale the moment a window opens, and a stale index on a *write* is
   how you edit the wrong document.
2. The hook independently requires that window to be in **its own** window list.
3. For `kind:"te"`, the TERec's `inPort` must be that same window — so a handle
   belonging to another process cannot be written through, because its `inPort`
   names none of our windows.

**A caller-supplied handle used to be dereferenced twice before any of that.**
Measured: `textset {kind:"te", handle:1234}` did not answer `bad_handle` — it
**timed out, and took SimpleText's open dialog with it.** `pt_handle_in_heap()`
now range-checks the handle and its master pointer against `ApplZone`'s own
limits first, the discipline `pt_shared` and `qd_shared` already use for the
system heap. Proven by mutation in the honest direction: the same probe against
the pre-fix binary destroys the dialog, and against the fixed one answers
`bad_handle` eight times over with the dialog alive.

It is a plausibility check and not a proof — an in-zone address that is not a
TEHandle still gets past it — which is why the `inPort` test remains and remains
the identity guard.

#### The redraw obligation

`TESetText` does **not** display (Inside Macintosh: Text, TextEdit), and a field
that shows stale text is the screen lying about the document. The TE path
therefore calls `TECalText`, erases and `TEUpdate`s the view rect, and also
`InvalRect`s it so the application's own update pass agrees — with the port
saved and restored, since we are running inside the app's `GetNextEvent`. The
dialog-item path needs none of that: `SetDialogItemText` draws the item itself.
One extra step there instead — if the item is the Dialog Manager's currently
open edit field (`DialogRecord.editField == item - 1`), it is re-selected, so
the live TERec reloads from the item handle rather than resurrecting the old
string on the next keystroke.

#### What was NOT tested

- **Only SimpleText.** The Find and Save As dialogs. No other application was
  driven at all — not the Finder, not a Control Panel, nothing.
- **No application *document* TextEdit view.** `kind:"te"` is the door to one,
  and it is verified end to end using a handle the guest itself reported — but
  **discovering** an app's private `TEHandle` is not implemented and is not
  possible from here by any documented route we found. The Dialog Manager
  publishes `DialogRecord.textH`; an application publishes nothing. Named, not
  papered over.
- **Pixels.** The redraw path is called and the TERec agrees, but nothing
  compared the screen. That is a render-shot or a human, and neither was run.
- **Text longer than 255 bytes**, and non-ASCII/MacRoman bytes. The buffer is
  Str255-shaped because `GetDialogItemText` takes a `Str255`; a longer TE record
  reports `truncated:true` with its true `length`, and that path is untested.
- **Two targets at once.** Single-flight, like every other op.
...
- **A bypass switch, in the shared block.** `portal {enabled:false}` and the
  ...
  **Decided 2026-07-31 (Michelle): on for now, flipped at a stable 0.1.**
  (Default-on is also why the menu leak was a live defect rather than a
  hypothetical one, and why it was fixed the day it was measured.) It
...
3. **No hijack** — with a request armed, a real user click on a different menu
   still does what the user asked. **PASSES for both ops as of 2026-07-31**:
   menu went 18/20 hijacked → 0/19, control stayed 0/20, and the user's own
   click went 0/20 → 19/19 (before, the hijack consumed it). The legitimate
   `menuinvoke` still lands 20/20 — a guard that also blocks the real request
   is not a fix, so both numbers are required, not one.

   **A new op is not done until this criterion has a number.** The menu leak
   existed for weeks because acceptance 3 was written down and never run.
