...
#include <AppleEvents.h>     /* AEDesc/AECreateDesc (the `script` verb) */
#include <AppleScript.h>     /* typeAppleScript */
#include <Components.h>      /* OpenDefaultComponent/CloseComponent */
#include <OSA.h>             /* OSADoScript, OSASetActiveProc */
#include <Folders.h>         /* FindFolder: name the boot volume, never hardcode it */
#include <OSUtils.h>
#include <string.h>
#include <stdio.h>
#include <stdarg.h>
 ...
 *
 * It is a HASH OVER THE SOURCES (cmake/buildstamp.cmake), not __DATE__/__TIME__.
 * A per-translation-unit clock stamp reports when *this file* was compiled: a
 * build whose only change was main.c recompiled main.c, relinked, and reported
 * an unchanged stamp - measured, and it had already bitten once. A stamp that
 * cannot detect a deploy is worse than none, because every measurement taken
 * against the binary it mislabels inherits the error. */
#include "mirrorbuildstamp.h"
#define kBuildStamp MIRROR_SRC_HASH " " MIRROR_BUILT_AT
                                  ...
                                  const char **emit_error_message)
{
    ...
    for (menu_index = 0; menu_index < list.count; menu_index++) {
        ...
        for (;;) {
            ...
            /* `title` is the ADDRESSABLE text: axmenu drops the leading NUL
             * bytes the system writes in front of Apple-menu entries, which
             * otherwise made the one menu every application has the one menu a
             * caller could not match by name. The count is reported when it is
             * non-zero (and only then - it is 0 for almost every item, and this
             * frame is budgeted) so the raw shape is never silently lost. */
            ...
            if (item.title_nul_prefix != 0
                && !append(out, cap, used, ",\"titleNulPrefix\":%u",
                           (unsigned int)item.title_nul_prefix)) {
                return ax_emit_fail(emit_error_code, emit_error_message,
                                    "overflow", "axtree menu item");
            }
            if (!append(out, cap, used, "}")) {
                           ...
                           const char *line, size_t len)
{
    ...
    /* A resident INIT older than this build has no armPointH field, so arming
     * it would leave the hijack guard off while we believed it on. Refuse
     * loudly: a stale extension is a reboot, a silent unguarded patch is not
     * something the caller can even see. */
    if (pt->version < 2UL) {
        return resp_error(out, cap, id, "portal_stale",
                          "the resident Portal predates the MenuSelect click "
                          "guard - reinstall the extension and reboot");
    }
    if (!pt->enabled) {
        return resp_error(out, cap, id, "portal_disabled",
                          "the Portal is installed but bypassed - "
    ...
    /* 1. Arm, in the target's context. The point we are going to press is part
     * of the request: the patch answers only a MenuSelect that receives THIS
     * press, so a user clicking any other menu chains through. Written before
     * `armed` is set, so the guard is never briefly absent. */
    pt->armPointH = (int32_t)(title_left + 4);
    pt->armPointV = 10;
 ...
 * it ask.
 *
 * The part code is the caller's, because only the caller knows the intent.
 * They are Control Manager constants, not ours to guess from a rect, and the
 * values are (Inside Macintosh, the Control Manager; ControlDefinitions.h):
 *
 *      10  inButton / kControlButtonPart      a push button
 *      11  inCheckBox / kControlCheckBoxPart  a check box or radio button
 *      20  inUpButton    scroll bar's up arrow      (one line)
 *      21  inDownButton  scroll bar's down arrow    (one line)
 *      22  inPageUp      scroll bar's page-up gutter
 *      23  inPageDown    scroll bar's page-down gutter
 *     129  inThumb / kControlIndicatorPart    the scroll bar's indicator
 *
 * This comment previously listed 10/11/12/13 for the four scroll-bar parts,
 * which is wrong in both directions: 10 and 11 are the BUTTON parts, and 12
 * and 13 are not part codes at all. A caller that believed it produced an
 * `answered:true` reply and a scroll bar that never moved, because the app's
 * action procedure was handed a part its scroll bar has no meaning for. No
 * phantom constants — these are checked against the header.
 */
                          ...
                          const char *line, size_t len)
{
    ...
    /* Re-read the control out of the GUEST, so the reply can carry evidence
     * rather than only its own say-so.
     *
     * `answered` says the application's TrackControl returned our part code —
     * nothing more, and that distinction is what four retracted findings were
     * made of. A control with a live range publishes its position in
     * contrlValue, so for those we can state what the guest itself now holds,
     * before and after. A push button has no such range (min == max == 0) and
     * this proves nothing about it: its effect is whatever its own handler did,
     * which only the CALLER can name and check. So the stronger claim is made
     * exactly where the guest supports it and nowhere else — `valueChanged`
     * appears only when the control had a range to move within.
     *
     * A failure to re-resolve is not an error: the act already happened. The
     * fields are simply absent, which is this project's rule for an unknown. */
    value_before = resolved.control.value;
    has_range = (resolved.control.max > resolved.control.min);
    evidence[0] = '\0';
    if (ax_open_target(&target, &expected, &expected, 0,
                       &error_code, &error_message)
        && ax_resolve_ref(&target.memory, target.sample.windowList,
                          &ref, &resolved) == AX_RESOLVE_OK) {
        short value_after = resolved.control.value;

        if (has_range) {
            snprintf(evidence, sizeof evidence,
                     "\"valueBefore\":%d,\"valueAfter\":%d,"
                     "\"valueChanged\":%s,",
                     (int)value_before, (int)value_after,
                     value_after != value_before ? "true" : "false");
        } else {
            /* No range means contrlValue carries no position to compare —
             * saying `valueChanged:false` about a push button would read as a
             * failure when it is simply the wrong question. */
            snprintf(evidence, sizeof evidence, "\"hasRange\":false,");
        }
    }

    n = snprintf(out, cap,
        "{\"proto\":1,\"id\":%ld,\"ok\":true,\"result\":{"
        "\"part\":%ld,\"answered\":true,\"sawActionProc\":%lu,%s"
        "\"mechanism\":\"portal-trackcontrol\","
        "\"availability\":\"metal-safe\"},"
        "\"backing\":\"" TBT_BACKING "\"}\n", id, part,
        (unsigned long)pt->sawActionProc, evidence);
    return (n < 0 || (size_t)n >= cap) ? -1 : n;
}
...
/*
 * `launch` - start an application, by path or by name.
 *
 * The MCP contract has specified `mirror.app {op:"launch"}` since day one and
 * the guest had no such verb, so every call answered `unknown_verb`. This makes
 * the contract true.
 *
 * MECHANISM: LaunchApplication (Process Manager), not the Finder's 'odoc'/'oapp'
 * Apple Event. The two behave differently exactly when it matters. An Apple
 * Event to the Finder needs the Finder scheduled and pumping its event loop to
 * answer, and this agent is a faceless background process talking to a host that
 * is driving other applications: a Finder busy in a tracking loop, showing a
 * modal, or simply not being given time answers late or not at all, and an
 * AESend that times out is indistinguishable here from an app that failed to
 * start. LaunchApplication is a direct Process Manager call in OUR context with
 * a synchronous OSErr, and it is the call the lab's worker already uses to
 * launch this very agent during spin-up - a path with a year of use behind it.
 *
 * The reply reports `launched` and the new PSN, which is what LaunchApplication
 * returned - NOT that a window exists. The caller's oracle is guest state: the
 * application appearing in `observe`, and its window in `axtree`.
 *
 * `name` search: an application named on the wire has no path, so it is looked
 * up by a BOUNDED catalog walk from a search root (default: the boot volume's
 * root, from FindFolder(kOnSystemDisk) + fsRtDirID). Bounded means bounded -
 * ticks, directories visited, and depth are all capped and all reported, so a
 * miss is never confused with a walk that ran out of budget. Every match is
 * counted: two applications with one name is an ambiguity the caller must
 * resolve with a path, not something to guess at.
 */
#define kLaunchMaxDirs   768     /* directories visited before giving up */
#define kLaunchMaxTicks  240UL   /* ~4 s at 60 Hz; the wire waits on us */
#define kLaunchMaxDepth  6       /* Vol:A:B:C:D:E - deeper than any app lives */
#define kLaunchStackMax  256     /* pending directories; static, not stack */
...
/* Walk for an 'APPL' whose file name equals `want` (case-insensitive, the HFS
 * rule - EqualString with caseSens=false, IM: Text). Returns the number of
 * matches found; the first match's FSSpec lands in `hit`. */
static int launch_find_app(short vref, long root_dir, const Str255 want,
                           FSSpec *hit, int *dirs_visited,
                           int *budget_exhausted)
{
    static launch_dir stack[kLaunchStackMax];
    CInfoPBRec        pb;
    Str255            iname;
    unsigned long     deadline;
    int               top = 0;
    ...
    deadline = (unsigned long)LMGetTicks() + kLaunchMaxTicks;
    *budget_exhausted = 0;
    stack[top].dir_id = root_dir;
    stack[top].depth = 1;
    top = 1;
    ...
    while (top > 0) {
        ...
        for (index = 1; ; index++) {
            memset(&pb, 0, sizeof(pb));
            pb.hFileInfo.ioNamePtr   = iname;
            pb.hFileInfo.ioVRefNum   = vref;
            pb.hFileInfo.ioDirID     = here.dir_id;  /* the call rewrites it */
            pb.hFileInfo.ioFDirIndex = index;
            if (PBGetCatInfoSync(&pb) != noErr) {
                break;                               /* past the last entry */
            }
            if ((pb.hFileInfo.ioFlAttrib & ioDirMask) != 0) {
                if (here.depth < kLaunchMaxDepth && top < kLaunchStackMax) {
                    stack[top].dir_id = pb.dirInfo.ioDrDirID;
                    stack[top].depth = (short)(here.depth + 1);
                    top++;
                }
                continue;
            }
            ...
            if (!EqualString(iname, want, false, true)) {
                continue;
            }
            matches++;
            if (matches == 1 && hit != NULL) {
                (void)FSMakeFSSpec(vref, here.dir_id, iname, hit);
            }
        }
    }
...
static int verb_launch(char *out, size_t cap, long id,
                       const char *line, size_t len)
{
    char                path[kPathMax];
    char                esc[kPathMax * 6 + 1];
    ...
    nlen = wire_find_str(line, len, "path", path, sizeof(path));
    if (nlen > 0) {
        ppath[0] = (unsigned char)nlen;
        memcpy(ppath + 1, path, (size_t)nlen);
        err = FSMakeFSSpec(0, 0, ppath, &spec);
        if (err != noErr) {
            /* A path that names nothing is `not_found` whichever segment of it
             * is missing: fnfErr (-43) is the leaf, dirNFErr (-120) an absent
             * folder, nsvErr (-35) an absent volume (IM: Files, result codes).
             * Measured: a bad middle segment returns -120, which reported as
             * `io_error` reads like a broken guest rather than a typo. */
            snprintf(msg, sizeof(msg), "FSMakeFSSpec err %d", (int)err);
            return resp_error(out, cap, id,
                              (err == fnfErr || err == dirNFErr
                               || err == nsvErr) ? "not_found" : "io_error",
                              msg);
        }
    } else {
        nlen = wire_find_str(line, len, "name", path, sizeof(path));
        if (nlen <= 0 || nlen > 255) {
            return resp_error(out, cap, id, "bad_request",
                              "launch needs a path or a name");
        }
        by_name = 1;
        want[0] = (unsigned char)nlen;
        memcpy(want + 1, path, (size_t)nlen);
        /* The boot volume, named by the OS rather than by us: hardcoding
         * "Macintosh HD" would be a phantom constant, and a search root that is
         * wrong on the next image is worse than none. fsRtDirID (2) is the
         * documented root directory ID (IM: Files). */
        if (FindFolder(kOnSystemDisk, kSystemFolderType, kDontCreateFolder,
                       &boot_vref, &boot_dir) != noErr) {
            return resp_error(out, cap, id, "io_error",
                              "FindFolder could not name the boot volume");
        }
        matches = launch_find_app(boot_vref, fsRtDirID, want, &spec,
                                  &dirs, &exhausted);
        if (matches == 0) {
            snprintf(msg, sizeof(msg),
                     "no application by that name (%d dirs%s)",
                     dirs, exhausted ? ", budget exhausted" : ", search complete");
            return resp_error(out, cap, id, "not_found", msg);
        }
        if (matches > 1) {
            snprintf(msg, sizeof(msg),
                     "%d applications share that name - pass a path", matches);
            return resp_error(out, cap, id, "ambiguous", msg);
        }
    }
    ...
    if (err != noErr) {
        /* memFullErr here is the classic one: the app's partition does not fit
         * in what is left, which is a real, reportable condition and not a bug
         * in the caller's path. */
        snprintf(msg, sizeof(msg), "LaunchApplication err %d", (int)err);
        return resp_error(out, cap, id,
                          (err == memFullErr) ? "no_memory" : "launch_failed",
                          msg);
    }

    pstr_escape(spec.name, esc, sizeof(esc));
    n = snprintf(out, cap,
        "{\"proto\":1,\"id\":%ld,\"ok\":true,\"result\":{"
        /* `launched`, not `performed`: LaunchApplication returned noErr and
         * handed back a PSN. Whether the application then opened a window is
         * the CALLER's to verify in guest state (observe/axtree) - reporting
         * the stronger claim is how a broken act plane stays invisible. */
        "\"name\":\"%s\",\"launched\":true,"
        "\"serialHi\":%lu,\"serialLo\":%lu,"
        "\"resolvedBy\":\"%s\",\"dirsVisited\":%d,\"searchTruncated\":%s,"
        "\"mechanism\":\"launch-application\","
        "\"availability\":\"metal-safe\"},"
        "\"backing\":\"" TBT_BACKING "\"}\n",
        id, esc,
        (unsigned long)pb.launchProcessSN.highLongOfPSN,
        (unsigned long)pb.launchProcessSN.lowLongOfPSN,
        by_name ? "name" : "path", dirs,
        exhausted ? "true" : "false");
    return (n < 0 || (size_t)n >= cap) ? -1 : n;
}

/* --- appended by lane H2 (2026-07-31): the `script` verb ------------------ */

/* AppleScript through the OSA component. The mirror's host has called this
 * verb since the desktop-disks work; the agent never had it, so every such
 * request failed silently into a `try?` and the feature simply never appeared
 * (the comment at ScenePoller's volumes branch is that bug, written down).
 *
 * P-DOC: OpenDefaultComponent(kOSAComponentType, typeAppleScript) and
 * OSADoScript (compile + execute + display-as-source, one call) are Universal
 * Interfaces / Inside Macintosh: Interapplication Communication, "Open
 * Scripting Architecture". OSADoScript's result is produced as OSADisplay
 * would - SOURCE form - so a text result arrives WRAPPED IN QUOTES. The host
 * strips them; do not "fix" that here without changing both ends.
 *
 * P-OBS (lab findings, ported as facts and re-implemented here - AGENTS.md
 * forbids porting the function, not the lesson):
 *   - `script-osa-1757-8.1`: a ~9 KB stack frame corrupted the OSA component.
 *     Every scratch buffer below is static; the agent is single-threaded and
 *     this handler never re-enters.
 *   - the same finding: classic AppleScript separates statements with CR
 *     (0x0D); AS 1.1 rejects an embedded LF. The source is normalised LF->CR
 *     before compiling. Harmless on OS 9.
 *   - 2026-07-05: a runaway script blocks the whole guest with no over-the-
 *     wire cancel (a whole-disk Finder search hung the harness ~12 minutes).
 *     Two independent stops: a `with timeout` wrapper so a blocked cross-app
 *     AESend returns errAETimeout at the cap, and an OSA active proc that
 *     aborts at the same deadline. The active proc must also YIELD
 *     (WaitNextEvent) - a deadline-only proc destabilised even fast scripts,
 *     because the scripted application never got to run and reply.
 *
 * An OSA error is DATA, not a transport failure: it comes back ok:true with
 * `osaErr` set and the error message text in `output`. Only a missing
 * component, a failed AEDesc, or the deadline are errors. */

#define kScriptSrcMax     2048      /* source bytes accepted from the host */
#define kScriptOutMax     4096      /* result bytes returned (pre-escape) */
#define kScriptDefaultMs  15000L
#define kScriptMaxMs      60000L    /* hard ceiling; the guest is serial */
#define kScriptCancelErr  (-128)    /* userCanceledErr: any nonzero aborts */
#define kAETimeoutErr     (-1712)   /* errAETimeout: `with timeout` fired */

static unsigned long g_script_deadline = 0;   /* TickCount; 0 = disarmed */
static int           g_script_timedout = 0;
static OSAActiveUPP  g_script_active   = NULL;
...
static pascal OSErr script_active_proc(long refCon)
{
    ...
    if (g_script_deadline != 0
        ...
        g_script_timedout = 1;
        return kScriptCancelErr;          /* nonzero -> OSA aborts cleanly */
    }
    /* Yield: the scripted app (usually the Finder) runs on the same
     * cooperative machine and cannot answer our Apple Event otherwise. */
    (void)WaitNextEvent(everyEvent, &evt, 1L, NULL);
    return noErr;
}

static int verb_script(char *out, size_t cap, long id,
                       const char *line, size_t len)
{
    /* Static, never locals - see the fat-frame finding above. */
    static char       src[kScriptSrcMax + 1];
    static char       wrapped[kScriptSrcMax + 64];
    static char       result[kScriptOutMax + 1];
    static char       esc[kScriptOutMax * 6 + 1];
    long              timeoutMs = 0;
    long              timeoutSec;
    int               slen, wlen, i;
    int               rlen = 0;
    ...
    srcDesc.descriptorType = typeNull;
    srcDesc.dataHandle     = NULL;
    resDesc.descriptorType = typeNull;
    resDesc.dataHandle     = NULL;

    slen = wire_find_str(line, len, "source", src, sizeof(src));
    if (slen == kWireOverflow) {
        return resp_error(out, cap, id, "overflow", "source too long");
    }
    if (slen < 0) {
        return resp_error(out, cap, id, "bad_request", "script needs source");
    }
    if (!wire_find_int(line, len, "timeoutMs", &timeoutMs)) {
        timeoutMs = kScriptDefaultMs;
    }
    if (timeoutMs < 500L) {
        timeoutMs = 500L;
    }
    if (timeoutMs > kScriptMaxMs) {
        timeoutMs = kScriptMaxMs;
    }
    timeoutSec = (timeoutMs + 999L) / 1000L;

    wlen = snprintf(wrapped, sizeof(wrapped),
                    "with timeout of %ld seconds\n%.*s\nend timeout",
                    timeoutSec, slen, src);
    if (wlen < 0 || (size_t)wlen >= sizeof(wrapped)) {
        memcpy(wrapped, src, (size_t)slen);      /* too big to wrap; raw */
        wlen = slen;
    }
    for (i = 0; i < wlen; i++) {
        if (wrapped[i] == '\n') {
    ...
    if (osa == NULL) {
        return resp_error(out, cap, id, "unavailable",
                          "no AppleScript OSA component");
    }
    err = AECreateDesc(typeChar, wrapped, (Size)wlen, &srcDesc);
    if (err != noErr) {
        (void)CloseComponent(osa);
        snprintf(msg, sizeof(msg), "AECreateDesc err %d", (int)err);
        return resp_error(out, cap, id, "io_error", msg);
    }
    ...
    g_script_deadline = (unsigned long)TickCount()
                        + (unsigned long)(timeoutMs * 3L / 50L);  /* ms->ticks */

    oerr = OSADoScript(osa, &srcDesc, kOSANullScript, typeChar,
                       kOSAModeCanInteract, &resDesc);
    AEDisposeDesc(&srcDesc);
    hitDeadline = (g_script_deadline != 0
                   && (unsigned long)TickCount() >= g_script_deadline);
    g_script_deadline = 0;

    if (resDesc.dataHandle != NULL) {
        Size sz = GetHandleSize((Handle)resDesc.dataHandle);

        rlen = (sz > (Size)kScriptOutMax) ? kScriptOutMax : (int)sz;
        HLock((Handle)resDesc.dataHandle);
        memcpy(result, *(Handle)resDesc.dataHandle, (size_t)rlen);
        HUnlock((Handle)resDesc.dataHandle);
    }
    result[rlen] = '\0';
    AEDisposeDesc(&resDesc);
    (void)CloseComponent(osa);

    /* A `with timeout` cap surfaces as a generic OSA error wrapping
     * errAETimeout, so any error that reached the deadline is a timeout. */
    if (g_script_timedout || oerr == kAETimeoutErr
        || (oerr != noErr && hitDeadline)) {
        return resp_error(out, cap, id, "timeout",
                          "script aborted at the deadline");
    }
    if (wire_escape(result, (size_t)rlen, esc, sizeof(esc)) < 0) {
        return resp_error(out, cap, id, "overflow", "script result");
    }
    n = snprintf(out, cap,
        "{\"proto\":1,\"id\":%ld,\"ok\":true,\"result\":{"
        "\"output\":\"%s\",\"osaErr\":%ld,\"truncated\":%s},"
        "\"backing\":\"" TBT_BACKING "\"}\n",
        id, esc, (long)oerr, (rlen >= kScriptOutMax) ? "true" : "false");
    }

/* ==========================================================================
 * Appended by lane P1 (2026-07-31): `winact`, the Portal's WINDOW_ACT.
 * New functions go at the END of this file and new dispatch rows at the END
 * of the table; nothing above is reordered. (docs/FANOUT.md, "the partition
 * is by FILE".)
 * ========================================================================== */

/*
 * Find one window in a target by title and occurrence.
 *
 * `ax_resolve_ref` cannot do this: it resolves a ref all the way down to a
 * CONTROL, and a window is addressed here without one. So this is the same
 * bounded walk with the control half removed — front-first down WindowList,
 * counting same-titled windows, refusing a cycle, capped like every other walk
 * in this file. `occurrence` is 0-based and matches the numbering `axtree`
 * emits, which is where a caller gets the title in the first place.
 */
static int winact_find_window(ax_target *target, const char *title,
                              size_t title_len, unsigned int occurrence,
                              ax_window_info *out, unsigned int *out_z)
{
    unsigned long address = target->sample.windowList;
    unsigned long seen[kAXMaxWins];
    ...
    while (address != 0) {
        ...
        seen[count] = address;
        rc = ax_read_window(&target->memory, address, &window);
        if (rc != AX_OK) {
            return rc;
        }
        if (window.title_len == title_len
            && memcmp(window.title, title, title_len) == 0) {
...
/* Emit one window rect the way `axtree` emits it: [left, top, right, bottom],
 * the CONTENT region in global coordinates (ax_read_window reads contRgn at
 * WindowRecord+118, so this is the same rectangle throughout). */
static int winact_rect(char *buf, size_t cap, const char *key,
                       const ax_window_info *w)
{
    int n = snprintf(buf, cap, "\"%s\":[%d,%d,%d,%d]", key,
                     (int)w->left, (int)w->top,
                     (int)w->right, (int)w->bottom);
    return (n < 0 || (size_t)n >= cap) ? -1 : n;
}

/*
 * `winact` — move, resize, zoom or close a window without a drag.
 *
 * This is the verb that takes QMP out of the act plane. Dragging a window by
 * coordinates needs injected mouse MOTION, and injected motion is QMP, and QMP
 * exists only on the emulator — so for as long as moving a window meant
 * dragging one, the act plane could not work on a real PowerBook. Everything
 * here is an in-guest Toolbox call the application itself would have made.
 *
 *   winact {serialHi, serialLo, title, occurrence, op, ...}
 *
 *   op:"move"    x, y   the new global top-left of the window's CONTENT region
 *   op:"resize"  w, h   the new content width and height, in pixels
 *   op:"zoom"    zoom:"in"|"out"
 *   op:"close"          ask the application to close it — see below
 *
 * `title` + `occurrence` rather than an `ax2/` ref because a ref resolves to a
 * control and a window has none to name. Occurrence is 0-based and is the same
 * numbering `axtree` reports, which is where the title comes from.
 *
 * ---- what `close` promises, and what it does not -------------------------
 *
 * This op can destroy a document, so its contract is written down rather than
 * left to be inferred. `winact close` does NOT promise the window closes. It
 * promises the application was ASKED to close it, in exactly the way a user
 * clicking the close box asks: the Portal answers the application's own
 * TrackGoAway with true, and the application's own close path runs from there.
 *
 * The consequence is the point. An unsaved document answers with a
 * save-changes dialog and the window stays open — and that is a CORRECT
 * outcome, reported as `windowGone:false` with `answered:true`, not as a
 * failure. The alternative, calling CloseWindow or DisposeWindow ourselves,
 * would close it every time by throwing the user's unsaved work away with no
 * prompt, and would leave the application holding a pointer to a window that no
 * longer exists. A caller that wants the window definitely gone must handle
 * the dialog, which it can see in `axtree` and act on with `ctlinvoke`.
 *
 * ---- part codes, cited ----------------------------------------------------
 *
 * The FindWindow part codes this verb drives are inDesk 0, inMenuBar 1,
 * inSysWindow 2, inContent 3, inDrag 4, inGrow 5, inGoAway 6, inZoomIn 7,
 * inZoomOut 8 — Inside Macintosh: Macintosh Toolbox Essentials, the Window
 * Manager, verified against `MacWindows.h:445-456` (Universal Interfaces 3.4).
 * They are named as PT_IN_* in ptshared.h and never spelled as bare numbers
 * here, because there is a SECOND, similarly-named set — the WDEF message
 * codes wInContent 1, wInDrag 2, wInGrow 3, wInGoAway 4, wInZoomIn 5,
 * wInZoomOut 6, `MacWindows.h:465-470` — and mistaking one for the other is
 * precisely the phantom-constant bug that cost `ctlinvoke` a day.
 */
static int verb_winact(char *out, size_t cap, long id,
                       const char *line, size_t len)
{
    volatile PTShared  *pt;
    static ax_target    target;      /* the record is far too big for 68K stack */
    static char         title[AX_TITLE_MAX + 1];
    static ax_window_info before;
    static ax_window_info after;
    ProcessSerialNumber psn;
    const char         *err_code = NULL;
    const char         *err_msg = NULL;
    char                op[16];
    ...
    char                rect_after[48];
    long                occurrence = 0;
    long                arg_x = 0, arg_y = 0, arg_w = 0, arg_h = 0;
    uint32_t            hi = 0, lo = 0;
    unsigned long       a5;
    unsigned long       deadline;
    unsigned int        z = 0;
    unsigned int        z_after = 0;
    int                 window_op;
    int                 zoom_part = 0;
    int                 title_len;
    int                 click_h;
    ...
    /* A resident INIT older than this build has no windowPtr/windowOp fields,
     * so arming one would write past the block it actually allocated — silent
     * corruption of shared memory rather than an error. Refuse loudly, the same
     * way `menuinvoke` refuses a Portal that predates its click guard. */
    if (pt->version < 3UL) {
        return resp_error(out, cap, id, "portal_stale",
                          "the resident Portal predates WINDOW_ACT - "
                          "reinstall the extension and reboot");
    }
    if (!pt->enabled) {
        return resp_error(out, cap, id, "portal_disabled",
                          "the Portal is installed but bypassed - "
    ...
    if (wire_find_str(line, len, "op", op, sizeof(op)) < 0) {
        return resp_error(out, cap, id, "bad_request",
                          "need op (move, resize, zoom or close)");
    }
    if (strcmp(op, "move") == 0) {
        window_op = PT_WIN_MOVE;
        if (!wire_find_int(line, len, "x", &arg_x)
            || !wire_find_int(line, len, "y", &arg_y)) {
            return resp_error(out, cap, id, "bad_request",
                              "move needs x and y (the new global top-left)");
        }
    } else if (strcmp(op, "resize") == 0) {
        window_op = PT_WIN_RESIZE;
        if (!wire_find_int(line, len, "w", &arg_w)
            || !wire_find_int(line, len, "h", &arg_h)) {
            return resp_error(out, cap, id, "bad_request",
                              "resize needs w and h (the new content size)");
        }
        if (arg_w < 1 || arg_h < 1 || arg_w > 32767L || arg_h > 32767L) {
            return resp_error(out, cap, id, "bad_request",
                              "w and h must be positive and fit in a short");
        }
    } else if (strcmp(op, "zoom") == 0) {
        window_op = PT_WIN_ZOOM;
        if (wire_find_str(line, len, "zoom", zoom, sizeof(zoom)) < 0) {
            return resp_error(out, cap, id, "bad_request",
                              "zoom needs zoom:\"in\" or zoom:\"out\"");
        }
        if (strcmp(zoom, "out") == 0) {
            zoom_part = PT_IN_ZOOM_OUT;     /* 8: out to the standard state */
        } else if (strcmp(zoom, "in") == 0) {
            zoom_part = PT_IN_ZOOM_IN;      /* 7: back to the user state    */
        } else {
            return resp_error(out, cap, id, "bad_request",
                              "zoom must be \"in\" or \"out\"");
        }
    } else if (strcmp(op, "close") == 0) {
        window_op = PT_WIN_CLOSE;
    } else {
        return resp_error(out, cap, id, "bad_request",
                          "op must be move, resize, zoom or close");
    }

    title_len = wire_find_str(line, len, "title", title, sizeof(title));
    if (title_len < 0) {
        return resp_error(out, cap, id, "bad_request",
                          "need title (the window title, as axtree reports it)");
    }
    (void)wire_find_int(line, len, "occurrence", &occurrence);
    if (occurrence < 0 || occurrence > 65535L) {
        return resp_error(out, cap, id, "bad_request",
                          "occurrence is 0-based and must fit in 16 bits");
    }

    /* Resolve the target the way every other AX verb does: a PSN on the wire,
     * an A5 world out of the oracle, defaulting to the front process. */
    if (wire_find_u32(line, len, "serialHi", &hi)
        && wire_find_u32(line, len, "serialLo", &lo)) {
        psn.highLongOfPSN = hi;
        psn.lowLongOfPSN = lo;
    ...
    psn = target.process;
    rc = winact_find_window(&target, title, (size_t)title_len,
                            (unsigned int)occurrence, &before, &z);
    if (rc != AX_RESOLVE_OK) {
        return resp_error(out, cap, id, "element_not_found",
                          "no window with that title and occurrence");
    }

    /* The click goes at the CENTRE of the content region, and where it lands
     * decides nothing: the FindWindow patch answers with the part the request
     * names, so the grow box and the close box never have to be located. That
     * is the whole reason FindWindow is patched at all — locating them means
     * inventing a title-bar height and a corner size no guest structure
     * reports, which is a phantom constant by another name.
     *
     * The centre is still the right place to aim, for two reasons that are not
     * about correctness: it is certainly inside the window, so if the patch
     * ever declines the click is at least harmless rather than landing on a
     * neighbour; and the patch matches on these exact coordinates, so the
     * point has to be one we can state. */
    click_h = (int)((before.left + before.right) / 2);
    click_v = (int)((before.top + before.bottom) / 2);

    pt->op = PT_OP_WINDOW_ACT;
    pt->windowOp = (int32_t)window_op;
    pt->windowPtr = (uint32_t)before.address;
    pt->zoomPart = (int32_t)zoom_part;
    pt->clickH = (int32_t)click_h;
    pt->clickV = (int32_t)click_v;
    if (window_op == PT_WIN_MOVE) {
        pt->winH = (int32_t)arg_x;
        pt->winV = (int32_t)arg_y;
    } else {
        pt->winH = (int32_t)arg_w;      /* GrowWindow's low word  */
        pt->winV = (int32_t)arg_h;      /* GrowWindow's high word */
    }
    pt->targetA5 = (uint32_t)a5;
    pt->error = PT_ERR_NONE;
    pt->armed = PT_ARM_NONE;
    pt->fired = 0;
    pt->findWindowFired = 0;
    pt->fwAnswers = 0;
    /* Zero the per-request counters, never the global ones: these describe
     * THIS request and would otherwise carry the last one's answer in. */
    pt->trapHitsTarget[0] = 0;
    pt->trapHitsTarget[1] = 0;
    pt->trapHitsTarget[2] = 0;
    pt->trapHitsTarget[3] = 0;
    pt->status = PT_STATUS_PENDING;

    /* Wait for the target to serve — and YIELD, never spin. A busy-wait holds
     * the CPU in THIS process under cooperative multitasking, so the target
     * never runs and never serves: the spin guarantees its own timeout. */
    deadline = (unsigned long)LMGetTicks() + 300UL;
    while (pt->status == PT_STATUS_PENDING
           && (unsigned long)LMGetTicks() < deadline) {
        EventRecord ev;
    ...
    if (pt->status != PT_STATUS_DONE) {
        uint32_t why = pt->error;

        pt->status = PT_STATUS_IDLE;
        pt->armed = PT_ARM_NONE;
        return resp_error(out, cap, id,
                          why == PT_ERR_NO_WIN_PATCH ? "no_patch"
                          : why == PT_ERR_BAD_WINDOW_OP ? "bad_request"
                          : why == PT_ERR_NO_WINDOW ? "element_not_found"
                          : "portal_timeout",
                          "the target did not serve (not pumping, or no patch)");
    }

    /* MOVE is served outright in the target's context — MoveWindow has already
     * run by the time the status flips, because DragWindow returns void and
     * there is no question for a patch to answer. The three that DO arm need a
     * click to make the application call FindWindow. */
    if (window_op != PT_WIN_MOVE) {
        if (pt->armed != PT_ARM_READY) {
            pt->status = PT_STATUS_IDLE;
            pt->armed = PT_ARM_NONE;
            return resp_error(out, cap, id, "not_armed",
                              "the target served but did not arm");
        }
        (void)post_click_at((short)click_h, (short)click_v, 1, 0);
        ...
        if (!pt->fired) {
            ...
            /* The counters are the whole diagnostic. A guarded patch cannot
             * answer, about itself, whether nothing happened because the trap
             * was never called or because it was called and declined — and
             * those are opposite repairs. `trapHits` is bumped at the top of
             * each answer function, before any guard, so the failure names
             * which one it is instead of leaving it to be guessed. */
            snprintf(detail, sizeof detail,
                     "%s (fwAnswers=%lu target find=%lu grow=%lu box=%lu "
                     "goaway=%lu; global find=%lu)",
                     pt->findWindowFired
                     ? "the app took the part code and did not call the trap "
                       "that goes with it"
                     : "the app never called FindWindow for our click",
                     (unsigned long)pt->fwAnswers,
                     (unsigned long)pt->trapHitsTarget[0],
                     (unsigned long)pt->trapHitsTarget[1],
                     (unsigned long)pt->trapHitsTarget[2],
                     (unsigned long)pt->trapHitsTarget[3],
                     (unsigned long)pt->trapHits[0]);
            /* DISARM on the way out, always. A request left armed is a patch
             * waiting to fire on somebody else's click, which is the one
             * failure mode a trap patch must not have. */
            pt->armed = PT_ARM_NONE;
            pt->status = PT_STATUS_IDLE;
            return resp_error(out, cap, id, "not_taken", detail);
        }
    }
    pt->status = PT_STATUS_IDLE;

    /* ---- the oracle: the window's own rect, re-read out of the guest -------
     *
     * `answered` means only that the patch answered, or that MoveWindow was
     * called. That is not evidence, and this project has four retracted
     * findings made of treating it as evidence. So re-open the target and walk
     * to the window again: `rectAfter` is what the guest itself now holds.
     *
     * A window that has gone is not a failure to re-read — for close it is the
     * whole result — so `windowGone` is reported rather than an error. */
    window_gone = 0;
    rect_before[0] = '\0';
    rect_after[0] = '\0';
    (void)winact_rect(rect_before, sizeof rect_before, "rectBefore", &before);
    if (ax_open_target(&target, &psn, NULL, 1, &err_code, &err_msg)) {
        rc = winact_find_window(&target, title, (size_t)title_len,
                                (unsigned int)occurrence, &after, &z_after);
        if (rc == AX_RESOLVE_OK) {
            (void)winact_rect(rect_after, sizeof rect_after, "rectAfter",
                              &after);
        } else if (rc == AX_RESOLVE_NOT_FOUND) {
            window_gone = 1;
        }
    }
    ...
    n = snprintf(out, cap,
        "{\"proto\":1,\"id\":%ld,\"ok\":true,\"result\":{"
        "\"op\":\"%s\",\"answered\":true,"
        "\"findWindowFired\":%s,\"fwAnswers\":%lu,\"windowGone\":%s,"
        "%s%s%s%s"
        /* `portal-movewindow` and `portal-findwindow` are different mechanisms
         * and the reply says which one ran, because they degrade differently:
         * one is a direct Window Manager call, the other depends on the
         * application dispatching a click it was handed. */
        "\"mechanism\":\"%s\","
        /* No QMP anywhere in this path — PostEvent and Toolbox calls only —
         * which is what metal-safe means here. It is NOT a safety review;
         * docs/PORTAL-PLAN.md keeps every op emulator-only until one exists. */
        "\"availability\":\"metal-safe\"},"
        "\"backing\":\"" TBT_BACKING "\"}\n",
        id, op,
        pt->findWindowFired ? "true" : "false",
        (unsigned long)pt->fwAnswers,
        window_gone ? "true" : "false",
        rect_before, rect_before[0] != '\0' ? "," : "",
        rect_after, rect_after[0] != '\0' ? "," : "",
        window_op == PT_WIN_MOVE ? "portal-movewindow" : "portal-findwindow");
    return (n < 0 || (size_t)n >= cap) ? -1 : n;
}


/* --- Portal ops 6 and 7: `textget` / `textset` (lane P2, 2026-07-31) ------
 *
 * Read a text field's contents, and write them, IN THE APPLICATION'S OWN
 * CONTEXT — the difference between driving an app and filling in a form.
 *
 * Why it needs the Portal at all: a TextEdit record and a dialog's item list
 * are per-process Toolbox roots. The AX walk can READ a dialog's TERec from
 * outside (axtree's `textEdit`, via axtext.c) because it has an ax_memory seam
 * into the target's partition — but it cannot WRITE, and it cannot ask the
 * Dialog Manager anything, because a foreign process's item list is a handle
 * only that process can dereference. From inside the GNEFilter hook both are
 * ordinary memory the Toolbox will hand us on request.
 *
 * Addressing, and the identity check that IS the guard. The Portal's earlier
 * ops taught this the hard way (PORTAL-PLAN.md, 2026-07-31: MENU_INVOKE checked
 * only "armed in this process" and hijacked a real user's click 18/20). So the
 * caller names the window TWICE — by its window-list index and by its title —
 * and the guest refuses unless both agree, before anything is written. The hook
 * then independently requires that window to be in ITS OWN window list, and for
 * a caller-supplied TEHandle, that the TERec's `inPort` be that same window.
 * Nothing is written on the strength of "a request is pending".
 *
 *   textget {windowZ, window?, kind?, item?, handle?, serialHi?, serialLo?}
 *   textset {windowZ, window?, kind?, item?, handle?, text, ...}
 *
 * `kind` is "ditem" (a dialog item by 1-based number), "dialogte" (the
 * dialog's own live TERec — DialogRecord.textH), or "te" (a TEHandle the
 * caller names, which is how an application document window is reached; the
 * handle for one is not discoverable from here, and that gap is stated in
 * PORTAL-PLAN.md rather than papered over). It defaults to "ditem" when an
 * `item` is given and "dialogte" otherwise.
 */

/* Walk the target's window list to index `z`, reporting the address and title.
 * Bounded like every other walk here — a corrupt chain costs a loop count. */
#define kTextMaxWindowWalk 64

static int text_window_at(const ax_target *target, long z,
                          unsigned long *addr_out, ax_window_info *info_out)
{
    unsigned long addr = (unsigned long)target->sample.windowList;
    long          i;

    if (z < 0) {
        return 0;
    }
    for (i = 0; addr != 0 && i < kTextMaxWindowWalk; i++) {
        ax_window_info w;

        if (ax_read_window(&target->memory, addr, &w) != AX_OK) {
            return 0;
        }
        if (i == z) {
            *addr_out = addr;
...
/* Everything `textget` and `textset` share: resolve the target and the window,
 * post the request, yield until the hook serves it, and take a seqlock-coherent
 * snapshot of the reply. Returns 0 having already written an error envelope. */
static int text_exchange(char *out, size_t cap, long id,
                         const char *line, size_t len, int is_set,
                         PTShared *snap_out)
{
    volatile PTShared  *pt;
    static ax_target    target;
    ...
    char                body[PT_TEXT_MAX + 1];
    unsigned long       window_addr = 0;
    unsigned long       a5;
    unsigned long       deadline;
    uint32_t            hi = 0, lo = 0, handle = 0;
    long                window_z = 0;
    long                item = 0;
    uint32_t            kind;
    int                 kind_len, title_len, body_len;
    int                 i;
    ...
    /* A resident INIT older than this build allocated a block with no room
     * for textBuf, so writing the request into it would run off the end of a
     * system-heap pointer. Refuse loudly — a stale extension is a reboot. */
    if (pt->version < 4UL) {
        return resp_error(out, cap, id, "portal_stale",
                          "the resident Portal predates the text ops - "
                          "reinstall the extension and reboot");
    }
    if (!pt->enabled) {
        return resp_error(out, cap, id, "portal_disabled",
    ...
    if (!wire_find_int(line, len, "windowZ", &window_z)) {
        return resp_error(out, cap, id, "bad_request",
                          "need windowZ (the window's z from axtree)");
    }
    (void)wire_find_int(line, len, "item", &item);
    (void)wire_find_u32(line, len, "handle", &handle);

    kind_len = wire_find_str(line, len, "kind", kind_text, sizeof(kind_text));
    if (kind_len == kWireOverflow) {
        return resp_error(out, cap, id, "bad_request", "kind too large");
    }
    if (kind_len > 0) {
        if (strcmp(kind_text, "ditem") == 0) {
            kind = PT_TEXT_KIND_DITEM;
        } else if (strcmp(kind_text, "dialogte") == 0) {
            kind = PT_TEXT_KIND_DIALOG_TE;
        } else if (strcmp(kind_text, "te") == 0) {
            kind = PT_TEXT_KIND_TE;
        } else {
            return resp_error(out, cap, id, "bad_request",
                              "kind must be ditem, dialogte or te");
        }
    } else {
        kind = (item > 0) ? PT_TEXT_KIND_DITEM : PT_TEXT_KIND_DIALOG_TE;
    }
    if (kind == PT_TEXT_KIND_DITEM && item < 1) {
        return resp_error(out, cap, id, "bad_request",
                          "kind ditem needs a 1-based item");
    }
    if (kind == PT_TEXT_KIND_TE && handle == 0) {
        return resp_error(out, cap, id, "bad_request",
                          "kind te needs a handle");
    }

    body_len = 0;
    body[0] = '\0';
    if (is_set) {
        body_len = wire_find_str(line, len, "text", body, sizeof(body));
        if (body_len == kWireOverflow) {
            return resp_error(out, cap, id, "bad_request",
                              "text longer than 255 bytes");
        }
        if (body_len < 0) {
            return resp_error(out, cap, id, "bad_request", "textset needs text");
        }
    }
    ...
    if (!text_window_at(&target, window_z, &window_addr, &win)) {
        return resp_error(out, cap, id, "element_not_found",
                          "no window at that windowZ");
    }
    /* The second half of the identity check. A z index alone goes stale the
     * moment a window opens or comes forward, and a stale index on a WRITE is
     * how you edit the wrong document. When the caller also names the title,
     * the two must agree or nothing happens. */
    title_len = wire_find_str(line, len, "window", title, sizeof(title));
    if (title_len == kWireOverflow) {
        return resp_error(out, cap, id, "bad_request", "window title too large");
    }
    if (title_len >= 0) {
        if ((size_t)title_len != (size_t)win.title_len
            || memcmp(title, win.title, (size_t)title_len) != 0) {
            return resp_error(out, cap, id, "element_not_found",
                              "the window at that z has a different title");
        }
    }

    pt->op = is_set ? PT_OP_TEXT_SET : PT_OP_TEXT_GET;
    pt->textKind = kind;
    pt->textWindow = (uint32_t)window_addr;
    pt->textHandle = handle;
    pt->textItem = (int32_t)item;
    pt->textLength = (int32_t)body_len;
    pt->textBufLength = 0;
    pt->textItemType = 0;
    pt->textTE = 0;
    for (i = 0; i < body_len; i++) {
        pt->textBuf[i] = (uint8_t)(unsigned char)body[i];
    }
    pt->targetA5 = (uint32_t)a5;
    pt->error = PT_ERR_NONE;
    pt->status = PT_STATUS_PENDING;

    /* Yield, never spin: under cooperative multitasking a busy-wait holds the
     * CPU in OUR process, so the target never runs and never serves. */
    deadline = (unsigned long)LMGetTicks() + 300UL;
    while (pt->status == PT_STATUS_PENDING
           && (unsigned long)LMGetTicks() < deadline) {
        EventRecord ev;
    ...
    if (pt->status == PT_STATUS_PENDING) {
        pt->status = PT_STATUS_IDLE;              /* withdraw it */
        return resp_error(out, cap, id, "portal_timeout",
                          "the target did not pump its event loop in ~5 s");
    }
    ...
    for (i = 0; i < 8; i++) {
        ...
        memcpy(snap_out, (const void *)pt, sizeof(PTShared));
        if (pt->seq == before) {
            break;
        }
    }
    ...
    if (snap_out->status != PT_STATUS_DONE) {
        const char *code = "portal_error";
        const char *msg = "the target refused the request";

        switch (snap_out->error) {
        case PT_ERR_TEXT_NO_WINDOW:
            code = "element_not_found";
            msg = "that window is not in the target process's window list";
            break;
        case PT_ERR_NOT_DIALOG:
            code = "not_a_dialog";
            msg = "windowKind is not dialogKind, so it has no item list";
            break;
        case PT_ERR_NO_ITEM:
            code = "element_not_found";
            msg = "no such dialog item";
            break;
        case PT_ERR_NOT_TEXT:
            code = "not_text";
            msg = "that dialog item is not editText or statText";
            break;
        case PT_ERR_BAD_TE:
            code = "bad_handle";
            msg = "the TEHandle is empty, or its inPort is not that window";
            break;
        case PT_ERR_TEXT_KIND:
            code = "bad_request";
            msg = "unknown text kind";
            break;
        default:
            break;
        }
        return resp_error(out, cap, id, code, msg);
    }
    return 1;
}

/* Emit the shared reply body. `text` is what the OBJECT holds now — on a set
 * that is a read-back through the Dialog Manager or the TERec, not an echo of
 * what we sent, because an echo would prove only that we can remember a
 * string. It is still the same round trip, so a probe must ALSO re-read
 * independently; see tests/textops-probe.py. */
static int text_reply(char *out, size_t cap, long id, const PTShared *s,
                      const char *mechanism, const char *extra)
{
    char esc[PT_TEXT_MAX * 6 + 1];
    int  n;

    if (wire_escape((const char *)s->textBuf, (size_t)s->textBufLength,
                    esc, sizeof(esc)) < 0) {
        return resp_error(out, cap, id, "overflow", "text");
    }
    n = snprintf(out, cap,
        "{\"proto\":1,\"id\":%ld,\"ok\":true,\"result\":{"
        "\"text\":\"%s\",\"length\":%ld,\"returned\":%ld,"
        "\"truncated\":%s,\"itemType\":%ld,\"teHandle\":%lu,%s"
        "\"mechanism\":\"%s\","
        "\"availability\":\"metal-safe\"},"
        "\"backing\":\"" TBT_BACKING "\"}\n",
        id, esc, (long)s->textLength, (long)s->textBufLength,
        (s->textLength > s->textBufLength) ? "true" : "false",
        (long)s->textItemType, (unsigned long)s->textTE, extra, mechanism);
    return (n < 0 || (size_t)n >= cap) ? -1 : n;
}

static int verb_textget(char *out, size_t cap, long id,
                        const char *line, size_t len)
{
    static PTShared snap;
    int             rc;

    rc = text_exchange(out, cap, id, line, len, 0, &snap);
    if (rc != 1) {
        return rc;
    }
    return text_reply(out, cap, id, &snap, "portal-text-get", "");
}

static int verb_textset(char *out, size_t cap, long id,
                        const char *line, size_t len)
{
    static PTShared snap;
    int             rc;

    rc = text_exchange(out, cap, id, line, len, 1, &snap);
    if (rc != 1) {
        return rc;
    }
    /* `written`, not `performed`: the field now holds this, read back out of
     * the object. Whether the APPLICATION agrees — whether its own shadow copy
     * of the string, if it keeps one, was updated too — is a separate question
     * this verb deliberately does not answer. */
    return text_reply(out, cap, id, &snap, "portal-text-set",
                      "\"written\":true,");
}

/* Appended by lane G1 (2026-07-31): `apple-event`. The includes ride with the
 * verb rather than joining the block at the top of the file, so this lane's
 * addition is one contiguous append and merges beside the other lanes'.
 * AEDataModel declares AECreateDesc/AESend; Aliases declares NewAliasMinimal
 * for the odoc/pdoc direct object. */
#include <AppleEvents.h>
#include <AEDataModel.h>
#include <Aliases.h>

/* apple-event - send a core Apple Event to another process (cross-app control).
 * `event` is quit/oapp/odoc/pdoc and serialHi/serialLo address the target (the
 * PSN `observe` reports); odoc/pdoc also take `path`, the document.
 *
 * Ported from the lab's harness/src/verbs.c verb_apple_event as a FACT, not a
 * function (AGENTS.md): same whitelist, same PSN addressing, our own code.
 *
 * The host has sent this verb for `mirror.app {op:"quit"}` since day one and
 * the guest answered `unknown_verb` — the same hole lane G1 found under
 * `launch`. Unlike `launch`, quit is the Apple Event and nothing else: the
 * Process Manager has no "ask this app to quit" call, and a cmd-Q keystroke is
 * an input-injection act on a different plane.
 *
 * Fire-and-forget (kAENoReply): a reply would mean blocking our single-threaded
 * loop on a foreign app's event loop, and the answer we want is not the target's
 * acknowledgement anyway. So this reports `sent`, never `performed` — an app
 * with an unsaved document answers a quit AE by putting up a save-changes alert
 * and STAYING RUNNING, which is a correct outcome and only visible in `observe`.
 * kAECanInteract is what permits that alert to appear rather than the AE failing.
 *
 * Above the bright line: the AppleEvent Manager is a first-class OS API, so this
 * is metal-safe, unlike the emulator-only input planes. */
static int verb_apple_event(char *out, size_t cap, long id,
                            const char *line, size_t len)
{
    char                event[16];
    char                esc[kEscMax];
    ...
    if (wire_find_str(line, len, "event", event, sizeof(event)) == kWireAbsent) {
        return resp_error(out, cap, id, "bad_request", "missing event");
    }
    /* An overflowing name is left truncated at 15 chars — never a false match
     * against the whitelist below, which is the only thing it feeds. */
    if (!wire_find_u32(line, len, "serialHi", &shi)
        || !wire_find_u32(line, len, "serialLo", &slo)) {
        return resp_error(out, cap, id, "bad_request",
                          "missing or invalid unsigned serialHi/serialLo");
    }
    if (strcmp(event, "quit") == 0) {
        eid = kAEQuitApplication;
    } else if (strcmp(event, "oapp") == 0) {
        eid = kAEOpenApplication;
    } else if (strcmp(event, "odoc") == 0) {
        eid = kAEOpenDocuments;
    } else if (strcmp(event, "pdoc") == 0) {
        eid = kAEPrintDocuments;
    } else {
        /* A whitelist, not a passthrough: an arbitrary event class from the
         * wire is a much larger surface than the four core events the control
         * contract names, and each of the four has a known effect we can state. */
        return resp_error(out, cap, id, "bad_request",
                          "event must be quit/oapp/odoc/pdoc");
    }
    wantsDoc = (eid == kAEOpenDocuments || eid == kAEPrintDocuments);

    psn.highLongOfPSN = (unsigned long)shi;
    psn.lowLongOfPSN  = (unsigned long)slo;
    err = AECreateDesc(typeProcessSerialNumber, &psn, sizeof(psn), &target);
    if (err != noErr) {
        snprintf(msg, sizeof(msg), "AECreateDesc err %d", (int)err);
        return resp_error(out, cap, id, "ae_error", msg);
    }
    err = AECreateAppleEvent(kCoreEventClass, eid, &target,
                             kAutoGenerateReturnID, kAnyTransactionID, &ae);
    AEDisposeDesc(&target);
    if (err != noErr) {
        snprintf(msg, sizeof(msg), "AECreateAppleEvent err %d", (int)err);
        return resp_error(out, cap, id, "ae_error", msg);
    }
    ...
    if (wantsDoc) {
        ...
        if (plen == kWireOverflow) {
            AEDisposeDesc(&ae);
            return resp_error(out, cap, id, "overflow", "path too long");
        }
        if (plen < 0 || plen > 255) {
            AEDisposeDesc(&ae);
            return resp_error(out, cap, id, "bad_request",
                              "odoc/pdoc needs path");
        }
        ppath[0] = (unsigned char)plen;
        memcpy(ppath + 1, path, (size_t)plen);
        err = FSMakeFSSpec(0, 0, ppath, &spec);
        if (err != noErr) {
            AEDisposeDesc(&ae);
            /* Same reading as `list`/`stat`/`launch`: any absent segment is
             * `not_found`, whether it is the leaf (-43), a folder (-120), or
             * the volume (-35). */
            snprintf(msg, sizeof(msg), "FSMakeFSSpec err %d", (int)err);
            return resp_error(out, cap, id,
                              (err == fnfErr || err == dirNFErr
        ...
        if (err != noErr || alias == NULL) {
            AEDisposeDesc(&ae);
            return resp_error(out, cap, id, "ae_error", "alias failed");
        }
        ...
        if (err == noErr) {
            (void)AEPutDesc(&list, 1, &fileDesc);
        ...
        if (err != noErr) {
            AEDisposeDesc(&ae);
            return resp_error(out, cap, id, "ae_error", "doc desc failed");
        }
    }

    /* Initialised before the send: with kAENoReply the AppleEvent Manager
     * returns a null descriptor here, but on an early error it may not write
     * `reply` at all, and disposing a stack-garbage descriptor is a crash we
     * would have to reproduce on metal to believe. */
    ...
    AEDisposeDesc(&ae);
    if (err != noErr) {
        /* procNotFound (-600) is the PSN that no longer names a process — a
         * stale psn from an older `observe`, not a broken caller. */
        snprintf(msg, sizeof(msg), "AESend err %d", (int)err);
        return resp_error(out, cap, id,
                          (err == procNotFound) ? "not_found" : "ae_error", msg);
    }

    (void)wire_escape(event, strlen(event), esc, sizeof(esc));
    n = snprintf(out, cap,
        "{\"proto\":1,\"id\":%ld,\"ok\":true,\"result\":{"
        /* `sent`, not `performed`: the event left here. Whether the target quit,
         * or put up a save-changes alert and stayed, is the CALLER's to read out
         * of guest state — reporting the stronger claim is how a broken act
         * plane stays invisible (the same rule `launch` states). */
        "\"event\":\"%s\",\"serialHi\":%lu,\"serialLo\":%lu,\"sent\":true,"
        "\"mechanism\":\"apple-event\",\"availability\":\"metal-safe\"},"
        "\"backing\":\"" TBT_BACKING "\"}\n",
        id, esc,
        (unsigned long)psn.highLongOfPSN, (unsigned long)psn.lowLongOfPSN);
    return (n < 0 || (size_t)n >= cap) ? -1 : n;
                         ...
                         const char *line, size_t len)
{
    ...
    /* Appended by lane G1 (2026-07-31): the contract's `mirror.app
     * {op:"launch"}` had no guest verb behind it. New rows go at the END. */
    if (strcmp(verb, "launch") == 0) {
        return verb_launch(out, cap, id, line, len);
    }
    /* Appended by lane H2 (2026-07-31): the host has called `script` since the
     * desktop-disks work and no guest verb answered it. New rows go at the
     * END. */
    if (strcmp(verb, "script") == 0) {
        return verb_script(out, cap, id, line, len);
    }

    /* Appended by lane P1 (2026-07-31): the Portal's WINDOW_ACT. Named
     * `winact` and not `window` because `close` is already taken by the
     * capture transfer, and a window op called `close` next to it is the kind
     * of collision that reads fine and does the wrong thing. */
    if (strcmp(verb, "winact") == 0) {
        return verb_winact(out, cap, id, line, len);
    }
    /* Appended by lane P2 (2026-07-31): Portal ops 6 and 7, text read and
     * write in the application's own context. New rows go at the END. */
    if (strcmp(verb, "textget") == 0) {
        return verb_textget(out, cap, id, line, len);
    }
    if (strcmp(verb, "textset") == 0) {
        return verb_textset(out, cap, id, line, len);
    }
    /* Appended by lane G1 (2026-07-31): `mirror.app {op:"quit"}` sends this
     * verb and had no guest verb behind it either. New rows go at the END. */
    if (strcmp(verb, "apple-event") == 0) {
        return verb_apple_event(out, cap, id, line, len);
    }
