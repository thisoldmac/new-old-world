...
enum {
    ...
    /* --- text ops (5, 6). Appended 2026-07-31; existing codes unmoved. --- */
    PT_ERR_NO_WINDOW = 8,       /* the named window is not in THIS process's
                                 * window list — the identity check failing,
                                 * which is the case that must never write   */
    PT_ERR_NOT_DIALOG = 9,      /* windowKind != dialogKind, so it has no
                                 * item list to index                        */
    PT_ERR_NO_ITEM = 10,        /* item number outside 1..CountDITL          */
    PT_ERR_BAD_TE = 11,         /* the TEHandle is NULL, purged, or its
                                 * inPort is not the window named            */
    PT_ERR_TEXT_KIND = 12,      /* unknown textKind                          */
    PT_ERR_NOT_TEXT = 13        /* the dialog item holds no text (not
...
typedef struct {
     ...
     * Appended 2026-07-31 at the END of the record on purpose: the INIT and the
     * app each compile their own copy of this header, and a field inserted
     * among the existing ones would leave a deployed pair silently disagreeing
     * about every offset after it. */
