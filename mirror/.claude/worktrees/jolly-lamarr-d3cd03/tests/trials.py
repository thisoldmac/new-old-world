...
class Agent:
    ...
    # --- split send/receive, for measuring what happens WHILE a verb runs ----
    #
    # `call` is blocking, which is right for every case above: a verb is asked
    # and answered. The no-hijack probe needs the other shape — a Portal
    # request is ARMED for as long as the verb is waiting, and the whole
    # question is what a real click does during that window. So the request is
    # sent, the stimulus is injected from OUTSIDE the guest (QMP), and only
    # then is the reply read.
    #
    # Deliberately WITHOUT the retry `call` has: a retry re-sends the request,
    # which would arm the Portal a second time and quietly measure a machine in
    # a state the trial never asked for.

    def send_async(self, verb: str, args: dict | None = None) -> int:
        """Send one request and return its id, without reading the reply."""
        self._id += 1
        req = {"proto": 1, "id": self._id, "verb": verb}
        if args:
            req.update(args)
    ...
    def read_reply(self, timeout: float | None = None) -> dict:
        """Read the whole reply object, `ok` included — never raises on
        `ok:false`, because for this probe an honest refusal is the EXPECTED
        answer and must be readable as data."""
        s = self._connect()
        old = s.gettimeout()
