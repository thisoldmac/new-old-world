...
public struct ScenePoller {
    ...
    /// Fetch Finder-window icon-view items. OFF: `fdLocation` is a saved
    /// LOGICAL grid the Finder rescales when it lays a window out (128px
    /// steps stored vs ~172px drawn here), and the Finder doesn't expose
    /// live window-item positions via AppleScript — so rendered positions
    /// don't match the guest and clicks miss. The reliable source is where
    /// the Finder actually PlotIcon'd them: the QuickDraw content plane
    /// (qdpeek). Re-enable when that lands with real positions.
    public var includeWindowItems = false
    ...
    public mutating func poll(_ plane: Plane = .axtree,
        ...
        if includeDesktopItems {
            ...
            // Finder folder windows: the Finder itself tells us each window's
            // folder (`item of window` → HFS path), so ANY window resolves —
            // subfolders, the disk root, control panels. Then list each folder
            // for its icon-view items (window-local coords). Gated: the
            // positions aren't guest-accurate yet (see includeWindowItems).
            let finderTitles = includeWindowItems
                ? scene.windows
                    .filter { $0.app == "Finder" && !HitTester.isDesktopBackdrop($0) }
                    .map(\.title)
                : []
            if !finderTitles.isEmpty {
                resolveFinderPaths(neededTitles: finderTitles)
                for i in scene.windows.indices {
                    let win = scene.windows[i]
                    guard win.app == "Finder",
                          !HitTester.isDesktopBackdrop(win),
                          let path = finderPaths[win.title] else { continue }
                    if let (result, _) = try? wire.request("list", ["path": path]) {
                        scene.windows[i].items =
                            SceneBuilder.desktopItems(from: result)
                    }
                }
            }
        }
    ...
    /// Finder window title → its folder's HFS path, from the Finder itself.
    /// Cached; refreshed only when a current window title isn't yet known
    /// (a new/navigated window) — one `script` call, not one per poll.
    private var finderPaths: [String: String] = [:]

    /// AppleScript: every Finder window as `name|path;;` (item of window is
    /// the shown folder — works for subfolders, disk roots, control panels).
    private static let finderWindowsScript = [
        "tell application \"Finder\"",
        "set r to \"\"",
        "repeat with i from 1 to (count windows)",
        "set p to \"\"",
        "try",
        "set p to (item of window i) as text",
        "end try",
        "set r to r & (name of window i) & \"|\" & p & \";;\"",
        "end repeat",
        "return r",
        "end tell",
    ].joined(separator: "\n")

    private mutating func resolveFinderPaths(neededTitles: [String]) {
        // Skip the script call when every current window is already resolved.
        if neededTitles.allSatisfy({ finderPaths[$0] != nil }) { return }
        guard let (result, _) = try? wire.request(
                "script", ["source": Self.finderWindowsScript]),
              var raw = result["output"] as? String else { return }
        // The script verb returns the AppleScript text in source form (outer
        // quotes); strip them, then split records/fields.
        if raw.hasPrefix("\""), raw.hasSuffix("\""), raw.count >= 2 {
            raw = String(raw.dropFirst().dropLast())
        }
        ...
        for record in raw.components(separatedBy: ";;") where !record.isEmpty {
            guard let bar = record.firstIndex(of: "|") else { continue }
            let name = String(record[..<bar])
            let path = String(record[record.index(after: bar)...])
            if !path.isEmpty { map[name] = path }
        }
        finderPaths = map   // fresh snapshot of what's open now
    }
