...
0. **The `MENU_INVOKE` guard leaks — an armed request fires on the USER's
   click.** 18/20, measured; the control guard is 0/20 under the same test. The
   Portal is default-**on**, so this is live. Cause, exposure window and the
   cheap fix are in "No hijack" below. Two smaller ones from the same
   measurement: the **guest never ages a request out** (an agent that dies
   mid-verb leaves a patch armed indefinitely), and **both arming verbs warp the
   user's pointer**. Assigned to the `WINDOW_ACT` lane, ahead of `WINDOW_ACT`.
...
- **`mirror.app {op:"quit"}` is broken exactly the way `launch` was** — the host
  sends wire verb `apple-event`, which the guest does not implement. A chip is
  adding the guest verb; the host side is then one line and a live check.
- **`WINDOW_ACT` (Portal op 4) and `TEXT_GET`/`TEXT_SET` (op 5)** — op 4 is what
  removes QMP from the act plane, and with it the last emulator-only mechanism
  the plan forbids as load-bearing.
...
## No hijack (Portal acceptance 3) — measured 2026-07-31
