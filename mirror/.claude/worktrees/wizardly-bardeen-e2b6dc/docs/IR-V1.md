...
| `windows[].items` | renderer only, in-process | no | **dropped** — see below |
...
- **`windows[].items`** — `ScenePoller.includeWindowItems` ships `false`
  because Finder folder-item positions are **not guest-accurate yet** (Lane H2
  owns making them true), and no fixture covers them. Freezing a field whose
  values are known wrong is the expensive half of a contract: it obliges us to
  keep serving the wrong number. It re-enters additively, via
  `IRSchema.v1Additions`, once the positions are real — no major bump needed.
