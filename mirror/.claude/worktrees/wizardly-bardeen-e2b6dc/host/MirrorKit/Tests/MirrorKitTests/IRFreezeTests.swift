...
final class IRFreezeTests: XCTestCase {
    ...
    /// The two host-internal shelves stay off the wire. Stated as its own
    /// assertion because it is a *decision*, not a byproduct: re-adding either
    /// to `CodingKeys` should read as breaking this promise, not as editing a
    /// list.
    func testIslandAndWindowItemsAreNotOnTheWire() throws {
        let data = try JSONEncoder().encode(Self.maximalScene())
        ...
        XCTAssertFalse(paths.contains("windows[].items"),
                       "window items are not guest-accurate; not frozen in v1")
        // …and they are still declared, so the renderer keeps its shelves.
        let props = IRSchema.declaredProperties(of: Self.maximalScene())
        XCTAssertTrue(props.contains("Scene.Window.island"))
        XCTAssertTrue(props.contains("Scene.Window.items"))
    }
    ...
    func testConsumerAcceptsTheCurrentMajor() throws {
        ...
        // The excluded shelves do not survive the wire, by design.
        XCTAssertNil(decoded.windows.first?.island)
        XCTAssertNil(decoded.windows.first?.items)
    }
