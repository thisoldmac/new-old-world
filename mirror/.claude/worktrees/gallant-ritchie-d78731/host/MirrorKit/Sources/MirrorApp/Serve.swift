...
final class MirrorService {
    ...
    private func sceneMethod(_ p: [String: Any]) -> [String: Any] {
        ...
        //
        // The scene body still carries its own `version` (Scene.version, the
        // same number) as the IR's self-stamp — that field is part of the
        // fixture corpus and is left untouched, so this adds an envelope key
        // without churning the pre-freeze IR.
    ...
    private func actOpen(_ p: [String: Any],
        ...
        guard let name = p["desktopItem"] as? String else {
            return err("bad_request", "act.open needs desktopItem (v0)")
        }
        guard let s = pollScene(),
