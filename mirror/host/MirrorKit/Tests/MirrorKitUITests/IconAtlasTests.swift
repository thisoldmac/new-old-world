...
/// The pack is built offline by `tools/extract-assets-offline` and lives in
/// the bundle. These are the claims the renderer makes about it — that both
/// sizes exist as their own art, and that a reported creator signature
/// reaches a real per-application icon rather than a generic one.
///
/// A missing file makes `IconAtlas` return nil silently, which draws a
/// procedural fallback and reads on screen as "the mirror is a bit rough"
/// rather than "the pack did not ship". These fail instead.
final class IconAtlasTests: XCTestCase {

    func testEveryGenericIconShipsInBothSizes() {
    ...
    func testSmallArtIsItsOwnDrawing() {
    ...
    func testAReportedSignatureReachesTheApplicationsOwnIcon() {
    ...
    func testAnItemWithoutACreatorStaysGenericByKind() {
