...
struct Options {
    ...
    /// "NAME" — select a Finder folder-window icon by name, at the point the
    /// FINDER says it is. The measurement path for folder items: the point is
    /// computed from the item's reported position and pushed through the real
    /// hit-test, so a wrong position surfaces as a wrong target rather than as
    /// a click that quietly lands somewhere else.
    var actWindowItem: String?
...
func parseArgs() -> Options? {
    ...
    while let arg = it.next() {
        ...
        case "--act-window-item": o.actWindowItem = value()
...
if options.actScroll != nil || options.actMenu != nil || options.actControl != nil
    ...
    do {
        ...
        } else if let name = options.actWindowItem {
            guard let win = before.windows.first(where: {
                FinderItems.isFolderWindow($0) && ($0.items ?? [])
                    .contains { $0.name == name }
            }), let item = (win.items ?? []).first(where: { $0.name == name })
            else { fail("no open Finder window holds an item named \(name)") }
            guard let point = FinderItems.clickPoint(item, in: win) else {
                fail("\(name) is at (\(item.x),\(item.y)) in \(win.title), "
                     + "which the Finder has scrolled out of view")
            }
            // Through the hit-tester, always: a position that is off by more
            // than the icon box resolves to a neighbour or to bare content,
            // and that must read as a failure, not as a click.
            let hit = HitTester.hitTest(before, x: point.x, y: point.y)
            guard case .windowItem(_, let hitName, _, _) = hit, hitName == name
            else { fail("hit-test at (\(point.x),\(point.y)) resolved to \(hit), "
                        + "not the item \(name)") }
            print("windowItem: \(name) in \(win.title) at pos=(\(item.x),"
                  + "\(item.y)) -> click (\(point.x),\(point.y))")
            actions = ActionModel.click(on: hit)
