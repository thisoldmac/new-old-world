...
#
# `overwrite` is not optional: the current base image already carries a
# mirror.port, so without it EVERY fresh clone dies on this line with
# `exists: file exists` — after all four pushes have already succeeded, which
# reads like a deploy failure and is not one. (Found 2026-07-31, lane P2.)
line = f"{GUEST_PORT}\n"
h.request("write", {"path": f"{DEV}:mirror.port", "data": line,
                    "offset": 0, "truncate": True, "overwrite": True})
