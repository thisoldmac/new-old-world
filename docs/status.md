...
- **A log on both machines** — one file per launch, in `now-logs` beside
  ...
  invert the canvas and to turn disk persistence on or off. Classic guests
  retain 10 launch logs by default; PPC exposes a 1–100 control here and
  NOW-68K reads the same bound from its dev settings file. The guest page
