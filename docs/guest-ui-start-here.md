...
**The log opens before `conn_init`.** Keep that order: a hang during
connection setup is precisely the case that needs a durable line.
`now-logs` sits beside the application, one file per launch; the guest keeps
10 recognized files by default (configurable 1–100), and `tail` works from
either console.
