...
| Can the QuickDraw stream tell us where Finder window icons are? | **YES, from the OFFSCREEN port** — 2026-08-06 a resident found and hooked the Finder's GWorld from outside and read its icon LABELS as text ops at true pens ('Documents' [280,67], 'TimBotTu' [282,131], …). Still **no** from the WINDOW port — the Finder composites offscreen and emits one blit; re-confirmed independently 2026-08-06. which emits one opaque blit. Three Toolbox facts make it work and each defeats it alone: join on SHAPE (LockPixels relocates the PixMap record, so pointers, handles and baseAddr are all snapshots); MemTop is NOT the address-space ceiling (~14.8 MB while app heaps sit ~512 MB up), so a read guard bounded by it rejects every candidate; and the hook must be HELD across the repaint, because re-arming unhooks the world. Icons themselves still arrive as identity-less bits — labels are semantic, images are not (yet) | [content plane](mirror-content-plane.md), [the probe's brief](gworld-probe-brief.md) |
