...
| **Finder folder views and 8.6 chrome** | **Broadened in NOW 2026-08-11:** `view of window` reports `icon`, `button`, `name` and `small icon`; Buttons owns a 48 by 48 raised well rather than collapsing into small icons. Item `bounds` carries the live box, while the renderer derives the count-and-available strip, ruled List field, disclosure triangles and labels locally. Optional `file.listing.freeBytes` reports responder-observed volume capacity without sending pixels or coupling Finder to the census plane. The 8.6 oracle now has replayable Icons, Buttons and List cases plus File, Edit, View, Special and Help menus. |
