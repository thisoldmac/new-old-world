...
| Can the QuickDraw stream tell us where Finder window icons are? | **No** from the WINDOW port — the Finder composites offscreen and emits one blit; re-confirmed independently 2026-08-06. Whether the OFFSCREEN port can be hooked instead is **open, not closed**: the probe that asks exists (`qdtrace mode=probe`) and its own positive control currently fails, so it has no verdict for any application | [content plane](mirror-content-plane.md), [the probe's brief](gworld-probe-brief.md) |
