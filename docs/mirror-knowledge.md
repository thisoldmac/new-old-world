...
| Can the QuickDraw stream tell us where Finder window icons are? | **No** from the WINDOW port — the Finder composites offscreen and emits one blit; re-confirmed independently 2026-08-06. Hooking the OFFSCREEN port is **mechanically possible** — a spike (`tools/guest-gworld`) proves `grafProcs` fires on a GWorld, `PlotIconSuite` included, and that the port sits in the application zone shaped as expected. What is unproven is DISCOVERY of a foreign process's GWorld, so there is still no per-application verdict | [content plane](mirror-content-plane.md), [the probe's brief](gworld-probe-brief.md) |
