...
| Can Finder folder windows be useful without waiting for Finder state? | **Tested host-side, not metal-verified.** With **Emulate Finder Windows** enabled, NOW owns the interior, view, sort, selection and scroll and obtains entries through bounded `file.list` pages. Optional lifecycle and geometry switches apply host changes immediately, send the corresponding guest act, then reconcile from a later scene. Turning them off leaves those axes host-only. The ordinary guest-follow semantic mode remains available when the toggle is off | [open issues](open-issues.md) |
