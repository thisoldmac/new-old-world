...
The standalone `timbottu/mirror` project is retained in the private lineage
history: a semantic mirror of a Mac OS 9 desktop, with a guest application,
resident extensions and a Swift host. NOW owns the production pieces it
retained and inherits the archived project's recorded knowledge. **Everything
in it was paid for once already.** The maintained public provenance map is the
...
| **Finder folder views and 8.6 chrome** | **Broadened in NOW 2026-08-12:** `view of window` reports `icon`, `button`, `name` and `small icon`; Buttons owns a 48 by 48 raised well rather than collapsing into small icons, and one primary click activates rather than retaining selection. Icons and List keep Finder's select-then-double-click model. Measured 8.6 icon selection multiplies each icon channel by 128/255 and uses a tight black name patch with white Geneva text; List selects only the name cell, not the full row. Item `bounds` carries the live box, while the renderer derives the count-and-available strip, ruled List field, disclosure triangles and labels locally. Optional `file.listing.freeBytes` reports responder-observed volume capacity without sending pixels or coupling Finder to the census plane. The 8.6 oracle now has replayable Icons, selected Icons, Buttons and List cases plus File, Edit, View, Special and Help menus. |
...
| Can Finder folder windows be useful without waiting for Finder state? | **Experimental; tested host-side, not metal-verified.** With **Emulate Finder Window Interiors (Experimental)** enabled, NOW retains each guest-observed Finder shell and owns only its interior, view, sort, selection and scroll through bounded `file.list` pages. Several Finder behaviors remain incomplete. Enabling it opens nothing. Position/size synchronization also follows guest opens/closes and joins by exact window identity plus HFS path, so it cannot create a parallel window set. View changes are host-only by default; optional **Sync Finder view type** sends the measured OS 9 word and reconciles later without making guest settlement the render gate. Documents and `cdev` control panels are Finder opens, not `launch` requests. **Emulate Desktop** is independent: off uses Finder's live roster and positions, on lays out the Desktop Folder locally. The desktop roster explicitly asks Finder for disks and Trash because `every item of desktop` is incomplete on the tested systems | [open issues](open-issues.md) |
| What does Rebuild State discard? | The active observation generation, displayed scene, reducer snapshot/history, retained content, Finder rosters/art/catalogs, desktop catalog and visibility projection. It then asks the same pinned guest again. The operation journal remains because rebuilding observation is not permission to forget acts | [open issues](open-issues.md) |
| Does Mirror resume because it was running when the host last quit? | No. The launch default is always stopped and the retired persisted intent is deleted. An explicit `--open-mirror` still starts it, and a guest reconnect may resume a start requested during the current host process | [open issues](open-issues.md) |
