...
| Can Finder folder windows be useful without waiting for Finder state? | **Tested host-side, not metal-verified.** With **Emulate Finder Windows** enabled, NOW owns the interior, view, sort, selection and scroll and obtains entries through bounded `file.list` pages. Enabling the mode opens nothing by itself; desktop and guest-opened folders create host windows. Optional lifecycle and geometry switches apply host changes immediately, defer exact acts until a guest window reference exists, then reconcile from a later scene. Turning them off leaves those axes host-only. The ordinary guest-follow semantic mode remains available when the toggle is off | [open issues](open-issues.md) |
