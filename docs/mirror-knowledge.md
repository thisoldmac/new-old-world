...
| Does bounded semantic Finder mean share-root-only browsing? | **No.** It means one displayed guest directory at a time. The host may follow Finder windows anywhere on the guest disk; it does not recursively enumerate the volume or inherit the Files module's shared-tree boundary | [open issues](open-issues.md) |
