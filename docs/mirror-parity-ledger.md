...
| Bitmap font (Tier-2 Platinum glyphs) | `9552d9d` "real Platinum assets — Tier-2 bitmap fonts + desktop pattern" | **crossed (2026-08-06)** | `now-host/Sources/MirrorKitUI/BitmapFont.swift` still has 0 callers outside `MirrorKitUI/DisplayReplay.swift` — but the second half of the old reasoning has gone: `DisplayReplay` IS now fed, because the content plane's producer landed (see the content-plane row below). This row was unreachable only by inheritance | Reached through the replay, not directly. Emulator only |
| Desktop pattern draw | `f46861e` | crossed | `PlatinumTheme` grays consumed by `SceneRenderer`; reachable via same `SceneView` chain | |
| Content-plane (QuickDraw pixel) draw of window interiors | `70a70f0` "carry the pixel plane — window interiors now render", `c3d2aff` "carry qdtrace + QDPeek" | **crossed (2026-08-06)** | ~~wired-but-unreachable: nothing on the Host side ever issues the guest verb, and `grep -rn "qdtrace" now-host/Sources` finds only a contract-message comment~~ — **that is no longer true, and the row's own reproduction command is what falsifies it.** The same grep now returns `now-host/Sources/Host/NOWMirrorContentPlane.swift:141, 210, 238, 290` — `listener.runCommand("qdtrace", …)` for stop / status / start / drain. The producer exists, arms in `record` mode, drains twelve pages a cycle and re-homes offscreen ops into the window | Was the standing "display plane" example of a wired-but-unreachable capability; it is now the example of one closing. Emulator only — see [status.md](status.md) |
