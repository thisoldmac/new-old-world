...
> a private lineage snapshot that is deliberately absent from the clean public
> history. No active build, staging path, or runtime asset lookup depends on
> it. The retained public provenance map is the
...
keys were derived from AXPeek's sample record, QDPeek's operation constants,
Portal's operation and window-operation constants, the agent dispatch, and the
old staging/service paths. The public, source-digested census is
`tools/mirror-gate-tests/legacy-parity-fixture.json`; the private source
snapshot itself is not a test dependency. Adding a legacy capability without a
row fails the ordinary native gate. The source column says where the key came
from; it is not evidence that NOW already provides the outcome.
