...
Two build systems compile this code, and they disagree about what `NOWAgentIntegration` is. SwiftPM has a real target, so `Sources/Host` must import it. The Xcode project has no package reference: it pulls `Sources/NOWAgentIntegration` into the app target as a second synchronized file group, so the types are already in scope and no such module exists. **Every `import NOWAgentIntegration` under `Sources/Host` therefore has to be wrapped in `#if canImport(NOWAgentIntegration)`.** An unguarded one breaks `xcodebuild` while `swift build` and `swift test` stay green — which is exactly how it reached main and sat there unnoticed. Nothing enforces this yet; the honest fix is to give the Xcode target a real local package dependency and delete all eight guards.
