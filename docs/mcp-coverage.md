...
| `chat.projectroster` | message | ppc | deliberate | The host's answer to `chat.projects` — the sessions half of the same guest-to-host family, added 2026-08-18. Same definitional direction as `chat.catalog` and the same citation ([command-parity.md](command-parity.md)): the subject is the host's own saved chats and project filing, which no classic Mac serves, so there is nothing here for a projection to ask a guest for. The agent-facing reading of projects is `now_projects`, which reaches the Projects store directly rather than through somebody's chat sidebar. |
| `chat.roster` | message | ppc | deliberate | The host's answer to `chat.chats` — one page of saved chats, added 2026-08-18. Same direction and reason ([command-parity.md](command-parity.md)). Deliberately NOT projected for a second reason worth stating on its own: a person's saved conversations are not an agent capability. An agent that could list and read them would be reading the person's chat history with their models, which is a different kind of access from driving their machine and is not something this surface should acquire by accident. |
| `chat.transcript` | message | ppc | deliberate | One page of a saved chat's contents, added 2026-08-18. Same direction and the same citation ([command-parity.md](command-parity.md)), and the sharper end of the reason above: this is the transcript text itself. |
