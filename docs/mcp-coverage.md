...
| `key` | command | ppc | planned | **W3** of the parity slice — the driving verbs, of which this is the last without a row. One keystroke, posted through the Event Manager — the ground `now_text_set` cannot cover: a dialog that answers only keystrokes, and keys that carry no text (Return, Escape, the arrows). **Planned rather than deliberate**, and the distinction is the point: nothing argues against a row here, the row simply is not written. `ActionModel` already types `key` as `unavailable` with the reason recorded as *a hole in NOW's surface, not a rule against it*, so the day a keystroke row lands that answer fails and points at itself. What a row must decide first is the honest limit: an event's modifiers live on the Event Manager's queue element, and the only call that returns it is `PPostEvent`, which is `CALL_NOT_IN_CARBON` — so this verb refuses `mods != 0` outright rather than posting a bare key and reporting success. A row that exposed it without exposing that refusal would promise a reach the guest does not have ([input-plane-decisions.md](input-plane-decisions.md)). |
