...
| `development-open` | command | ppc | deliberate | The optional transition from headless work to a human editor. It locates and launches CodeKitten, opens only the active `Project.ckp`, and brings the IDE forward, but it is intentionally an explicit app action rather than agent authority: project sync, build, test, promote and run do not depend on an IDE. This is the initial projection boundary in the [Projects and Development plan](plans/2026-08-09-029-feat-projects-and-development-plan.md) (projection-family table and U9). |
