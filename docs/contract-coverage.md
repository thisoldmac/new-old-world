...
these rows open into 38 command verbs and 14 hardware probes.
...
`reveal`, `vers`, `putstat`, `key`, `net`, `hide`, the ten of the act
plane and the reference layer, and the six registered on 2026-07-31 —
`activate`, `actselftest`, `mouseloc`, `script`, `aesend`, `qdtrace`.
The last six are not a 68K debt: four of them reach for OSA, Apple
Events or a content-plane ring that guest does not carry, and no one has
asked for them there.
