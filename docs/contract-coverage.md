...
| `chat.catalog` / `chat.delta` / `chat.status` / `chat.result` | ❌ | ❌ | the ASKER's half of the chat family (contract `guestAsksChat`): a guest that speaks it SENDS `chat.models` / `chat.send` / `chat.cancel` / `chat.reset` and consumes these as answers. Declared in the contract; NEITHER guest asks it yet and the host does not serve it yet — this row ticks when the implementations land. Like cloud, its subject is the host's own model harness, so it can never grow guest-serving ticks; 68K is expected to stay a deliberate never-asks |
