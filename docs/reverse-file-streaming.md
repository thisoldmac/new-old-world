...
- Host full suite: 291 tests passed, 12 metal tests skipped, 0 failures.
...
This is **Tested**, not **metal-verified**. A safe metal pass still needs
a separately named guest with its own configured listener port so it
cannot interrupt the normal paired NOW session. The minimum ladder is
256 KiB, 2 MiB, 16 MiB, and one file whose transfer exceeds two minutes;
run both data and MacBinary, cancel one midstream, mutate one source
after acceptance, and compare byte count plus CRC at the host.

## Integration recommendation

Hand this commit to the V0.5 branch as one isolated transport change.
Re-run its host suite and Retro68 build after rebasing, then perform the
dedicated metal ladder before changing product claims. Keep reverse
resume as a second contract-first change after selecting the source
