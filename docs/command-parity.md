...
| `showmirror` | console only (PPC) | asks the HOST to show its own Mirror window, through the `host.*` family. The host reaches its own Mirror from its Window menu and the `mirror_open` agent verb, so there is nothing for it to type at the guest. Its second face is not a wire verb but the **Mirror page's button** — both sit over `mirror_show.h` and `now_wire_host_show`, which is the one-implementation half of the rule doing its work in a direction this file had not had to describe before |
