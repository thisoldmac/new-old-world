...
| 2 | **Collision: slice 0 OVER-CLAIMED and the claim is withdrawn** (2026-08-17, Michelle's attended screenshot: the Finder's own "already exists — replace?" ask is alive on this machine when its copy/move engine owns the operation). What slice 0 actually measured was one undiagnosed silent failure — `transfer-failed` is OUR verdict string; the refusing layer was never identified. Diagnose it: (a) if our send proc dies on `dupFNErr` before the Finder can ask, stop failing and re-observe — the native ask may simply appear; (b) only if the promise-receive path measurably bypasses the Finder's engine does a send-proc policy question exist, and then the answer is designed against the measured behavior, not invented. Copy-progress observation still open (every completed pull was 4 KB; re-observe with a large file once the 600 KB defect falls). | diagnosis first; no policy decision on an unexplained failure |
