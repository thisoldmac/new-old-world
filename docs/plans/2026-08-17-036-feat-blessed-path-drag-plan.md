...
| 1 | **DONE 2026-08-17** (`feat/gh-native-drag-guest`, receipts `/private/tmp/now-s1-guest-receipts/`, ledger entry the same day): both open questions answered by measurement — **background TrackDrag WORKS** (`front=other` on every gesture, no fallback needed or built) and **the sprite FOLLOWS for free** (before 300,200 / held 120,380 in the screendumps; no new resident code). Desktop drop, **Finder-WINDOW drop** (`dest dir 2`, the never-yet-watched gate), the Finder's own collision ask, and a native abort (`userCanceledErr`, `asks=0`) all measured. **600 KB and 1 MB complete byte-identical** — the pull was starved by TWO defects, neither a missing pump: the pull lane never sent `file.progress` so the host's send window never engaged, and the Finder-selection poll Apple-Evented a Finder blocked inside our own send proc. Original text: `continuity.hostDragBegin` carrying only the *starting* state (ordinary Continuity datagrams carry the rest — transport settled). **NOW never targets anything (Michelle, 2026-08-17): where the file lands is the Manager's and the receiver's business entirely.** NOW's product duties during the guest-owned segment are exactly three — feed the input proc, notice the edge cross-back from its own fed position (native abort, no window knowledge), serve the promise when asked. Any code asking "is this a Finder window" is re-invented targeting and a blessed-path violation. Gates inherited from earlier slices: one RIG-ONLY observation that the Manager delivers into a window container as it does the desktop (never yet watched — slice 0's attempt had NOW's window occluding it, and the agent logged the occlusion instead of just hiding the window with the guest's own verbs; the rig fix is exactly that — `winact` the self-window away. Product question, OPEN — front-ness is NOT an established requirement: the experiment's "front-ness was the variable" fact was about the RIG's induction press travelling through the Event Manager, which routes to the front process. hostDragBegin needs no delivered event at all — NOW synthesizes the starting EventRecord (a struct parameter) and the input proc supplies position AND button thereafter. Slice 1's rig tests background TrackDrag FIRST; the desired product answer is that NOW stays background and invisible, the user's front app stays front, and the drag materializes under the cursor. Only if the OS refuses a background TrackDrag does the fallback apply: NOW front-but-hidden for the drag's duration), and the **600 KB pull** (diagnosed slice 2: the wire starves inside the drop's nested loop, pull times out ~46 s — a transport-liveness fix, not File Manager). Byte-compare method proven in slice 2. Sprite decision made deliberately (resident low-mem chase vs ghost-only). | emulator |
| 2 | **DONE 2026-08-17 — Michelle's invariant CONFIRMED by measurement** (`test/hg-drag-input-proc` @ 88a5dd68, receipts `/private/tmp/now-slice2-receipts/`): the silent collision was OUR protocol violation — the send proc materialised the promise in the receiver's folder under its final name, and our own pull pre-check refused the second drop (`kFilesExists`, fileshare.c:1704); no File Manager error, no Finder involvement. Fix: stage into Temporary Items on the drop volume, hand the receiver that FSSpec, zero collision logic. Result: **the Finder raises its own native replace ask** ("An older item named … already exists…"), Cancel keeps / OK replaces (exactly one file after), announced by the OS's own "the Finder needs your attention" notification — none of it ours. Byte identity established (4096/4096; slice 0's -1753 was a script phrasing defect). 600 KB still fails as a DIFFERENT class: the wire starves inside the drop's nested loop and the pull times out (~46 s) — slice 1's remaining gate. | passed |
| 3 | **DONE 2026-08-17, attended** — the edge handoff drives `hostDragBegin`; the carry illustration, drag-lane windoid engagement and `now_confirm` are structurally out (and the dead carry-presentation lane was deleted in the PR-prep prune, 2026-08-18); drops land at the release, the Finder's own replace ask handles collision, cross-back aborts natively. | passed (metal) |
| 4 | Host reconciliation OPEN — `ChangeConfirmation` → `.confirmationDialog` not yet done (small; the host sweep's one imitation finding); host native claims otherwise held attended. | follow-up |
| 5 | **Metal rounds happened 2026-08-17** — the drag plane ran attended on the 1400c through four rounds; the input-proc cursor nudge (task-time Cursor Device apply from inside `TrackDrag`) has attended coverage via the working session, with the module header now stating the boundary. Remaining metal watch-items live in issues #38–#43. | passed, watch-items filed |
