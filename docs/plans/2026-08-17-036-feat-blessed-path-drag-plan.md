...
| 2 | **Collision: slice 0 OVER-CLAIMED and the claim is withdrawn** (2026-08-17, Michelle's attended screenshot: the Finder's own "already exists — replace?" ask is alive on this machine when its copy/move engine owns the operation). What slice 0 actually measured was one undiagnosed silent failure — `transfer-failed` is OUR verdict string; the refusing layer was never identified. **Invariant (Michelle, 2026-08-17): the promise protocol makes the RECEIVER own the destination — the Finder decides the FSSpec, hits dupFNErr in its own house, and resolves it. NOW ships ZERO collision handling on the drag lane, and any observed collision failure is by definition a NOW protocol bug** (we created something we shouldn't, errored an ask we should have served, or handed a skeleton that dies at destination-create). Slice 2 diagnoses which violation it is and removes it; there is no policy question. Copy-progress observation still open (every completed pull was 4 KB; re-observe with a large file once the 600 KB defect falls). | diagnosis first; no policy decision on an unexplained failure |
