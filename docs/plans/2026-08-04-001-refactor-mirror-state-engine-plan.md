...
- R26. QMP is a read-only development-oracle adapter for explicit-socket framebuffer capture, coherent virtual-memory inspection, and identity checks. Memory inspection may explain which Mac OS structures or events a metal-compatible guest producer must expose; QEMU bytes and addresses never enter the state engine. No production module imports it, no mutation uses it, and the architecture must run unchanged on metal.
...
- KTD6. `session-settled: user-directed` — QEMU remains a host-side oracle, never an implementation dependency. It may inspect emulator memory to reverse-engineer the corresponding Mac OS state/event, but shipped NOW code consumes only the Mac OS mechanism that exists on metal. Rejected: QMP input, QEMU bytes in the state engine, or a QEMU-specific guest/Host path.
