...
| D-036 | The first CLI credential draft would have taught the client the MCP token filename or silently rotated the shared secret under a neutral name | API v1 and MCP deliberately share one private host-issued secret, and existing MCP clients may already hold it | Rename the private file to `now-api-key`, migrate the legacy item byte-for-byte by atomic rename (or atomic-copy fallback), retain mode 0600, and expose neither value nor path through API metadata or logs | implemented in S6; focused-tested |
...
| D-040 | The public API design promised UI bootstrap without private-state discovery, but the current host labels the only copy control for MCP bearer auth | `MCPModuleView` shows **Copy Bearer Token** only in bearer mode; the CLI can read the shared private mode-0600 credential | Document the current bridge honestly and keep a dedicated application-facing key bootstrap control open; third-party clients must not parse the private file | open |
