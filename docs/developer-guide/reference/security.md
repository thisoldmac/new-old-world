...
The canonical vulnerability-reporting contact is intentionally not invented on this documentation branch. Publication with `NOW_DOCS_RELEASE=1` refuses until `security_contact`, its expiry, and the canonical website origin are set in `docs/site-integration.yaml`; the release build then publishes the matching RFC 9116 `/.well-known/security.txt`.
