...
/// **What the renderer actually puts in a window's content area**, asserted by
/// rendering offscreen (RenderShot — the app's own drawn canvas, never the
/// guest framebuffer and never the host screen) and reading the pixels back.
///
/// Renamed from `IslandRenderTests` on 2026-08-07. It was born to prove a held
/// pixel island reached a background window's interior; that feature is gone
/// (`archive/pixel-islands-2026-08-07/`) and its two island assertions went
/// with it, but nineteen guards about placeholders, erase, truncation and
/// semantic precedence had accumulated here and none of them ever needed a
/// photograph. The old name is quoted in `BitmapFont.swift` and
/// `SceneView.swift`.
@MainActor
final class WindowContentRenderTests: XCTestCase {
