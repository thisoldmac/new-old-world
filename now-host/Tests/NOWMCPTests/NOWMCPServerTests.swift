...
final class NOWMCPServerTests: XCTestCase {
    ...
    func testConcurrentSessionHealthCallsAllReturn() async throws {
        ...
        let encodedResponses = try await withThrowingTaskGroup(
            of: Data.self
        ) { group in
            for id in 2..<10 {
                group.addTask {
                    guard let response = await server.handle(
                        try Self.request(
                            id: id,
                            method: "tools/call",
                            params: [
        ...
        let responses = try encodedResponses.map { try Self.object($0) }
    ...
    func testAConsentDenialIsItsOwnErrorWithTypedData() async throws {
        ...
        client.healthResult = .available(.init(
            ...
            failure: nil))
        let server = try await Self.initializedServer(client: client)
