...
final class FakeGuest {
    ...
    init(port: UInt16, host: NWEndpoint.Host = .ipv4(.loopback)) {
        self.port = port
        self.host = host
        connection = Self.dial(port, host: host)
    }

    private static func dial(_ port: UInt16,
                             host: NWEndpoint.Host) -> NWConnection {
...
final class GuestListenerTests: XCTestCase {
    ...
    /// The approved alpha boundary is a trusted LAN, not loopback. This
    /// dials the listener through this Mac's routable interface so a future
    /// hardening change cannot quietly turn the two-machine product into a
    /// same-machine service while the loopback-heavy suite stays green.
    func testListenerAcceptsTheDocumentedTrustedLANScope() async throws {
        guard let text = HostAddressDetector.primaryIPv4(),
              let address = IPv4Address(text) else {
            throw XCTSkip("runner has no routable IPv4 interface")
        }
        let guest = FakeGuest(
            port: listener.boundPort!, host: .ipv4(address))
        defer { guest.connection.cancel() }
        guest.start()
        try guest.send(guestHello(name: "Trusted LAN guest"))

        try await waitUntil("non-loopback guest admission") {
            self.listener.guests.contains { $0.name == "Trusted LAN guest" }
        }
        XCTAssertEqual(listener.guests.first?.name, "Trusted LAN guest")
    }
