...
/// for the composition described in now-guest-ppc/src/processes/proc_actions.h (PowerPC) and
/// now-guest-68k/src/processes/proc68.h (NOW-68K). Opt-in:
///
...
/// now-guest-68k/src/core/wire68.c :: handle_control_message) for everything else,
...
final class MetalQuitTests: XCTestCase {
    ...
    /// now-guest-68k/src/core/hello.h as NOW68K_HELLO_NAME) with `os` "7.1", while
    /// the PowerPC guest sends the MACHINE's name — `now_machine_name()`
    /// in now-guest-ppc/src/core/wire.c, e.g. "PowerBook 1400c" — with `os` "9".
    ...
    /// self-refusal check. The emulator build is "now-guest-ppc"; the
    /// canonical PowerPC one is "New Old World"; the 68K target builds as
    /// "now-guest-68k" (now-guest-68k/CMakeLists.txt).
    private func guestSelfName(_ kind: GuestKind) -> String {
        ...
        return kind == .now68k ? "now-guest-68k" : "now-guest-ppc"
    }
    ...
    private func identifyGuest() throws -> GuestKind {
        ...
        case nil:
            // now-guest-68k/src/core/hello.h :: NOW68K_HELLO_NAME.
