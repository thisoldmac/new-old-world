...
final class MirrorGesturePathTests: XCTestCase {
    ...
    ///
    /// The window is at (100,100)–(400,300) and the control's rect is
    /// content-local, so the button occupies global x 120–200, y 140–160
    /// (the content origin is the window's left, and its top pushed down by
    /// the 20 px title bar).
    private static func document(controlRef: String) -> Data {
        ...
        return Data("""
            ...
            "rect":{"l":20,"t":20,"r":100,"b":40},"enabled":true,\
            "visible":true}]}],\
