...
final class AgentIntegrationDownloadTests: XCTestCase {
    ...
    func testWithoutPrivateStorageDownloadIsDeferredNotAttempted()
        ...
        XCTAssertTrue(script.gotPaths.isEmpty)
        /* The four mutations joined `available` on the integration branch,
           and they are host-side unconditionally — so they appear here even
           with the download store switched off. Restated exactly rather than
           filtered, because what this test is for is that ONE name moved:
           the two assertions below say so directly, and the list says the
           absence is `download`'s alone. */
        XCTAssertEqual(capabilities?.availableCommands,
                       [.capabilities, .list, .stat, .put,
                        .mkdir, .move, .trash, .restore])
        XCTAssertFalse(
            capabilities?.availableCommands.contains(.download) ?? true)
        XCTAssertTrue(
