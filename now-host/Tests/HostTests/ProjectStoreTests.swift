...
final class ProjectStoreTests: XCTestCase {
    ...
    private func assertReceiptRejectsUnknownSchema<T: Codable>(
        _ value: T, schema: String, file: StaticString = #filePath,
        line: UInt = #line
    ) throws {
        ...
        var object = try XCTUnwrap(
            ...
            file: file, line: line)
        XCTAssertEqual(object["schema"] as? String, schema,
                       file: file, line: line)
        object["schema"] = schema + "-unknown"
        let changed = try JSONSerialization.data(withJSONObject: object)
        XCTAssertThrowsError(
            try JSONDecoder().decode(T.self, from: changed),
            "an unknown receipt schema must not decode as the current one",
            file: file, line: line)
    }

    func testReceiptsRejectUnknownSchemas() throws {
        let projectID = ProjectID.mint()
        let candidateID = ProjectCandidateID.mint()
        try assertReceiptRejectsUnknownSchema(
            ProjectCandidateReceipt(
                candidateID: candidateID, projectID: projectID, home: .host,
                sourceRevision: 1, sourceCommit: "commit",
                workspaceID: nil, baseGuestDigest: nil,
                contentDigest: "digest", manifest: [], stagedAt: Date()),
            schema: "ckproject.candidate-receipt/1")
        try assertReceiptRejectsUnknownSchema(
            ProjectPromotionReceipt(
                candidateID: candidateID, projectID: projectID, home: .host,
                baseGuestDigest: nil, currentGuestDigest: nil,
                promotedRevision: 1, promotedCommit: "commit",
                contentDigest: "digest", promotedAt: Date()),
            schema: "ckproject.promotion-receipt/1")
        try assertReceiptRejectsUnknownSchema(
            ProjectRevisionReceipt(
                projectID: projectID, home: .host, revision: 1,
                commit: "commit", contentDigest: "digest", changedPaths: [],
                committedAt: Date()),
            schema: "ckproject.revision-receipt/1")
    }
