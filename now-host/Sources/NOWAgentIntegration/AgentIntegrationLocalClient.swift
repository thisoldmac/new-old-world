...
public struct AgentIntegrationLocalClient: Sendable {
    ...
    /* Typed sends for the eleven projected verbs. The result vocabulary still
       carries `.notImplemented` for compatibility with an older host whose
       local adapter predates a verb; a current host's exhaustive dispatch has
       an adapter for every case. Keeping that answer typed means version skew
       is reported as an unavailable operation rather than as a malformed
       response missing its result field. */
