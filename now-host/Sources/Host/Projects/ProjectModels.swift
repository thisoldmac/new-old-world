...
enum ProjectCandidateReceiptSchema: String, Codable, Sendable {
    case v1 = "ckproject.candidate-receipt/1"
}

enum ProjectPromotionReceiptSchema: String, Codable, Sendable {
    case v1 = "ckproject.promotion-receipt/1"
}

enum ProjectRevisionReceiptSchema: String, Codable, Sendable {
    case v1 = "ckproject.revision-receipt/1"
}

struct ProjectCandidateReceipt: Codable, Equatable, Sendable {
    let schema: ProjectCandidateReceiptSchema
    ...
    init(candidateID: ProjectCandidateID, projectID: ProjectID,
         home: ProjectHome, sourceRevision: Int, sourceCommit: String,
         workspaceID: ProjectWorkspaceID?, baseGuestDigest: String?,
         contentDigest: String, manifest: [ProjectManifestEntry],
         stagedAt: Date) {
        schema = .v1
...
struct ProjectPromotionReceipt: Codable, Equatable, Sendable {
    let schema: ProjectPromotionReceiptSchema
    let candidateID: ProjectCandidateID
    let projectID: ProjectID
    let home: ProjectHome
    let baseGuestDigest: String?
    ...
    init(candidateID: ProjectCandidateID, projectID: ProjectID,
         home: ProjectHome, baseGuestDigest: String?,
         currentGuestDigest: String?, promotedRevision: Int,
         promotedCommit: String, contentDigest: String, promotedAt: Date) {
        schema = .v1
        self.candidateID = candidateID
        self.projectID = projectID
        self.home = home
        self.baseGuestDigest = baseGuestDigest
        self.currentGuestDigest = currentGuestDigest
        self.promotedRevision = promotedRevision
        self.promotedCommit = promotedCommit
        self.contentDigest = contentDigest
...
struct ProjectRevisionReceipt: Codable, Equatable, Sendable {
    let schema: ProjectRevisionReceiptSchema
    let projectID: ProjectID
    let home: ProjectHome
    let revision: Int
    let commit: String
    ...
    init(projectID: ProjectID, home: ProjectHome, revision: Int,
         commit: String, contentDigest: String, changedPaths: [String],
         committedAt: Date) {
        schema = .v1
        self.projectID = projectID
        self.home = home
