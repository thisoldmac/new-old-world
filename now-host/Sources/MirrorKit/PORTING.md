...
| `ScenePoller` | the poll loop over `axtree`/`list`/`video`/`qdtrace` | NOW fetches a scene because **a person asked** (`MirrorModuleModel.fetchScene`); a timer would take the shared bulk lane at intervals nobody chose. Its measurements live on in `SceneGeometry` and `SceneIslands`. |
