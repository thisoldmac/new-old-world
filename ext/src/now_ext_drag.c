...
int now_ext_drag_boot(NowPeekTable *table);
void now_ext_drag_rollback(NowPeekTable *table);
...
{
    if (gDragInstalled) {
        /* Construction never primes the task, so no callback can race this
           pre-publication removal. */
        RmvTime((QElemPtr)&gDragTask.task);
        gDragInstalled = false;
    }
    gDragTask.table = NULL;
    if (table != NULL) {
        table->drag_format = 0;
        table->drag.state = 0;
        table->drag.button_down = 0;
