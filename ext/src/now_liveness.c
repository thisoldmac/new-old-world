 ...
 * the tick DECLINING TO RE-PRIME. After publication nothing calls RmvTime,
 * and that is deliberate rather than timid: this task re-primes itself at the end of
 ...
 * — the tick — ever touches the Time Manager queue after publication, so the
...
int now_liveness_install(NowPeekTable *table)
{
    if (gTaskInstalled || table == NULL) return 0;
...
void now_liveness_rollback(NowPeekTable *table)
{
    if (gTaskInstalled) {
        /* This path exists only before publication, while the task is queued
           but has never been primed. The post-publication stand-down rule
           remains unchanged and never races RmvTime against a tick. */
        RmvTime((QElemPtr)&gLivenessTask.task);
        gTaskInstalled = false;
    }
    gLivenessTask.table = NULL;
    gLivenessTask.visible_epoch = 0;
    gTaskPriming = false;
    if (table != NULL) {
        table->liveness_ticks = 0;
        table->caps &= ~((NowPeekU32)kNowPeekTableCapLiveness
                         | (NowPeekU32)kNowPeekTableCapLivenessNet);
    }
}
