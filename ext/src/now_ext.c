...
extern int now_ext_drag_boot(NowPeekTable *table);
extern void now_ext_drag_rollback(NowPeekTable *table);
...
extern int now_content_boot(NowPeekTable *table);
extern void now_content_rollback(NowPeekTable *table);
...
extern int now_liveness_install(NowPeekTable *table);
extern void now_liveness_rollback(NowPeekTable *table);
...
/* P5, the transition tail (now_event.c). Same two-entry shape, and the
   ...
   no record. */
extern int now_event_boot(NowPeekTable *table);
extern void now_event_rollback(NowPeekTable *table);
...
typedef struct NowExtInstallContext {
    Handle self;
} NowExtInstallContext;

static NowPeekTable *install_make_table(void *opaque)
{
    NowPeekTable *table;
    (void)opaque;
...
static void install_drop_table(void *opaque, NowPeekTable *table)
{
    (void)opaque;
    if (table == NULL) return;
    table->magic = 0;
    if (gNowExtTable == table) gNowExtTable = NULL;
    DisposePtr((Ptr)table);
}

#define INSTALL_STAGE(NAME, BOOT, ROLLBACK) \
    static int install_##NAME(void *opaque, NowPeekTable *table) \
    { (void)opaque; return BOOT(table); } \
    static void rollback_##NAME(void *opaque, NowPeekTable *table) \
    { (void)opaque; ROLLBACK(table); }

INSTALL_STAGE(content, now_content_boot, now_content_rollback)
...
{
    NowExtInstallContext *context = (NowExtInstallContext *)opaque;
    SelectorFunctionUPP gestalt_upp;
    OSErr err;

    /* Magic and the private callback pointer become valid together, but no
       process can discover either until NewGestalt succeeds below. */
    ...
    /* NewGestalt is the only fallible publication operation. Everything
       after it is the non-failing commit tail: retain the INIT and install
       the event hook only after every allocation and private queue entry is
       ready. A failed pre-publication path therefore leaves the resource
       attached and lets the Resource Manager reclaim it normally. */
    DetachResource(context->self);
...
void _start(void)
{
    NowExtInstallContext context;
    NowExtInstallOps ops;

    RETRO68_RELOCATE();
    Retro68CallConstructors();

    context.self = Get1Resource('INIT', 128);
    if (context.self == NULL) return;

    ops.context = &context;
    ops.make_table = install_make_table;
    ops.drop_table = install_drop_table;
    ops.prepare_content = install_content;
    ops.rollback_content = rollback_content;
    ops.prepare_event = install_event;
    ops.rollback_event = rollback_event;
    ops.prepare_drag = install_drag;
    ...
    /* Allocation and private initialization are one reversible transaction.
       The returned table exists only after the final registration commit. */
    (void)now_ext_install_transaction(&ops);
