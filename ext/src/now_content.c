 ...
 * system heap. In the transactional constructor this block is boot-critical:
 * allocation failure aborts the unpublished resident and the coordinator
 * releases every earlier stage. That is stricter than a runtime capability
 * absence; it prevents a partially useful resident from masking a failed
 * construction.
 */
int now_content_boot(NowPeekTable *table)
{
    ...
    if (!now_content_chain_is_sound()) {
        return 0;                     /* no standard procs, no plane */
    }
    gHooks = gStd;
...
/* Used only while _start still owns an unpublished construction. No port can
   have been armed at that point, so releasing the block and clearing the
   table/global publication is the complete inverse of now_content_boot. */
void now_content_rollback(NowPeekTable *table)
{
    if (gBlock != NULL) {
