...
int cloud_preview_parse_begin(const char *reply, CloudPreviewBegin *out)
{
    ...
    min_row = out->depth == 8 ? out->width
                              : out->width / 8 + (out->width % 8 != 0);
    if (out->row_bytes < min_row) {
        return 0;
    }
    if (out->bytes < 1 || out->bytes > kCloudPreviewMaxBytes) {
        return 0;
    }
    if (out->row_bytes != out->bytes / out->height
        || out->bytes % out->height != 0) {
        return 0;
    }
    return 1;
}
