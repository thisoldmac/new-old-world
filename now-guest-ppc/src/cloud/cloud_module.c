...
/* The list the current mode is actually showing: drive mode swaps in
   cloud_drive_view's own four-column browser, contacts mode swaps in
...
static void choose_service(int index)
{
    ...
    retitle_button();
    clear_list();
    /* Three lists, one shown: the mode owns which browser is the
       page's, and the history pair exists only where a history does.
       Hidden is for the control the mode does not use; empty-history
       dimming is cloud_idle's HiliteControl diff, never a hide. */
    if (g_owner != NULL && g_visible) {
        show_control(g_browser, !g_drive_mode && !g_contacts_mode);
        show_control(cloud_drive_view_browser(), g_drive_mode);
        show_control(cloud_contacts_view_browser(), g_contacts_mode);
...
static OSErr cloud_create(WindowRef owner, const Rect *body)
{
    ...
    /* Photos' create only records the window; the GWorld waits for
       pixels. The wire's ONE cloud.preview hook is registered once,
       below, for the shared well — not per view. */
    {
...
static void cloud_show(Boolean visible)
{
    ...
    show_control(g_browser, visible && !g_drive_mode && !g_contacts_mode);
    show_control(cloud_drive_view_browser(), visible && g_drive_mode);
    show_control(cloud_contacts_view_browser(), visible && g_contacts_mode);
...
static void apply_layout(void)
{
    ...
    /* g_r fits exactly one mode, so only the browser that mode shows
       ...
       until their own mode's apply_layout runs. Drive's and Contacts'
       browsers are placed by their own view's layout op, below. */
    if (!g_drive_mode && !g_contacts_mode && g_browser != NULL) {
...
{
    /* All three browsers follow the window: a hidden one must not
       wake in yesterday's activation state when its mode returns. */
    ControlRef lists[3];
    int i;

    lists[0] = g_browser;
    lists[1] = cloud_drive_view_browser();
    lists[2] = cloud_contacts_view_browser();
    for (i = 0; i < 3; ++i) {
        if (lists[i] == NULL) {
            continue;
        }
