...
{
    unsigned long total =
        (row->data_bytes > 0 ? (unsigned long)row->data_bytes : 0UL)
        + (row->rsrc_bytes > 0 ? (unsigned long)row->rsrc_bytes : 0UL);

    if (total < 1024) {
        snprintf(buf, cap, "%lu bytes", total);
    } else if (total < 1024L * 1024L) {
        snprintf(buf, cap, "%lu K", total / 1024UL);
    } else {
        snprintf(buf, cap, "%lu.%lu MB", total / (1024UL * 1024UL),
                 (total % (1024UL * 1024UL)) / (105UL * 1024UL));
    }
}
...
static OSErr view_create(WindowRef owner)
{
    ...
    if (SetDataBrowserCallbacks(g_browser, &callbacks) != noErr
        || add_column(kColName, "Name", 200, true, 0) != noErr
        || add_column(kColKind, "Kind", 110, false, 1) != noErr
        || add_column(kColSize, "Size", 64, false, 2) != noErr
        || add_column(kColModified, "Modified", 90, false, 3) != noErr) {
        now_control_dispose(g_browser);
        g_browser = NULL;
        dispose_callbacks();
        return noErr;
