                          ...
                          DataBrowserItemDataRef data, Boolean changeValue)
{
    ...
    case kColSize:
        if (row->folder) {
            ...
            if (total < 1024) {
                snprintf(buf, sizeof buf, "%lu bytes", total);
            } else if (total < 1024L * 1024L) {
                snprintf(buf, sizeof buf, "%lu K", total / 1024UL);
            } else {
...
{
    /* Disposal can notify through callbacks already installed on the
       browser, so the UPPs must outlive the control. */
    if (g_browser != NULL) {
