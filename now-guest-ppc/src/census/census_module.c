...
static OSErr census_create(WindowRef owner, const Rect *body)
{
    ...
    if (SetDataBrowserCallbacks(g_browser, &cb) != noErr
        || add_column(kColName, "Field", 150, true, 0) != noErr
        || add_column(kColValue, "Value", 200, false, 1) != noErr) {
        now_control_dispose(g_browser);
        g_browser = NULL;
        dispose_upps();
        return memFullErr;
    ...
    if (g_run == NULL || g_rerun == NULL) {
        if (g_run != NULL) {
            now_control_dispose(g_run);
            g_run = NULL;
        }
        if (g_rerun != NULL) {
            now_control_dispose(g_rerun);
            g_rerun = NULL;
        }
        now_control_dispose(g_browser);
        g_browser = NULL;
