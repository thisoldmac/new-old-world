                          ...
                          DataBrowserItemDataRef data, Boolean changeValue)
{
    ...
    if (item >= kGroupIdBase
        ...
        case kColSize: {
            ...
            for (k = 0; k < g_group_size[gid]; ++k) {
                ...
                if (m->size_k > 0) {
                    total += (unsigned long)m->size_k;
                }
            }
            sw_size_k_text(total, caption, sizeof caption);
            break;
        }
        case kColState: {
    ...
    case kColSize:
        if (it->size_k >= 0) {
            sw_size_k_text((unsigned long)it->size_k, caption,
                           sizeof caption);
        } else {
            caption[0] = '\0';
        }
...
{
    /* The control owns the installed callbacks until its disposal returns. */
    if (g_browser != NULL) {
