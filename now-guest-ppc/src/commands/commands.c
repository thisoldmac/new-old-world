...
static void run_update(const char *request_json, long id, char *out, long cap)
{
    ...
    if (component_word[0] != '\0') {
        ...
        /* A bare console or wire command cannot stand in for consent.
           hostApproved is emitted only by the native host after its own
           confirmation; the guest still binds it to the exact offer. */
