...
static void remember(WindowRef window, ControlRef control, short procID)
{
    ...
    for (i = 0; i < kSlots; ++i) {
        if (g_slots[i].control == NULL) {
            ...
            g_slots[i].window = window;
            g_slots[i].procID = procID;
            g_slots[i].born_generation = g_generation;
            return;
        }
    }
                                       ...
                                       unsigned long marker)
{
    ...
    for (;;) {
        int newest = -1;
        unsigned long generation = marker;

        for (i = 0; i < kSlots; ++i) {
            if (g_slots[i].control != NULL && g_slots[i].window == window
                && g_slots[i].born_generation > generation) {
                newest = i;
                generation = g_slots[i].born_generation;
            }
        }
        if (newest < 0) return;
        now_control_dispose(g_slots[newest].control);
    }
}
