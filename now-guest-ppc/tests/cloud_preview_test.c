...
static void test_parse_rejects(void)
{
    ...
    CHECK(cloud_preview_parse_begin(
              "{\"id\":1,\"transfer\":1,\"width\":65536,"
              "\"height\":65537,\"depth\":8,\"rowBytes\":65536,"
              "\"bytes\":65536}", &b) == 0,
          "32-bit row multiplication cannot wrap into a small allocation");
    CHECK(cloud_preview_parse_begin(
              "{\"id\":1,\"transfer\":1,\"width\":2147483647,"
              "\"height\":1,\"depth\":1,\"rowBytes\":1,\"bytes\":1}",
              &b) == 0,
          "1-bit row rounding cannot overflow width plus seven");
