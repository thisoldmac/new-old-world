...
static void check_formatters(void)
{
    ...
    sw_size_k_text(4194304UL, buf, sizeof buf);
    check(strcmp(buf, "4096.0M") == 0,
          "a combined four-gigabyte fork size formats without bytes overflow");
    check(sw_fork_size_k(2147483647L, 2147483647L) == 4194304L,
          "two maximum Classic fork lengths sum after division");
    check(sw_fork_size_k(-1L, 20L) == -1L,
          "invalid negative fork length stays unavailable");
