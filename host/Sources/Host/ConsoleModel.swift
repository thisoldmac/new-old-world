    ...
    static let catalog: [String: CommandInfo] = [
        ...
        "launch": .init(
            ...
            help: ["launch — open an application on the connected Mac",
                   "  Usage: launch <name | full path | #n>",
                   "  A bare name is found by an exact-name search of that",
                   "  Mac's startup disk; the app comes to its front, so a",
                   "  screenshot or ps right after shows it. Several apps",
                   "  with one name refuse and store a numbered list —",
                   "  \"vers <name>\" shows it, \"launch #2\" picks from it."]),
        "vers": .init(
