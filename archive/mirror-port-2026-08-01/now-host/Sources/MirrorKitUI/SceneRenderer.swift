...
public struct SceneRenderer {
    ...
    /// The open menu's REAL per-item geometry, from its own MDEF
    /// (`menugeom`) — nil when nothing has fetched one yet, the fetch
    /// hasn't answered, or the guest refused it (no NOW Extension, or one
    /// that predates the act plane). `drawDropdown` and `dropdownItem` both
    /// fall back to the uniform 16px assumption in every one of those
    /// cases — never a guess dressed as a reading.
    ...
    public func draw(in ctx: GraphicsContext, size: CGSize) {
        ...
        if let openMenu, let menus = scene.menubar?.menus,
           menus.indices.contains(openMenu) {
            drawDropdown(ctx, menus[openMenu], geometry: openMenuGeometry)
        }
        if let dragOutline {
    ...
    /// **Real geometry when it is usable, the uniform 16px assumption
    /// otherwise — and both paths return one CGRect the drawing and the
    /// hit test share, so they can never disagree with EACH OTHER even
    /// when one of them is wrong about the real machine.**
    ///
    /// `geometry` is the guest's own `menugeom` answer
    /// (`MirrorKit.MenuGeometry`) — real per-item rects inside a real
    /// `width`×`height` box, read from the menu's own MDEF rather than
    /// estimated from title lengths. `.matches(menu)` is what decides
    /// whether it is usable HERE: the wrong menu id (a stale read from a
    /// menu that has since closed) or an item the geometry does not answer
    /// for (the item cap, or a menu that changed shape since the read)
    /// both fall back to the estimate entirely — never a mix of real rects
    /// for some items and invented ones for the rest.
    public static func dropdownFrame(
        _ menu: MirrorKit.Scene.Menu,
        geometry: MirrorKit.MenuGeometry? = nil
    ) -> CGRect {
        if let geometry, geometry.matches(menu) {
            // The MDEF's own box IS the on-screen size — no estimate
            // needed. UNVERIFIED against a real machine: the ±1px border
            // this adds is this renderer's own drawn frame, the same one
            // pixel the estimate below reserves with its "+ 4" / "- 6".
            return CGRect(x: CGFloat(menu.left) - 1,
                          y: Platinum.menubarHeight,
                          width: CGFloat(geometry.width) + 2,
                          height: CGFloat(geometry.height) + 2)
        }
        // Rows follow the guest's 16 px standard; the panel width is an
        // estimate — long titles clip rather than shift the rows. This is
        // the fallback for an extension that is absent, stale, or has not
        // answered yet — not a lesser drawing, an honest one.
    ...
    /// One item's on-screen rect inside an already-placed dropdown `frame`
    /// — real, from `geometry`, when it answers for this item; the uniform
    /// 16px row at this item's position in the menu otherwise. The single
    /// place both the drawing loop and the hit test read an item's rect
    /// from, so they cannot compute two different boxes for the same row.
    private static func itemFrame(
        _ item: MirrorKit.Scene.MenuItem, at position: Int, in frame: CGRect,
        geometry: MirrorKit.MenuGeometry?
    ) -> CGRect {
        if let geometry, let r = geometry.rect(forItemIndex: item.index) {
            // +1 on each axis: the same 1px margin the fallback box below
            // keeps between the drawn border and its own rows, so the
            // border stroke never sits under the geometry's own item 0.
            return CGRect(x: frame.minX + 1 + CGFloat(r.left),
                          y: frame.minY + 1 + CGFloat(r.top),
                          width: CGFloat(r.width), height: CGFloat(r.height))
        }
        return CGRect(x: frame.minX + 1, y: frame.minY + 2 + CGFloat(position * 16),
                      width: frame.width - 2, height: 16)
    }

    /// The item under a guest point inside the dropdown, or nil.
    public static func dropdownItem(
        _ menu: MirrorKit.Scene.Menu, x: Int, y: Int,
        geometry: MirrorKit.MenuGeometry? = nil
    ) -> MirrorKit.Scene.MenuItem? {
        let frame = dropdownFrame(menu, geometry: geometry)
        guard CGFloat(x) >= frame.minX, CGFloat(x) < frame.maxX,
              CGFloat(y) >= frame.minY else { return nil }
        let usable = geometry?.matches(menu) == true ? geometry : nil
        for (position, item) in menu.items.enumerated() {
            let itemBox = itemFrame(item, at: position, in: frame,
                                    geometry: usable)
            if CGFloat(x) >= itemBox.minX, CGFloat(x) < itemBox.maxX,
               CGFloat(y) >= itemBox.minY, CGFloat(y) < itemBox.maxY {
                return item
            }
        }
        return nil
    ...
    private func drawDropdown(_ ctx: GraphicsContext,
        ...
        let usable = geometry?.matches(menu) == true ? geometry : nil
        let frame = Self.dropdownFrame(menu, geometry: usable)
        ctx.fill(Path(frame.offsetBy(dx: 2, dy: 2)),
                 with: .color(.black.opacity(0.35)))
        ctx.fill(Path(frame), with: .color(Platinum.g0))
        ctx.stroke(Path(frame), with: .color(Platinum.g6), lineWidth: 1)
        ...
        for (position, item) in menu.items.enumerated() {
            let box = Self.itemFrame(item, at: position, in: frame,
                                     geometry: usable)
            ...
            if item.separator {
                // A REAL separator's rect is its own thin sliver
                // (`~6px` against a `~16px` item — the exact mismatch
                // `menugeom` exists to answer for) when geometry supplied
                // one; the fallback still assumes a mid-row line inside
                // the uniform 16px box.
                ctx.fill(Path(CGRect(x: box.minX + 2,
                                     y: box.midY - 0.5,
                                     width: box.width - 4, height: 1)),
                         with: .color(Platinum.g3))
                continue
            }
            ...
            var clipped = ctx
            clipped.clip(to: Path(CGRect(x: box.minX, y: box.minY,
                                         width: max(0, box.width - 34),
                                         height: box.height)))
            let textBaseline = box.minY + box.height - 4
            if item.mark {
                clipped.draw(clipped.resolve(Text("✓").font(Platinum.systemFont(12))
                                .foregroundColor(color)),
                             at: CGPoint(x: box.minX + 6, y: box.midY),
                             anchor: .center)
            }
            sysText(item.title, clipped, x: box.minX + 14,
                    baselineY: textBaseline, color: color)
            // ⌘ isn't in the bitmap strike — draw the shortcut via Text.
            if !item.cmd.isEmpty {
                ctx.draw(ctx.resolve(Text("⌘\(item.cmd)")
                             .font(Platinum.systemFont(12))
                             .foregroundColor(color)),
                         at: CGPoint(x: box.maxX - 8, y: box.midY),
                         anchor: .trailing)
            }
        }
    }
    ...
    /// Two-letter initials for a process name (`AB` from `Adobe Bridge`).
    /// Was the shelf's own glyph; now shared with the host chrome's Live
    /// Processes section (`MirrorModuleView`) since the canvas no longer
    /// draws processes at all — see docs/local scratch notes for W0.1.
