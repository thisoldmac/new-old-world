...
public enum ActionModel {
    ...
    public static func availability(
        ...
        case .key:
            /* RETRACTED 2026-08-01, lane L7. This case used to be a
               function of `mods` — `mods == 0` available, `mods != 0`
               unavailable outright — on the true-at-the-time fact that
               CarbonLib has no `PPostEvent` and the guest refused any
               modifier before it could be carried. That is no longer the
               whole story: the guest's `key` verb now routes a nonzero
               `mods` through the act plane's own key op
               (`contract/asyncapi.yaml:key`, "THE ROUTE, AS OF
               2026-08-01"; `now-guest-ppc/src/input/input_cmds.c
               :run_key_via_act`), served by the NOW Extension's 68K
               resident filter — which CAN reach `PPostEvent`, the same
               reach `winact`'s window moves and `menuact`'s presses
               already use. CarbonLib's own application code still cannot,
               and that half of the old comment stands.

               So `key` is available regardless of `mods` — NOW's contract
               now declares a command that carries it. Whether the ROUTE
               is armed on the paired guest at this moment — extension
               present, current, target process pumping its event loop —
               is a fact about that guest, not about this vocabulary, and
               this function has no way to see it: the guest answers `ok`
               or refuses with a named reason (`act-plane-absent`,
               `act-plane-stale`, `act-plane-dark`, `act-no-anchor`, …),
               and `MirrorActionDriver` forwards that refusal honestly
               rather than this side manufacturing one in advance for a
               mechanism that, as of this change, sometimes exists.
               Blocking here the way it used to would refuse a caller with
               a current extension installed exactly as hard as one with
               none at all — the wrong failure for a route that is
               sometimes armed.
