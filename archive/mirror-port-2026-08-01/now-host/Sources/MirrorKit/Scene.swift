...
public struct Scene: Codable, Equatable, Sendable {
    ...
    public struct Menu: Codable, Equatable, Sendable {
        ...
        /// **Never emitted by NOW's guest** — `scene_json.c` states why in
        /// its own comment: "nothing this walk reads says which menu is
        /// the Apple menu - a title byte or a menu id could be made to
        /// look like evidence and would be a guess". A required `apple`
        /// key made every real scene undecodable; absence decodes to
        /// `false` — "not asserted", not "asserted to be an ordinary
        /// menu" — matching the default `MirrorSceneAdapter` already uses
        /// when crossing `NOWSceneDocument`.
    ...
    public struct MenuItem: Codable, Equatable, Sendable {
        ...
        ///
        /// **Absent on the wire for an item with no shortcut** —
        /// `scene_json.c :: put_menubar` writes the key only when
        /// `it->cmd != '\0'`. 65 of the 85 real Finder items in the
        /// 2026-08-01 live capture had none, which is what exposed a bare
        /// synthesized decode here: a required `cmd` key made a real scene
        /// undecodable. "" already means "no shortcut" to every reader of
        /// this field, so absence decodes to the same value rather than
        /// inventing a third state nothing reads.
        public var cmd: String

        enum CodingKeys: String, CodingKey {
            case title, index, separator, enabled, mark, cmd
        }
    }
    ...
    public struct Control: Codable, Equatable, Sendable {
        ...
        ///
        /// **Absent when the reference layer could not name this control**
        /// (`scene_json.c :: put_ref`: "present for every control the
        /// reference layer could name and absent for the rest"). "" is
        /// already MirrorKit's own vocabulary for "not actuatable" — the
        /// same value `axdo` refuses on — so a missing key decodes to it
        /// rather than becoming a new state.
        public var ref: String
        /// "control", or "scrollbar" when ranged.
        ///
        /// **Never emitted by NOW's guest, full stop** — `scene_json.c`
        /// says why in its own comment: "the walk reads a ControlRecord,
        /// not its defProc, so it cannot say what KIND of control this
        /// is". A required `role` key made every real scene undecodable;
        /// absence decodes to `"control"`, the base role, rather than a
        /// guessed `"scrollbar"` — deriving one from `min`/`max` would be
        /// exactly the plausible-false-claim the producer itself refused
        /// to make.
        ...
        public var min: Int?
        public var max: Int?
        /// **Never emitted by NOW's guest** — the same reason as `role`:
        /// "checked is meaningless without" a defProc read
        /// (`scene_json.c`). Absent decodes to `false`: "not asserted",
        /// not "asserted unchecked".
        public var checked: Bool

        enum CodingKeys: String, CodingKey {
            case ref, role, title, rect, enabled, visible, value, min, max
            case checked
        }
    }
...
extension Scene.Menu {
    public init(from decoder: Decoder) throws {
        ...
        // Never on the wire — see the property's own doc comment.
...
extension Scene.MenuItem {
    public init(from decoder: Decoder) throws {
        ...
        mark = try c.decode(Bool.self, forKey: .mark)
        // Absent for an item with no shortcut — see the property's own
        // doc comment.
...
extension Scene.Control {
    public init(from decoder: Decoder) throws {
        ...
        // Never on the wire — see the property's own doc comment.
        role = try c.decodeIfPresent(String.self, forKey: .role) ?? "control"
        title = try c.decode(String.self, forKey: .title)
