...
/// window at all.
///
/// ## Arming the plane, and what is still refused
///
/// **This host now arms the plane, and never by guessing an A5.** The
/// stale version of this comment said no NOW command reports an A5 — false
/// since `observe`/`axsnap` started carrying one, and doubly false now that
/// `qdtrace start` itself takes `front: true` and resolves the A5 on the
/// GUEST, where the anchor oracle that can answer the question actually
/// lives (`qdtrace_target.h`, `qdtrace_cmd.c`'s `resolve_start_target`).
/// This host still never touches an A5 itself: when a drain comes back
/// empty for no stated reason and `status` says nothing is armed, `attribute`
/// issues `qdtrace start` with `front: true` — the front window is the only
/// thing `join` ever attaches ops to anyway — and reports what the guest
/// did with that request.
///
/// **What is still refused, in the guest's own words.** Arming can fail
/// honestly: no front process, the anchor oracle ambiguous or mismatched
/// about it, its anchor captured too long ago to trust for arming rather
/// than merely observing. None of those become a happier sentence here —
/// `armAttemptOutcome` forwards the guest's code and message the same way
/// `apply` forwards a drain refusal, and a caller that resends an act after
/// arming still needs the truth about whether that act's target changed.
///
...
final class MirrorContentJoin {
    ...
    /// `attribution`'s answer for the one status shape that means "nothing
    /// is armed" (`active.a5` of zero, or unparseable). It is also the
    /// SIGNAL `attribute` below matches on to decide whether to try arming
    /// the front window — a plain string compare rather than a second
    /// a5-reading, so the two can never disagree about which status this is.
    static let armGap =
        "Nothing is armed to record drawing yet. This host arms the front "
        + "window automatically — qdtrace start front:true — and reports "
        + "what the guest does with that request."
    ...
    ///
    /// Finding the FIRST cause is now also where this host acts on it: `why
    /// == armGap` is exactly the status shape `attribution` reads off
    /// `active.a5`, so matching it here cannot disagree with what that
    /// function just said. Every other shape (count mode, armed and idle, no
    /// answer) is left as `attribution` describes it — this host only ever
    /// arms the one thing `join` draws into, the front window, and only when
    /// nothing is armed at all.
    private func attribute(_ scene: MirrorKit.Scene,
                           completion: @escaping (MirrorKit.Scene,
                                                  Outcome) -> Void) {
        listener.runCommand("qdtrace", args: ["op": "status"]) { [weak self] result in
            guard let self else { return }
            let why = Self.attribution(result)
            guard why == Self.armGap else {
                completion(scene, .empty(why))
                return
            }
            self.armFrontWindow(scene: scene, completion: completion)
        }
    }

    /// Arms the front window, in `record` mode — `count` is `qdtrace
    /// start`'s own default and would arm a plane that counts drawing
    /// without ever recording an op, which is no plane at all from a join
    /// that exists to draw ops. `front: true` is the whole ask: the GUEST
    /// resolves which process that is and its A5 (`qdtrace_target.h`), so
    /// this host still names no A5 anywhere in this file.
    private func armFrontWindow(scene: MirrorKit.Scene,
                                completion: @escaping (MirrorKit.Scene,
                                                       Outcome) -> Void) {
        listener.runCommand(
            "qdtrace",
            args: ["op": "start", "front": "true", "mode": "record"]) {
            result in
            completion(scene, Self.armAttemptOutcome(result))
        }
    }

    /// Turn one `qdtrace start` reply into an outcome. Split out from
    /// `armFrontWindow` for the same reason `apply` is split from `join`: a
    /// decision reachable only through a socket is a decision nobody tests.
    ///
    /// Never `.attached` — arming is not drawing. `requested: true, armed:
    /// false` (qdtrace_cmd.c's own claim discipline) means the honest report
    /// here is "nothing drew YET", the same `.empty` a quiet drain gets,
    /// with a sentence that says a request was just made rather than
    /// implying a machine that has been idle all along.
    static func armAttemptOutcome(_ result: CommandResult) -> Outcome {
        guard result.ok else {
            let error = result.error
            /* The guest's own refusal - no front process, the anchor oracle
               ambiguous or mismatched about it, an anchor too old to trust
               for ARMING (resolve_start_target's a5-stale) - forwarded
               verbatim, the same discipline `apply`'s drain refusal uses. */
            return .refused(error.map { "\($0.message) [\($0.code)]" }
                ?? "The Mac refused to arm the front window without saying "
                + "why.")
        }
        guard case .object(let object)? = result.outputObjects?["qdtrace"],
              case .string("start")? = object["cmd"] else {
            return .refused(
                "The Mac answered the arm attempt with something this host "
                + "cannot read as one.")
        }
        return .empty(
            "Nothing was armed to record drawing, so this host just armed "
            + "the front window for it. Press again to see what it draws.")
    }
