...
final class GuestListener: ObservableObject {
    ...
    /// Asks the guest to capture one GLOBAL-coordinate screen rect,
    /// side-effect-free — no process is fronted, unlike
    /// `requestProcessShot`. This is the pixel-island fallback's fetch
    /// (`SceneIslands.Capture`): it needs a window's CURRENT interior
    /// without disturbing which process is frontmost or forcing a
    /// redraw. The reply is an ordinary capture transfer, so it settles
    /// the same `pendingCapture` path a plain `requestCapture` does.
    func requestCaptureRegion(left: Int, top: Int, right: Int, bottom: Int,
                              depth: Int? = nil,
        ...
        session.sendCaptureRegion(id: id, left: left, top: top, right: right,
