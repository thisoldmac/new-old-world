...
struct HostRootView: View {
    ...
    /// The Mirror pane's standalone window, when one has ever been opened.
    /// Held here rather than inside the "mirror" case of `detail` below:
    /// `HostRootView` is the one view in this tree that is never torn down
    /// by a sidebar selection change, so a window opened while looking at
    /// Mirror survives navigating away and back. One controller per launch
    /// — `MirrorDetachedWindowController` itself refuses to construct a
    /// second `MirrorModuleModel`, and this is what keeps IT from being
    /// asked to.
    @State private var mirrorDetachController: MirrorDetachedWindowController?
    ...
    private var detail: some View {
        ...
        case "mirror":
            MirrorModuleView(model: state.mirror, detach: {
                let controller = mirrorDetachController
                    ?? MirrorDetachedWindowController(model: state.mirror)
                mirrorDetachController = controller
                controller.show()
            })
        case "console":
