    ...
    private func startAgentIntegrationServer() {
        ...
        do {
            let server = try AgentIntegrationLocalServer(
                ...
                case .scene:
                    /* Two shapes: a page continues what a take staged.
                       There is no abandon, unlike capture — see
                       AgentIntegrationSceneControl's header. */
                    if let sceneID = request.sceneID,
                       let offset = request.sceneOffset {
                        return .scene(
                            agentIntegration.scenePage(
                                sceneID: sceneID, offset: offset))
                    }
                    return .scene(
                        await agentIntegration.scene(
                            staleAfterMs: request.sceneStaleAfterMs))
                case .audit:
                ...
                case .key:
                    /* The input plane's own verb, not the four-op act
                       dispatch above — it shares this switch's addressing
                       check and nothing else with `.windowAct`'s neighbours.
                       The codec has already refused a request naming no
                       key and a mods value outside the modifier mask, so
                       this branch reads a validated request and refuses
                       only the one that named none at all. */
                    guard let act = request.keyRequest else {
                        return .key(.refused(.init(
                            code: "now-key-invalid",
                            message: "The request named no key")))
                    }
                    return .key(await agentIntegration.key(act))
                case .transferCancel:
