...
final class Session {
    ...
    private func finishCapture(_ end: CaptureEnd) {
        ...
        let format = CaptureFormat(
            ...
            // Absent means (0, 0) at scale 1 — a sender that predates the
            // fields always captured the whole screen at the origin.
            originX: begin.originX ?? 0, originY: begin.originY ?? 0,
            scale: begin.scale ?? 1)
    ...
    /// The side-effect-free sibling of `sendProcessShot`: names a rect
    /// directly (GLOBAL screen coordinates) instead of a process, so
    /// nothing is fronted. The answer rides the same capture transport,
    /// so the receive state is primed identically — this is the pixel-
    /// island producer's fetch (`SceneIslands.Capture`).
    func sendCaptureRegion(id: Int, left: Int, top: Int, right: Int,
        ...
        solicitedId = id
        captureStart = Date()
        send(.captureRegion(CaptureRegion(
            id: id, left: left, top: top, right: right, bottom: bottom,
            depth: depth, chunkKb: tuning.chunkKb, paceMs: tuning.paceMs,
            pack: tuning.pack)))
    }
