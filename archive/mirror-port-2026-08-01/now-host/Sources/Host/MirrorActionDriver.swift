...
/// - **`.key` → `key`, `mods` forwarded honestly.** A plain keystroke posts
///   through the guest's input plane, as it always has. RETRACTED
///   2026-08-01, lane L7: a MODIFIED keystroke used to be refused here
///   before a request was even built. It no longer is — the guest's `key`
///   verb now routes a nonzero `mods` through the act plane's own key op,
///   served by the NOW Extension's 68K resident filter — so this driver
///   sends it and lets the GUEST'S OWN reply decide: dispatched when the
///   route is armed, refused by name (`act-plane-absent`, `-stale`, `-dark`,
///   `act-no-anchor`, …) when it is not. See `ActionModel.availability
///   (.key)` for the fuller argument against blocking it here in advance.
...
struct MirrorActionDriver {
    ...
    func drive(_ action: MirrorAction) async -> Outcome {
        ...
        case .key(let name, let code, let char, let mods):
            /* `availability` now says `.available` for every `mods` — see
               its own comment for why blocking a nonzero one here would be
               this side refusing on behalf of a route that sometimes IS
               armed. So `mods` is forwarded exactly as the caller resolved
               it, and the guest's reply is what decides: `report` below
               turns a dispatch into `.dispatched` and a guest refusal
               (extension absent, stale, dark, or no anchor — its own named
               reasons) into `.refused` with that same sentence, never into
               `.unavailable` — the machine was asked and answered, which is
               a different fact from never asking at all. */
