...
final class AgentIntegrationHostAdapter {
    ...
    private lazy var captureControl = AgentIntegrationCaptureControl(
        listener: listener,
        currentSessionID: { [unowned self] in connectedSessionID() })
    /* Beside the capture control for the same reason streamControl is: the
       same one-transfer-lane observation, taken a third way — structure
       rather than pixels. See AgentIntegrationSceneControl's header for why
       it needs neither capture's busy pre-check nor its abandon. */
    private lazy var sceneControl = AgentIntegrationSceneControl(
        listener: listener,
        currentSessionID: { [unowned self] in connectedSessionID() })
    ...
    /// Read one menu's real per-item geometry, from its own MDEF — for the
    /// pane's own dropdown drawing and hit-testing (`SceneRenderer
    /// .dropdownFrame(_:geometry:)`), not for an act: nothing is armed and
    /// no click is queued.
    func menuGeom(_ request: AgentIntegrationMenuGeomRequest) async
        -> AgentIntegrationMenuGeomResult {
        await actControl.menuGeom(request)
    }
    ...
    /// One Mirror IR scene of the connected machine, staged for the pages
    /// the local surface has to carry it in. The lane's own reasoning lives
    /// in `AgentIntegrationSceneControl`.
    func scene(staleAfterMs: Int?) async -> AgentIntegrationSceneResult {
        await sceneControl.scene(staleAfterMs: staleAfterMs)
    }

    func scenePage(sceneID: UUID, offset: Int) -> AgentIntegrationSceneResult {
        sceneControl.page(sceneID: sceneID, offset: offset)
    }
