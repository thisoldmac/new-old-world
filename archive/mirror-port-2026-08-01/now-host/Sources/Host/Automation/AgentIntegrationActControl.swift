...
final class AgentIntegrationActControl {
    ...
    ///
    /// **`mods` is forwarded exactly as given, and the GUEST decides** —
    /// never smoothed over here. `mods == 0` posts through the plain event
    /// queue; a nonzero `mods` now routes through the act plane's own key
    /// op, served by the NOW Extension's 68K resident filter (CarbonLib
    /// itself has no `PPostEvent`, so this application's own code still
    /// cannot say what was held down while a key was posted — the resident
    /// filter can). The guest answers `ok` when that route is armed and
    /// refuses by name (`act-plane-absent`, `-stale`, `-dark`, `act-no
    /// -anchor`, …) when it is not — forwarded below, never replaced.
    /// `ActionModel.availability(.key)` no longer refuses a non-zero `mods`
    /// in advance, for the same reason: whether the route is armed is a
    /// fact about the paired guest, and this adapter is where that fact
    /// actually gets read (`contract/asyncapi.yaml:key`,
    /// `docs/input-plane-decisions.md`).
    ...
    /// Read one menu's real per-item geometry, from its own MDEF —
    /// `menugeom` is served outright like `textget`/`textset`, never armed
    /// and never a click (`now_act_run_menugeom`'s own comment), so this
    /// reads the verb's own `Menu`/`Width`/`Height`/`Items`/`Item N` rows
    /// directly rather than going through `dispatch()`'s `Dispatch` row —
    /// the same reasoning `key` reads its own lower-case rows for.
    func menuGeom(_ request: AgentIntegrationMenuGeomRequest) async
        -> AgentIntegrationMenuGeomResult {
        guard request.isWellFormed else {
            return .refused(Self.failure(
                "now-menu-geom-invalid",
                "A menu geometry read names a menu id, and either both "
                    + "process serials or neither"))
        }
        guard let sessionID = currentSessionID() else {
            return .unavailable(.guest)
        }
        var args: [String: String] = ["menu": String(request.menu)]
        if let hi = request.serialHi, let lo = request.serialLo {
            args["serialHi"] = String(hi)
            args["serialLo"] = String(lo)
        }

        let outcome = await run(verb: "menugeom", args: args)
        guard currentSessionID() == sessionID else {
            audit(.warn, "menugeom: the paired guest changed while the "
                      + "read was in progress")
            return .unavailable(Self.failure(
                "now-menu-geom-outcome-unknown",
                "The paired guest changed while the menu geometry read "
                    + "was in progress").asUnavailable)
        }
        switch outcome {
        case .timedOut:
            audit(.warn, "menugeom: no answer in time")
            return .refused(Self.failure(
                "now-menu-geom-outcome-unknown",
                "The paired guest did not answer the menu geometry read "
                    + "in time"))
        case .result(let result) where !result.ok:
            /* The guest's own sentence — `act-plane-absent` when the NOW
               Extension is not installed, current or reachable, or
               `now-menu-no-menu` when the id names nothing this process
               has — forwarded rather than replaced. */
            audit(.warn, "menugeom refused: "
                      + Self.sanitized(result.error?.message ?? ""))
            return .refused(Self.failure(
                "now-menu-geom-refused",
                Self.bounded(result.error?.message
                    ?? "The paired guest refused the menu geometry read")))
        case .result(let result):
            let rows = Self.rows(from: result, verb: "menugeom")
            guard let menuStr = rows["Menu"], let menu = Int(menuStr),
                  let widthStr = rows["Width"], let width = Int(widthStr),
                  let heightStr = rows["Height"], let height = Int(heightStr),
                  let countStr = rows["Items"], let count = Int(countStr)
            else {
                audit(.warn, "menugeom: the guest answered without its own "
                          + "menu/width/height/item-count rows")
                return .refused(Self.failure(
                    "now-menu-geom-outcome-unknown",
                    "The paired guest answered the menu geometry read "
                        + "without saying the menu, its size, or its item "
                        + "count"))
            }
            var items: [AgentIntegrationMenuItemRect] = []
            if count > 0 {
                for i in 1...count {
                    guard let raw = rows["Item \(i)"],
                          let rect = Self.parseMenuItemRect(raw) else {
                        audit(.warn, "menugeom: item \(i) of \(count) "
                                  + "missing or malformed")
                        return .refused(Self.failure(
                            "now-menu-geom-outcome-unknown",
                            "The paired guest reported \(count) items and "
                                + "did not answer for item \(i)"))
                    }
                    items.append(rect)
                }
            }
            audit(.info, "menugeom \(menu): \(items.count) of \(count) "
                      + "item(s)")
            return .completed(.init(menu: menu, width: width, height: height,
                                    items: items))
        }
    }

    /// `"top, left, bottom, right"` (`now_act_run_menugeom`'s own
    /// `"%d, %d, %d, %d"`) — parsed rather than assumed exact, so a stray
    /// extra space does not silently drop an item.
    private static func parseMenuItemRect(_ raw: String)
        -> AgentIntegrationMenuItemRect? {
        let parts = raw.split(separator: ",").map {
            $0.trimmingCharacters(in: .whitespaces)
        }
        guard parts.count == 4,
              let top = Int(parts[0]), let left = Int(parts[1]),
              let bottom = Int(parts[2]), let right = Int(parts[3])
        else { return nil }
