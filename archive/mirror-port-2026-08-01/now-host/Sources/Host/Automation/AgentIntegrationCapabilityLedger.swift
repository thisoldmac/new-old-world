...
final class AgentIntegrationCapabilityLedger {
    ...
    nonisolated static let familyPolicy: [FamilyPolicy] = {
        ...
        return [
            ...
            /* The same lane, the same trade, a third way: a scene holds the
               one transfer lane for the tens of milliseconds the walk
               takes, exactly as capture's whole-screen grab does, and
               probing it to settle a question nobody asked would take that
               lane out from under whoever is streaming or capturing.
               `GuestListener.requestScene` is not wrapped in `observing`
               either, so this reads `unproven` on every guest until a real
               call settles it — the same honest gap capture's own row
               states. */
            .init(names.sceneRequest, .notProbedCostly),
