...
/// The side-effect-free sibling of `process.shot`: names a rect
/// directly, in GLOBAL screen coordinates, instead of a process — so no
/// process is fronted and nothing is asked to redraw. This is the pixel-
/// island fallback's fetch (`SceneIslands.Capture`): it needs a window's
/// CURRENT interior without disturbing which process is frontmost.
/// Answers with the ordinary capture transfer (capture.begin/bulk/end),
/// exactly like `CaptureRequest` and `ProcessShot`.
...
struct CaptureBegin: Codable, Equatable, Sendable {
    ...
    var captureMs: Int?
    var encodeMs: Int?
    /// GLOBAL screen coordinate of this image's top-left pixel. Absent
    /// means (0, 0) — a sender that predates the field always captured
    /// the whole screen at the origin.
    var originX: Int?
    var originY: Int?
    /// Local-to-global pixel ratio for the same anchor. Absent means 1.
    var scale: Int?
}
...
enum ControlMessageCodec {
    ...
    static func decode(_ data: Data) throws -> ControlMessage {
        ...
        case "capture.region":
            return .captureRegion(
                try decoder.decode(CaptureRegion.self, from: data))
        case "file.list":
    ...
    static func encode(_ message: ControlMessage) throws -> Data {
        ...
        case .captureRegion(let m): return try tagged("capture.region", m)
