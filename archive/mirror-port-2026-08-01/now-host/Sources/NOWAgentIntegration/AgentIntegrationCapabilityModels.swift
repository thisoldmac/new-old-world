...
public enum AgentIntegrationCapabilityNames {
    ...
    /// Mirror's IR v1 walk of the connected machine — semantic structure,
    /// not pixels. A contract message name like `captureRequest` beside it,
    /// not an alias: `scene.request` is what the host sends and both the
    /// wire's decode side (`NOWSceneCodec`) and the guest's own producer
    /// name. There is no `scene.cancel` to require or fold in beside it —
    /// the contract states a scene transfer is short enough that
    /// cancelling it costs more than finishing it.
    public static let sceneRequest = "scene.request"
    ...
    /// plane's own verb rather than the act plane's four. Deliberately NOT
    /// in `actPlane` below, for the same reason `elementsCommand` is not:
    /// `MirrorActProjectionTests` holds the five act rows to properties
    /// about a shared dispatch vocabulary — one `Dispatch` row, one opaque
    /// reference as the identity check — and `key` answers neither. It
    /// answers the input plane's own `posted` row and is addressed by no
    /// reference at all, so folding it into that set would either fail
    /// those properties or force them to carve out the one row they were
    /// never written about.
    public static let keyCommand = "key"
    ...
    public static let all: Set<String> = [
        ...
        fileMkdir, censusRequest, captureRequest, sceneRequest, streamStart,
        streamStop, streamRefresh, launchCommand, revealCommand,
        catsearchCommand, gestaltCommand, tailCommand, vprobeCommand,
        shotdiagCommand, putstatCommand, windowActCommand, textGetCommand,
        textSetCommand, controlActCommand, menuActCommand, elementsCommand,
        keyCommand,
    ]
