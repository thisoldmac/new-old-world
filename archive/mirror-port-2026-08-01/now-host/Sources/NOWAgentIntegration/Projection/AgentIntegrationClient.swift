...
public protocol AgentIntegrationClient: Sendable {
    ...
    /// Post one keystroke into the connected guest's front application,
    /// through the input plane's `key` verb rather than the four-op act
    /// dispatch above. `posted` means the guest queued it; whether anything
    /// dequeued it is the caller's to verify against a fresh observation.
    /// See `KeyActProjection`.
    func key(_ request: AgentIntegrationKeyRequest) async
        -> AgentIntegrationKeyResult
    /// Ask the paired guest for one Mirror IR scene, and get back the first
    /// page of the result. Two calls rather than capture's three: a scene
    /// has no abandon — see `AgentIntegrationScenePolicy`'s header for why —
    /// so there is only "take" and "page", the same reason the paging is
    /// still needed at all: the document rarely fits the local 16 KiB cap.
    /// See `SceneProjection`.
...
extension AgentIntegrationClient {
    ...
    /* Defaulted with the capture trio, and in the same edit that declared
       it: a client that has no host to ask answers "no host" without every
       stub in the tree learning a new lane. */
    public func requestGuestScene(staleAfterMs: Int?) async
        -> AgentIntegrationSceneResult {
        .hostUnavailable
    }

    public func fetchGuestScenePage(sceneID: UUID, offset: Int) async
        -> AgentIntegrationSceneResult {
        .hostUnavailable
    }
    ...
    /// Defaulted with the act lane, and for the reason `key` is not IN the
    /// act plane's set: it is its own command with its own local-protocol
    /// operation, added the day this default was — a stub client with no
    /// host to ask answers "no act lane" without every conformer in the
    /// tree learning a sixth lane.
    public func key(_ request: AgentIntegrationKeyRequest) async
        -> AgentIntegrationKeyResult {
        .unavailable(.noActLane(AgentIntegrationCapabilityNames.keyCommand))
    }
