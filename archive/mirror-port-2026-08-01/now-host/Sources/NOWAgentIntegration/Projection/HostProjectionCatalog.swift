...
public enum HostProjectionCatalog {
    public static let projections: [any HostProjection.Type] = [
        ...
        /* Immediately after the listing it polls, for the reason
           WaitForProjection's header gives: it repeats
           ListProcessesProjection's own client.listProcesses() call rather
           than sending anything new, so it belongs directly beside the row
           whose traffic it reuses — before the walk that goes one level
           deeper into a single process, which is the next question a
           caller asks once whatever it was waiting for has appeared. */
        WaitForProjection.self,
        ...
        /* Immediately after the walk it filters: same guest traffic
           (`elements`), same consent tier, and the natural next question
           once a caller has seen — or already expects — roughly what a
           process contains. See FindElementsProjection's header for why it
           is a host-side narrowing of that tree rather than a second walk. */
        FindElementsProjection.self,
        ...
        /* Beside the two above rather than with the other observations: all
           three share the one bulk-transfer lane the guest enforces
           (GuestListener.transferLaneHolder), so a reader meeting one meets
           the others in the same breath. A scene is the third occupant of
           that lane, taken a third way — structure rather than pixels, and
           the only one of the three with no abandon (SceneProjection's
           header says why). */
        SceneProjection.self,
        ...
        /* Beside the five rather than folded into them: it is the input
           plane's own verb, not a sixth member of the four-op dispatch —
           see the header of KeyActProjection and MirrorActProjections'
           note on why now_observe_elements sits outside that group too.
           Kept adjacent because a caller reading the act-ish rows in order
           meets it where it belongs conceptually, even though no test
           holds it to that group's properties. */
        KeyActProjection.self,
