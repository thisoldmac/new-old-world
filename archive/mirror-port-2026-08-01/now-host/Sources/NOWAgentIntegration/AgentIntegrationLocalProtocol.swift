...
public struct AgentIntegrationLocalRequest: Codable, Equatable, Sendable {
    public enum Operation: String, Codable, Sendable {
        ...
        /// Post one keystroke into the guest's event queue, through the
        /// input plane's own `key` verb — its own operation and not a sixth
        /// member of the five above, because it shares none of their state:
        /// no reference, no `Dispatch` row, and (added 2026-08-01) a `mods`
        /// that may legally be nonzero. See `KeyActProjection`.
        case key = "key"

        /// Ask for one Mirror IR scene, or fetch one page of the one just
        /// taken. Two shapes rather than capture's three: a scene has no
        /// abandon (`AgentIntegrationScenePolicy`'s header says why), so a
        /// bare request always means "take", exactly the shape
        /// `guestLogTail` already uses for an all-optional operation. See
        /// `SceneProjection`.
        case scene = "scene"
    }
    ...
    /// The keystroke to post. Present only on `key`, and typed for the same
    /// reason `menuActRequest` is: `AgentIntegrationKeyRequest.isWellFormed`
    /// states the grammar once, including the modifier mask, rather than
    /// re-deriving it from loose scalars here.
    public var keyRequest: AgentIntegrationKeyRequest? = nil

    /* The scene lane's fields. Present only on `scene`; `sceneID` and
       `sceneOffset` travel together and only on a page fetch — either alone
       is a request nothing can serve, the same guard `captureID` and
       `captureOffset` keep. There is no `sceneAbandon`: see
       `AgentIntegrationScenePolicy`'s header for why capture's three-shape
       design does not apply here. */

    /// How old an anchor sample may be and still be reported clean. Present
    /// only on a TAKE, and optional there too — 0 or absent both mean "no
    /// age gate", the contract's own default.
    public var sceneStaleAfterMs: Int? = nil
    /// Which staged scene a continuation is reading, and from where.
    public var sceneID: UUID? = nil
    public var sceneOffset: Int? = nil
    ...
    /// Post one keystroke into the guest's event queue.
    public static func key(
        _ keyRequest: AgentIntegrationKeyRequest,
        requestID: UUID = UUID()
    ) -> Self {
        var request = projected(.key, requestID: requestID)
    ...
    /// Ask the addressed machine for one Mirror IR scene.
    public static func scene(
        staleAfterMs: Int? = nil,
        requestID: UUID = UUID()
    ) -> Self {
        var request = projected(.scene, requestID: requestID)
        request.sceneStaleAfterMs = staleAfterMs
        return request
    }

    /// Read one page of a scene already staged by this host.
    public static func scenePage(
        sceneID: UUID,
        offset: Int,
        requestID: UUID = UUID()
    ) -> Self {
...
public enum AgentIntegrationLocalResult: Equatable, Sendable {
    ...
    /// One posted keystroke, or the refusal for one that was not. Its own
    /// case for the reason the five above are five: it shares no state with
    /// any of them.
    case key(AgentIntegrationKeyResult)
    /// One staged scene, or one page of it.
    case scene(AgentIntegrationSceneResult)
...
public struct AgentIntegrationLocalResponse: Codable, Equatable, Sendable {
    ...
    /// One posted keystroke, or the refusal for one that was not.
    public var keyResult: AgentIntegrationKeyResult? = nil
    /// One staged scene, or one page of it.
    public var sceneResult: AgentIntegrationSceneResult? = nil
    ...
    public init(requestID: UUID,
                keyResult: AgentIntegrationKeyResult) {
        self.init(empty: requestID)
        self.keyResult = keyResult
    }

    public init(requestID: UUID,
                sceneResult: AgentIntegrationSceneResult) {
        self.init(empty: requestID)
...
public enum AgentIntegrationLocalCodec {
    ...
    static let projectedResultKeys: Set<String> = [
        ...
        // The input plane's key, in both gates from the day IT landed —
        // its own operation, not a sixth member of the five above.
        "keyResult",
        // The scene lane, in both gates from the day it landed.
        "sceneResult",
    ...
    public static func decodeRequest(_ data: Data) throws
        ...
        let object = try strictObject(data, allowedKeys: [
            ...
            "actElement", "actText",
            // The input plane's key, clearing the same two gates.
            "keyRequest",
            // The scene lane's fields, clearing the same two gates.
            "sceneStaleAfterMs", "sceneID", "sceneOffset",
        ...
        case .key:
            expectedKeys = [
                "version", "requestID", "operation", "keyRequest",
            ]
            guard let act = request.keyRequest, act.isWellFormed else {
                throw AgentIntegrationLocalTransportError.invalidMessage(
                    "Key request does not name a key by name, code or "
                        + "char, or names a mods value outside the "
                        + "modifier mask")
            }
        case .scene:
            /* Two shapes, unlike capture's three — there is no abandon (see
               AgentIntegrationScenePolicy's header). A page names both its
               stage and its offset, exactly as capture's does; everything
               else about a bare request means "take one", including an
               absent staleAfterMs, which is the contract's own default. */
            var sceneKeys: Set<String> = [
                "version", "requestID", "operation",
            ]
            let pages = request.sceneID != nil || request.sceneOffset != nil
            guard request.launchSelection == nil,
                  request.processReference == nil,
                  request.approvalReceipt == nil,
                  request.guestFilePath == nil,
                  ...
                  request.auditEvent == nil else {
                throw AgentIntegrationLocalTransportError.invalidMessage(
                    "Scene request does not match the schema")
            }
            if pages {
                guard request.sceneID != nil,
                      let offset = request.sceneOffset,
                      offset >= 0,
                      offset <= AgentIntegrationScenePolicy.maximumBytes,
                      offset % AgentIntegrationScenePolicy.pageBytes == 0
                else {
                    throw AgentIntegrationLocalTransportError.invalidMessage(
                        "Scene page request does not match the schema")
                }
                sceneKeys.formUnion(["sceneID", "sceneOffset"])
            } else if let staleAfterMs = request.sceneStaleAfterMs {
                guard staleAfterMs >= 0 else {
                    throw AgentIntegrationLocalTransportError.invalidMessage(
                        "Scene staleAfterMs is negative")
                }
                sceneKeys.insert("sceneStaleAfterMs")
            }
            expectedKeys = sceneKeys
        }
