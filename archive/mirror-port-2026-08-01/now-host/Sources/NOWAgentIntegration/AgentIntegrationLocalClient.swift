...
public struct AgentIntegrationLocalClient: Sendable {
    ...
    private func send(operation: AgentIntegrationLocalRequest.Operation)
        ...
        case .key:
            preconditionFailure("A key request names the key to post")
        case .scene:
            preconditionFailure(
                "A scene request says which of its two shapes it is")
        }
    }
    ...
    public func key(_ request: AgentIntegrationKeyRequest) async throws
        -> AgentIntegrationKeyResult {
        let response = try await send(.key(request))
        guard let result = response.keyResult else {
            throw AgentIntegrationLocalTransportError.invalidMessage(
                "Local response had no key result")
        }
        return result
    }
    ...
    public func requestScene(staleAfterMs: Int?) async throws
        -> AgentIntegrationSceneResult {
        try await sceneResult(of: .scene(staleAfterMs: staleAfterMs))
    }

    public func fetchScenePage(sceneID: UUID, offset: Int) async throws
        -> AgentIntegrationSceneResult {
        try await sceneResult(
            of: .scenePage(sceneID: sceneID, offset: offset))
    }

    private func sceneResult(of request: AgentIntegrationLocalRequest)
        async throws -> AgentIntegrationSceneResult {
        let response = try await send(request)
        guard let result = response.sceneResult else {
            throw AgentIntegrationLocalTransportError.invalidMessage(
                "Local response had no scene result")
        }
        return result
    }
    ...
    private func send(_ request: AgentIntegrationLocalRequest) async throws
        ...
        return try await Task.detached {
            ...
            case .windowAct, .controlAct, .menuAct, .textGet, .textSet, .key:
                /* THE ACT LANE gets the launch window, not the read-only
                   ...
                   than a broken socket. `key` shares it: it waits on
                   `AgentIntegrationActControl.run`, the same 15 s bound,
                   even though it answers no `Dispatch` row. */
                timeout = launchReceiveTimeout
            ...
            case .stream:
                ...
                timeout = captureReceiveTimeout
            case .scene:
                /* Only TAKING one waits on a machine, for the same reason
                   capture's own split does — a page fetch reaches no guest
                   at all. The guest's scene watchdog is 20 s, the same as
                   capture's, so this shares that window rather than the
                   read-only one. */
                timeout = request.sceneID == nil
                    ? captureReceiveTimeout : readOnlyReceiveTimeout
