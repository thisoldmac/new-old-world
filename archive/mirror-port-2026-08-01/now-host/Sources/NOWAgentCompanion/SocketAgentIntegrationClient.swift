...
struct SocketAgentIntegrationClient: AgentIntegrationClient {
    ...
    /// The input plane's `key`, forwarded the same way the five acts above
    /// are — nothing here decides what a keystroke may claim.
    func key(_ request: AgentIntegrationKeyRequest) async
        ...
        do {
            return try await client.key(request)
        } catch {
            return .unavailable(unavailable(for: error))
        }
    }
    ...
    func requestGuestScene(staleAfterMs: Int?) async
        ...
        do {
            return try await client.requestScene(staleAfterMs: staleAfterMs)
        } catch {
            return .unavailable(unavailable(for: error))
        }
    }
