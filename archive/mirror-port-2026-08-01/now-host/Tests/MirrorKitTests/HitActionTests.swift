...
final class HitActionTests: XCTestCase {
    ...
    /// RETRACTED 2026-08-01, lane L7: a MODIFIED keystroke used to have no
    /// home in NOW's contract, on the CarbonLib-has-no-PPostEvent fact. It
    /// does now — the guest's `key` verb routes a nonzero `mods` through the
    /// act plane's own key op, served by the NOW Extension's resident
    /// filter — so this asserts the new answer rather than the old one, and
    /// says where to look if a future change narrows it back.
    func testAModifiedKeystrokeIsAvailable() {
        XCTAssertEqual(
            ActionModel.availability(
                .key(name: nil, code: 0, char: 97, mods: 256)),
            .available(command: "key"),
            "a mods:256 key is available — the guest's own act plane "
                + "decides whether the route is armed, not this function")
    }

    /// A bare `.type` names no target at all — `typeInto` against a
    /// referenced control is the reachable route (see its own comment) —
    /// and that is unrelated to `key`'s `mods` question above.
    func testABareTypeWithNoTargetIsUnavailable() {
