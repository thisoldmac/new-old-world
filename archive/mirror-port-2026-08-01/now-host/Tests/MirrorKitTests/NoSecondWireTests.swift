...
final class NoSecondWireTests: XCTestCase {
    ...
    /// can address — and today that is two, plus `key`, WIDENED 2026-08-01
    /// (lane L7) for a documented, deliberate reason below.
    ///
    ...
    func testOnlyTheActsAScenecanAddressAreAvailable() {
        ...
        XCTAssertEqual(available,
            ["menuact", "activate", "script", "winact", "key"],
            """
            ...
            `key` is the deliberate widening: it names no element reference \
            at all (a keystroke addresses whatever has focus, the way it \
            always has) and used to be `.available` only for `mods == 0`. \
            It answers `.available` for EVERY `mods` now — including the \
            256 this suite's own `everyAction` entry carries — because the \
            guest's `key` verb gained a route for a nonzero one (the act \
            plane's key op, served by the NOW Extension's resident filter) \
            and whether that route is ARMED is a fact the guest's own reply \
            decides, not a fact this function can see in advance. See \
            `ActionModel.availability(.key)`'s own comment.

            Two of the rest name a command NOW declares and this HOST has \
            no lane for (`script`, and `activate`). That is deliberately not \
