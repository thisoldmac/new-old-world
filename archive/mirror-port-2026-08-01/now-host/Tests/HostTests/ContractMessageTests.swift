...
final class ContractMessageTests: XCTestCase {
    ...
    /// A CaptureBegin whose sender predates originX/originY/scale decodes
    /// with the fields absent — nil, not defaulted here. Session.swift is
    /// where absence becomes (0, 0) at scale 1; the wire type itself must
    /// not invent a value a sender never sent.
    func testCaptureBeginWithoutOriginFieldsDecodes() throws {
        let json = #"""
        {"type":"capture.begin","id":1,"transfer":5,"width":640, \#
        "height":480,"depth":8,"rowBytes":640,"bytes":307200}
        """#
        guard case .captureBegin(let begin) =
            try ControlMessageCodec.decode(Data(json.utf8)) else {
            return XCTFail("expected capture.begin")
        }
        XCTAssertNil(begin.originX)
        XCTAssertNil(begin.originY)
        XCTAssertNil(begin.scale)
    }

    /// A CaptureBegin carrying originX/originY/scale — a region or
    /// process.shot capture, or a future decimated one — round-trips
    /// them exactly.
    func testCaptureBeginCarriesOriginAndScale() throws {
        let begin = CaptureBegin(id: 1, transfer: 5, width: 100, height: 80,
                                 depth: 8, rowBytes: 100, bytes: 8000,
                                 originX: 240, originY: 60, scale: 1)
        XCTAssertEqual(
            try ControlMessageCodec.decode(
                ControlMessageCodec.encode(.captureBegin(begin))),
            .captureBegin(begin))
    }

    /// capture.region: the side-effect-free rect capture the pixel-island
    /// fallback fetches through. Round-trips like process.shot, minus the
    /// PSN.
    func testCaptureRegionRoundTrip() throws {
        let request = CaptureRegion(id: 30, left: 40, top: 60, right: 340,
                                    bottom: 460, depth: 8)
        XCTAssertEqual(
            try ControlMessageCodec.decode(
                ControlMessageCodec.encode(.captureRegion(request))),
            .captureRegion(request))
    }

    /// depth is optional on capture.region — the guest's own preference
    /// applies, exactly like a depth-less process.shot or capture.request
    /// already does.
    func testCaptureRegionWithoutDepthDecodes() throws {
        let json = #"{"type":"capture.region","id":31,"left":0,"top":0,"right":200,"bottom":150}"#
        guard case .captureRegion(let region) =
            try ControlMessageCodec.decode(Data(json.utf8)) else {
            return XCTFail("expected capture.region")
        }
        XCTAssertNil(region.depth)
        XCTAssertEqual(region.right, 200)
        XCTAssertEqual(region.bottom, 150)
    }
