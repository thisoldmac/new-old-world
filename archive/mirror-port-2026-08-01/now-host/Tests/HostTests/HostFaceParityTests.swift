...
final class HostFaceParityTests: XCTestCase {
    ...
    private static let appUIDivergences: Set<String> = [
        ...
        /* Registered 2026-08-01. A person watching the Processes page sees
           a state change as it happens and has no use for a poll loop that
           blocks waiting for one; that gap belongs to an agent caller, not
           to a pane. */
        "now_wait_for",
        ...
        /* Registered 2026-08-01, beside now_observe_elements and for the
           identical reason: its matches are the same opaque references to
           windows, controls and text elements, and the affordance lands
           with the scene view those rows are all waiting for. */
        "now_find_elements",
        "now_window_act",
        "now_control_act",
        "now_menu_act",
        "now_text_get",
        "now_text_set",
        /* Registered 2026-08-01, the MCP face of the guest's key verb. The
           app UI already sends a plain keystroke from a person's own
           keyboard (MirrorKeyCaptureView, ActionModel.paneKeystroke); what
           has no affordance is composing an arbitrary key BY NAME, CODE OR
           CHAR the way this row's caller can — a keyboard already does
           that for the person at the machine. */
        "now_key_act",
    ]
