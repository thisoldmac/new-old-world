...
final class SceneWireTests: XCTestCase {
    ...
    // MARK: - The desktop plane, on a scene the guest's own encoder wrote

    /// Emitted the same way `guestScene` above was: `scene_build.c` +
    /// `scene_json.c` compiled natively and run over a folder item, a typed
    /// file, and a mounted volume - plus a fourth, invisible item that never
    /// became a row at all (`now_scene_add_desktop_item` refuses one before
    /// assembly, independently of the walk that offered it).
    private static let guestSceneWithDesktopItems = """
        {"version":1,"seq":11,"capturedAt":1750000500.0,"source":"peek",\
        "screen":{"w":640,"h":480},"apps":[],"processes":[],"windows":[],\
        "desktopItems":[{"name":"Lab","kind":"folder","x":20,"y":40,\
        "placed":true,"alias":false,"invisible":false},\
        {"name":"ReadMe","kind":"file","type":"TEXT","creator":"ttxt",\
        "x":90,"y":40,"placed":true,"alias":false,"invisible":false},\
        {"name":"Macintosh HD","kind":"disk","x":0,"y":0,"placed":false,\
        "alias":false,"invisible":false}],"meta":{"errors":[]}}
        """

    func testTheDesktopPlaneDecodesWithThreeKindsAndNoInvisibleRow() throws {
        let scene = try NOWSceneCodec.decode(
            irVersion: 1, document: Data(Self.guestSceneWithDesktopItems.utf8))

        let items = try XCTUnwrap(scene.desktopItems)
        XCTAssertEqual(items.count, 4 - 1, "the invisible item is not a "
                       + "fourth row, dropped")

        let folder = try XCTUnwrap(items.first { $0.kind == "folder" })
        XCTAssertEqual(folder.name, "Lab")
        XCTAssertEqual(folder.x, 20)
        XCTAssertEqual(folder.y, 40)
        XCTAssertEqual(folder.placed, true)
        XCTAssertNil(folder.type, "a folder carries no Finder file type")

        let file = try XCTUnwrap(items.first { $0.kind == "file" })
        XCTAssertEqual(file.name, "ReadMe")
        XCTAssertEqual(file.type, "TEXT")
        XCTAssertEqual(file.creator, "ttxt")

        let disk = try XCTUnwrap(items.first { $0.kind == "disk" })
        XCTAssertEqual(disk.name, "Macintosh HD")
        XCTAssertEqual(disk.placed, false, "a volume is never placed by "
                       + "this producer - the host lays it out")
        XCTAssertNil(disk.type, "a volume carries no Finder file type either")
    }

    /// A scene that never touched the desktop plane - `guestScene` above -
    /// still decodes `desktopItems` to `nil`, not `[]`: the plane is
    /// scene-wide conditional, not scene-wide guaranteed.
    func testANonDesktopSceneStillLeavesDesktopItemsAbsent() throws {
        let scene = try decodeGuestScene()
