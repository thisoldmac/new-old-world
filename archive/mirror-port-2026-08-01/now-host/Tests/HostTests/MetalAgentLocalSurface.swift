...
final class MetalAgentLocalSurface {
    ...
    private func handle(_ request: AgentIntegrationLocalRequest) async
        ...
        case .scene:
            /* Served for real, like capture beside it: a scene is the
               third occupant of the one bulk-transfer lane and is exactly
               as read-only as a screenshot — see SceneProjection's header
               for why it has no abandon to refuse here either. */
            if let sceneID = request.sceneID,
               let offset = request.sceneOffset {
                return record(.scene(adapter.scenePage(
        ...
        case .launchSoftware, .requestQuit, .transferApprovedArtifact,
             ...
             /* And the act lane, which belongs in this list more plainly
                ...
                answered. `key` joins them for the same reason: it posts
                into the front application's own event queue on the
                person's own PowerBook, which this rig has no more business
                doing than closing their window. */
             .windowAct, .controlAct, .menuAct, .textGet, .textSet, .key:
    ...
    private func record(_ result: AgentIntegrationLocalResult)
        ...
        case .scene(let scene):
            response = .init(requestID: UUID(), sceneResult: scene)
...
private struct MetalLocalProjectionClient: AgentIntegrationClient {
    ...
    func requestGuestScene(staleAfterMs: Int?) async
        ...
        do {
            return try await client.requestScene(
    ...
    func fetchGuestScenePage(sceneID: UUID, offset: Int) async
        -> AgentIntegrationSceneResult {
        do {
            return try await client.fetchScenePage(
                sceneID: sceneID, offset: offset)
        } catch {
            return .unavailable(Self.unavailable(for: error))
        }
