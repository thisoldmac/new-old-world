...
final class MirrorContentJoinTests: XCTestCase {
    ...
    /// means nothing is armed - and this host now arms it, automatically,
    /// by sending `qdtrace start front:true` (see the arm-front tests below).
    func testUnarmedPlaneIsAttributedToTheArmGap() throws {
        ...
        XCTAssertTrue(MirrorContentJoin.armGap.contains(
            "arms the front window automatically"))
    }
    ...
    // MARK: - arming the front window automatically
    //
    // `armFrontWindow` itself is untestable without a live guest (it needs
    // `attribute` to see `armGap` first, which needs a connected listener's
    // `drain` to answer `ok: true` and empty - `join`'s tests below all hit
    // "not-connected" before reaching it). `armAttemptOutcome` is the
    // decision split out for exactly that reason, the same as `apply`.

    /// A successful arm is reported as "just armed", never as `.attached` -
    /// `qdtrace start` answers before anything has drawn, by its own claim
    /// discipline (`requested: true, armed: false`).
    func testSuccessfulArmIsReportedAsJustArmedNotAttached() throws {
        let start = try result(
            "{\"type\":\"command.result\",\"id\":6,\"ok\":true,"
            + "\"output\":{\"qdtrace\":{\"cmd\":\"start\",\"a5\":"
            + "\"0x0032a000\",\"resolvedVia\":\"front\",\"mode\":\"record\","
            + "\"expiry\":3600,\"now\":120,\"requested\":true,"
            + "\"armed\":false}}}")
        let outcome = MirrorContentJoin.armAttemptOutcome(start)
        guard case .empty(let why) = outcome else {
            return XCTFail("expected empty, got \(outcome)")
        }
        XCTAssertTrue(why.contains("just armed"), why)
        XCTAssertTrue(why.contains("Press again"), why)
    }

    /// The guest's own refusal to arm - no front process, an anchor the
    /// oracle will not trust for arming, and the rest `resolve_start_target`
    /// names - forwarded verbatim, never rewritten into a happier sentence.
    func testArmRefusalIsForwardedInItsOwnWords() throws {
        let refusal = try result(
            "{\"type\":\"command.result\",\"id\":6,\"ok\":false,"
            + "\"error\":{\"code\":\"no-front\",\"message\":"
            + "\"there is no front process to arm\"}}")
        let outcome = MirrorContentJoin.armAttemptOutcome(refusal)
        guard case .refused(let why) = outcome else {
            return XCTFail("expected refused, got \(outcome)")
        }
        XCTAssertTrue(why.contains("no-front"), why)
        XCTAssertTrue(why.contains("there is no front process to arm"), why)
    }

    /// An `ok: true` reply whose body is not a `start` reply is a refusal -
    /// the same discipline `apply` uses when a status reply lands where a
    /// drain was expected.
    func testArmReplyThatIsNotAStartIsRefused() throws {
        let status = try result(
            "{\"type\":\"command.result\",\"id\":6,\"ok\":true,"
            + "\"output\":{\"qdtrace\":{\"cmd\":\"status\",\"plane\":"
            + "{\"format\":1,\"length\":65676,\"ringCap\":65536}}}}")
        let outcome = MirrorContentJoin.armAttemptOutcome(status)
        guard case .refused = outcome else {
            return XCTFail("expected refused, got \(outcome)")
        }
    }
