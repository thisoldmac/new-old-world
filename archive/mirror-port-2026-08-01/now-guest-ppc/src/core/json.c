                          ...
                          char *out, long cap)
{
    ...
    out[0] = '\0';
    v = now_json_value(json, key);
    /* The `*v != '{'` check matters even though now_json_next_object
       would eventually find SOME brace: without it, an args value that
       is not an object (or a request with no args at all, where `v` is
       NULL) would let the scan run forward into whatever JSON follows -
       reading a sibling's object rather than reporting absence. */
    if (v == NULL || *v != '{') {
        return 0;
    }
    return now_json_next_object(v, out, cap) != NULL;
}
