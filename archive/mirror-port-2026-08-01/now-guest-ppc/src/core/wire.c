...
typedef struct {
    ...
    /* GLOBAL screen coordinate this image was captured from — copied off
       CaptureImage, which is the one place capture_screen_rect's clamp
       leaves it. capture.begin carries it as originX/originY so a pixel
       island can map back to guest coordinates. */
                       ...
                       unsigned long t_start, PixelBlob *blob, ShotMeta *meta)
{
    ...
    meta->origin_x = image->origin_x;
    meta->origin_y = image->origin_y;
                        ...
                        Boolean pushed)
{
    ...
    snprintf(json, sizeof json,
             ...
             meta->capture_ms, owned.encode_ms,
             (int)meta->origin_x, (int)meta->origin_y, frame_fields);
    if (!send_control(json)
...
/* capture.region: the side-effect-free sibling of process.shot. No PSN,
   no SetFrontProcess, no repaint wait — it names a GLOBAL rect directly
   and captures it now, one blit, no deferral. This is the pixel-island
   fallback's fetch: it needs a window's CURRENT interior without
   disturbing which process is frontmost (docs/mirror-knowledge.md,
   "Lane A-fallback"). Answers with the ordinary capture transfer
   (capture.begin/bulk/end), or capture.end ok:false, correlated by id
   exactly like capture.request. */
static void serve_capture_region(const char *request)
{
    NowPrefs prefs;
    PixelBlob blob;
    ShotMeta meta;
    CaptureRegionArgs args;
    Rect rect;
    char reason[160];
    long id = now_json_find_int(request, "id", 0);
    long left = now_json_find_int(request, "left", 0);
    long top = now_json_find_int(request, "top", 0);
    long right = now_json_find_int(request, "right", 0);
    long bottom = now_json_find_int(request, "bottom", 0);
    long depth_arg = now_json_find_int(request, "depth", 0);
    unsigned short xfer;
    long chunk;
    short pace_ms;
    ...
    if (g_stream.active || g_xfer.active || g_shot.active) {
        now_log(kLogWarn, "cap", "#%ld region refused: a transfer is in flight",
                id);
        capture_fail(id);             /* one transfer at a time */
        return;
    }
    if (!now_capture_region_parse(left, top, right, bottom, depth_arg,
                                  &args, reason, sizeof reason)) {
        now_log(kLogWarn, "cap", "#%ld region refused: %s", id, reason);
        capture_fail(id);
        return;
    }
    now_prefs_load(&prefs);
    SetRect(&rect, (short)args.left, (short)args.top,
            (short)args.right, (short)args.bottom);
    tuning_from_json(request, &prefs, &chunk, &pace_ms, &pack);
    xfer = next_xfer();

    memset(&blob, 0, sizeof blob);
    if (!gather_shot_rect(args.depth ? args.depth : prefs.shot_depth,
                          &rect, pack, &blob, &meta)) {
        char json[128];

        now_log(kLogWarn, "cap", "#%ld region capture failed", id);
        snprintf(json, sizeof json,
                 "{\"type\":\"capture.end\",\"id\":%ld,\"transfer\":%u,"
                 "\"ok\":false}", id, xfer);
        send_control(json);
...
static int handle_frame(const char *reply)
{
    ...
    if (now_json_type_is(reply, "capture.region")) {
        serve_capture_region(reply);
        return 1;
    }
