                   ...
                   Boolean *more, short *next_start)
{
    ...
    return now_files_list_folder(spec.vRefNum, dir_id, start, out, max,
                                 more, next_start);
}
    ...
    *next_start = index;
    ...
    while (count < max) {
        ...
        if ((pb.hFileInfo.ioFlAttrib & ioDirMask) != 0) {
            ...
            /* A folder's Finder info is DInfo, at ioDrUsrWds - a
               different field of the SAME union member the branch above
               already reads for a file, not a second call. */
            e->loc_h = pb.dirInfo.ioDrUsrWds.frLocation.h;
            e->loc_v = pb.dirInfo.ioDrUsrWds.frLocation.v;
            e->alias = (pb.dirInfo.ioDrUsrWds.frFlags & kFilesFdIsAlias) != 0;
            e->invisible =
                (pb.dirInfo.ioDrUsrWds.frFlags & kFilesFdInvisible) != 0;
        } else {
            ...
            e->loc_h = pb.hFileInfo.ioFlFndrInfo.fdLocation.h;
            e->loc_v = pb.hFileInfo.ioFlFndrInfo.fdLocation.v;
            e->alias =
                (pb.hFileInfo.ioFlFndrInfo.fdFlags & kFilesFdIsAlias) != 0;
            e->invisible =
                (pb.hFileInfo.ioFlFndrInfo.fdFlags & kFilesFdInvisible) != 0;
        }
