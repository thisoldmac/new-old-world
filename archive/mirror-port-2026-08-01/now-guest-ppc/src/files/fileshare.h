...
enum {
    ...
    kFilesNotAFolder = -3,
    kFilesIOError = -4,
    kFilesTooBig = -5,                /* insufficient disk/staging/work buffer */
    kFilesExists = -6                 /* would overwrite; caller must ask */
};

/* The two Finder flag bits this file reads out of the SAME word every
   caller already fetches (fdFlags for a file, frFlags for a folder via
   DInfo) - no new field, no new call, just two more bits of a word
   already in hand. */
enum {
    kFilesFdIsAlias = 0x8000,
    kFilesFdInvisible = 0x4000
};
...
typedef struct {
    ...
    /* The Finder's saved icon position and two of its flags - fdLocation
       (a file) or DInfo's frLocation (a folder), and fdIsAlias /
       fdInvisible out of the same flags word. Read for free alongside
       fdType/fdCreator above; {0,0} means the Finder auto-arranges this
       item rather than a real saved position (scene_desktop.c is the
       first caller that cares, but the read costs nothing extra here). */
    short loc_h, loc_v;
    Boolean alias, invisible;
} FileEntry;
...
/* The same catalog walk, addressed by volume and directory directly
   rather than by a path relative to the configured share. `now_files_list`
   is now a thin wrapper over this: the Desktop Folder (scene_desktop.c)
   is not inside anyone's share, so it needs the same read on a starting
   point of its own rather than a second copy of the loop. Never negative -
   a bad vref/dir just ends the page at 0, the way an emptied folder
   would. */
int now_files_list_folder(short vref, long dir_id, short start,
                          FileEntry *out, int max,
                          Boolean *more, short *next_start);
