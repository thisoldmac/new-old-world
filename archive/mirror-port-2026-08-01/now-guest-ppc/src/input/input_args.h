...
/* ---- key -------------------------------------------------------------
   ...
   So a `mods` argument used to be REFUSED here outright, and the
   defect upstream paid for and wrote down is why: an act that silently
   lost a command modifier typed a literal character into a document and
   answered ok. That is still the floor - a keystroke is never posted
   unmodified when a modifier was asked for - but it is no longer the
   whole story.

   NOW-2026-08-01: THE WALL NARROWED, IT DID NOT FALL. `ext/src/
   now_ext_act.c` is 68K resident, not Carbon, and it already proves this
   exact mechanism for a mouse press (act_post_click); `kNowPeekActOpKey`
   is the same proof for a key. So a `mods` request now ROUTES through
   the act plane's key op when one is available and reachable, and is
   refused only when it is not - absent, stale, dark, or the target
   process is not pumping. `mods_available` below is that decision,
   carried in rather than asked here: this file stays Toolbox-free, so it
   cannot itself query the plane, and the caller (now_input_run_key) is
   what actually knows.

   `mods: 0` is accepted the same way it always was. A caller that says
   "no modifiers" is asking for what the plain PostEvent path already
   does, and never needs the act plane at all.
...
enum {
    ...
    kNowKeyNameMax = 16,
    /* The five modifier bits the Event Manager's queue element carries
       (Events.h: cmdKey/shiftKey/alphaLock/optionKey/controlKey - the
       same bit positions PPostEvent's caller stamps into evtQModifiers).
       activeFlag and btnState live in the same word but name nothing a
       keystroke's modifiers mean, so they are not part of this mask -
       a caller that sent one sent an argument this verb does not have a
       reading for, and range-checking it is what says so. */
    kNowKeyModCmd = 1 << 8,
    kNowKeyModShift = 1 << 9,
    kNowKeyModAlphaLock = 1 << 10,
    kNowKeyModOption = 1 << 11,
    kNowKeyModControl = 1 << 12,
    kNowKeyModMask = kNowKeyModCmd | kNowKeyModShift | kNowKeyModAlphaLock
                     | kNowKeyModOption | kNowKeyModControl
};
...
typedef enum {
    ...
    kNowKeyCodeRange,
    kNowKeyCharRange,
    kNowKeyModRange,       /* mods is not 0 and not within kNowKeyModMask */
    kNowKeyModifiers       /* mods != 0 and no route can carry it right now */
} NowKeyStatus;
...
/* One resolved keystroke. `code_known` / `char_known` are 0 when this
   ...
   able to see why. `mods` is 0 unless the caller asked for one AND
   `mods_available` said the act plane could carry it - never partially
   applied, because a request this verb could not fully honour must not
   silently become the unmodified one. */
typedef struct {
    int code;
    int ch;
    int code_known;
...
/* The whole gate one key request passes before anything is queued.
   ...
   likewise. `mods_available` is NOT an argument the caller sent - it is
   whether the ACT PLANE can carry a modifier right now, decided by
   now_input_run_key (Toolbox: now_act_ready()) and passed in because this
   file has no way to ask on its own. Ignored when mods_present is 0 or
   mods is 0, which is every console call site.

   Checked in this order deliberately: the modifier range first, then
   whether a nonzero modifier has anywhere to go, so a caller that sent a
   modifier this verb has never heard of is told THAT rather than being
   sent to `mods_available`'s more expensive answer for no reason. */
...
enum { kNowKeyArgsScopeMax = 160 };   /* room for name+code+char+mods */

/* Extracts this verb's four arguments off the WIRE FRAME - the whole
   command.request now_command_run hands every verb, not just its `args`
   object.

   THE DEFECT THIS REPLACES. `request_json` is the whole envelope:
   {"type":"command.request","id":7,"name":"key","args":{"name":
   "return"}}. now_json_find_string's scan is flat and
   first-occurrence-wins, so asking it for "name" against the WHOLE frame
   matched the envelope's OWN "name" - "key", this command's name -
   before ever reaching args.name, and every call refused with
   kNowKeyUnknownName ("that is not a key this verb names"). Measured
   0/10 on the emulator, plain and modified keys alike, because the bug
   is in the lookup and does not care what name was sent.
   now_json_scope_object (core/json.h) isolates the args object first, so
   this function's own now_json_find_string("name",...) can only ever see
   the CALLER's key, never the envelope's. code/char/mods are scoped the
   same way even though none of the three currently collides with an
   envelope key - the collision class is "this verb's argument name
   equals a future envelope key", not "equals name specifically", and
   scoping once here closes it for all four rather than one.

   Every pointer is required and every output is written
   unconditionally, `*_present_out` reporting whether the CALLER sent
   that argument literally (the two-probe trick input_cmds.c's own
   arg_int uses, scoped to the args copy instead of the frame). This
   function does not call now_key_check itself: mods_available is a
   Toolbox fact (the act plane's own readiness) this Toolbox-free file
   has no way to ask for, so the caller (now_input_run_key) still decides
   it and still makes that call. */
...
/* The console grammar, which is the same decision from a typed line.
   ...
   ctrl) answers kNowKeyModifiers rather than "unknown key name" - the
   console grammar has no route to the act plane's key op (it names no
   process to arm; a typed line has no serialHi/serialLo), so this face
   of the verb still refuses every modifier outright. A person who types
   `key cmd n` is told there is no route FROM THE CONSOLE, which is a
   narrower and more honest claim than "this verb cannot", now that the
   wire face sometimes can. */
