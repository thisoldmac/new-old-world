...
static const char *const d_menugeom[] = {
    "  One menu's per-item rects, read from its own MDEF rather than",
    "  assumed at a fixed row height. No click, no arm - served",
    "  outright, the same shape as textget.",
...
static const char *const d_actstate[] = {
    "  The plane's own instruments, as numbers: how often each of the",
    "  six patched traps was ENTERED, how often the entry was ours,",
    "  what is armed, and whether a click would go through the pump or",
    "  by V3's inline route.",
    "  It exists because every measurement this plane has produced so",
    "  far had to be parsed out of a failure sentence, and a sentence",
    "  carries only what its author thought to put in it.",
    "  It arms nothing and takes no arguments. Read it before and",
    "  after an act; the difference is the measurement.",
    NULL
};
...
static const char *const d_key[] = {
    ...
    "  mods 0 (or omitted) never touches anything but this Mac's own",
    "  PostEvent. A nonzero mods now routes through the NOW",
    "  Extension's act plane - reachable only from that 68K resident",
    "  context, not from this Carbon application - and is refused,",
    "  naming why, only when that plane is absent, stale, dark, or no",
    "  process is pumping to serve it.",
    NULL
};
...
const NowCommandDoc kNowCommandDocs[] = {
    ...
    { "menugeom", 1, "one menu's per-item geometry, from its own MDEF",
      "menugeom <menu> [serialHi serialLo]", d_menugeom },
    ...
    { "actstate", 1, "read the act plane's own counters",
      "actstate", d_actstate },
