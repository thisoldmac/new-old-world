                     ...
                     char *out, long cap)
{
    ...
    /* menugeom (V2): served outright by the same extension, no click
       queued - see act_cmds.h. */
    if (strcmp(name, "menugeom") == 0) {
        now_act_run_menugeom(request_json, id, out, cap);
        return;
    }
    ...
    if (strcmp(name, "actstate") == 0) {
        now_mach_run_actstate(request_json, id, out, cap);
        return;
    }
