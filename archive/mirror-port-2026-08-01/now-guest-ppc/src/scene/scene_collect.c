                       ...
                       unsigned long stale_after_ticks)
{
    ...
    /* The desktop plane: the guest's own File Manager, not a foreign-
       memory read, so it needs no anchor and runs unconditionally -
       after the process/window walk rather than before, so a scene that
       runs short on time still has its windows. */
    now_scene_collect_desktop(out);
