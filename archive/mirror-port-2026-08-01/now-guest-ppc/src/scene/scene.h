...
/* The scene envelope: NOW's guest producing Mirror's frozen v1 scene IR
   ...
   `desktopItems[]` joined the produced set on 2026-08-01: it is not a
   foreign-memory read at all, unlike the four above - it is the guest's
   OWN File Manager, reading a Finder Info word every catalog walk in
   this tree already fetches (fileshare.c's `now_files_list_folder`) and
   an indexed `PBHGetVInfo` walk for mounted volumes
   (census_probes.c's `gather_volumes` is the proven pattern). Neither
   needs a foreign process, an anchor, or a verdict, which is why it is
   not listed beside the four conditional planes below - it has no
   process to be conditional ON.
   ...
   THE DESKTOP PLANE, added 2026-08-01, is conditional for a different
   reason than the five above - none of it is a foreign-memory read, so
   nothing here is gated on an anchor. `desktopItems[]` carries two
   independent walks. The Desktop Folder's own entries arrive with the
   Finder's SAVED `fdLocation` (mirror/docs/FOLDER-ITEMS.md: that value
   is the wrong quantity the moment a window scrolls, but the desktop is
   the one surface with no scroll frame to disagree with it, so the
   saved position IS the live one there). Mounted volumes are not
   Desktop Folder entries - `PBHGetVInfo` finds them, but not where the
   Finder chose to draw them, since that lives in the Finder's own
   desktop database and nowhere the File Manager can read - so a volume
   is reported `placed:false`, ALWAYS, and a host-side default (top
   right, stacked down) lays it out; a real position, should this
   producer ever have one, would win outright rather than needing this
   comment changed. `desktopItems[]` is present once EITHER walk ran,
   even when the other found nothing or could not resolve at all
   (an unmounted Desktop Folder on a machine with no volumes is not a
   configuration this producer can actually reach); it is absent only
   when neither ran. An invisible item (`fdInvisible`) is refused by
   assembly itself, the same independent-refusal shape
   `now_scene_add_window` already uses for a disallowed anchor - the
   Finder does not draw one, and neither does this scene.

   EVERYTHING ELSE IS AN ABSENT KEY, NOT AN EMPTY ONE.
   `windows[].display[]`, `windows[].items[]`,
...
enum {
    ...
    /* Desktop icons plus mounted volumes, one flat pool - there is no
       per-owner block here the way controls belong to a window, so
       (unlike kNowSceneMaxControls) nothing needs a tail-of-pool check.
       64 is a real desktop's worth (mirror/f46861e measured 16 Desktop
       Folder items on a live 9.1 machine) with room for several mounted
       volumes beside them. */
    kNowSceneMaxDesktopItems = 64,
    /* "folder" / "file" / "disk", the three strings this producer
       writes - "disk" LITERALLY, not "volume": a renderer that already
       knows the "disk" glyph fell through to a generic document icon
       the one time this was spelled differently (mirror/3b76f79). */
    kNowSceneDesktopKindMax = 8
};
...
/* One desktop icon or mounted volume, in GLOBAL desktop coordinates.
   `kind` is "folder", "file" or "disk" - "disk" literally, see
   kNowSceneDesktopKindMax.

   `has_type` is the presence bit for `file_type`/`creator`: a folder and
   a volume have neither concept in this reading (only a FILE's FInfo
   carries a type/creator), so the JSON key stays absent for them rather
   than reporting 0/'\0' as if it meant something.

   `placed` is DERIVED from `x`/`y` (false for exactly {0,0}), never
   passed in - the same rule for a Desktop Folder item whose saved
   position was never set and for a volume, which this producer never
   places at all (see the desktop-plane note in this header). A caller
   cannot get this wrong by forgetting to compute it. */
typedef struct {
    char name[kNowSceneNameMax];
    char kind[kNowSceneDesktopKindMax];
    int has_type;
    unsigned long file_type;       /* 0 unless has_type */
    unsigned long creator;         /* 0 unless has_type */
    short x, y;                    /* fdLocation; 0,0 = never placed */
    int placed;
    int alias;                     /* fdIsAlias */
} NowSceneDesktopItem;
...
typedef struct {
    ...
    /* The desktop plane: present once either the Desktop Folder walk or
       the volumes walk ran, whichever gets there - see the desktop-plane
       note in this header's opening comment. */
    NowSceneDesktopItem desktop_items[kNowSceneMaxDesktopItems];
    short desktop_item_count;
    int desktop_items_present;
    int desktop_items_truncated;
} NowScene;
...
/* Marks the desktop plane as having run. Idempotent, and safe to call
   from either walk (the Desktop Folder's, or the volumes') - whichever
   gets there first is what makes `desktopItems` present rather than
   absent, even when the other one fails outright or finds nothing. */
int now_scene_open_desktop_items(NowScene *s);

/* Appends one desktop icon or mounted volume. Opens the plane itself if
   it was not already open (the way now_scene_add_control opens
   `controls_present`), so a caller with nothing else to say before its
   first item still gets a present plane.

   Refuses - returns 0, adds nothing, sets no truncation flag - when
   `invisible` is true: the Finder does not draw one, and this scene
   does not report one, independently of whatever the walk passed in.
   That is the same shape as now_scene_add_window's independent refusal
   of a disallowed anchor - assembly enforces its own rule regardless of
   what the collector already believed.

   `placed` is not a parameter; it is derived from `x`/`y` (see
   NowSceneDesktopItem). Returns 1 on success, 0 when the pool is full
   (sets desktop_items_truncated) or invisible refused it. */
