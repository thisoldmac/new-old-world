...
/* The desktop plane: present once either the Desktop Folder walk or the
   volumes walk opened it (now_scene_open_desktop_items), absent when
   neither ran. `type`/`creator` are emitted only for a row with
   `has_type` - a folder and a volume have no Finder file type in this
   reading, so the keys stay absent for them rather than reporting a
   zeroed OSType as if it meant something. */
static void put_desktop_items(Sink *k, const NowScene *s)
{
    short i;

    if (!s->desktop_items_present) {
        return;
    }
    put(k, ",\"desktopItems\":[");
    for (i = 0; i < s->desktop_item_count; ++i) {
        const NowSceneDesktopItem *d = &s->desktop_items[i];
        ...
        if (d->has_type) {
            put(k, ",\"type\":");
            put_signature(k, d->file_type);
            put(k, ",\"creator\":");
            put_signature(k, d->creator);
        }
        put(k, ",\"x\":");
        put_num(k, d->x);
        put(k, ",\"y\":");
        put_num(k, d->y);
        put(k, ",\"placed\":");
        put(k, d->placed ? "true" : "false");
        put(k, ",\"alias\":");
        put(k, d->alias ? "true" : "false");
        /* Always false: an invisible item never reaches this array -
           now_scene_add_desktop_item refuses it before assembly - so the
           key is honestly false for everything that survived, rather
           than an absence that would leave a future producer no key to
           fill in. */
        put(k, ",\"invisible\":false}");
    }
    put(k, "]");
}
...
static void put_meta(Sink *k, const NowScene *s)
{
    ...
    if (s->desktop_items_truncated) {
        put(k, first ? "" : ",");
        put_str(k, "desktop items truncated: the desktop had more than this "
                "scene carries");
        first = 0;
    }
    put(k, "]");
