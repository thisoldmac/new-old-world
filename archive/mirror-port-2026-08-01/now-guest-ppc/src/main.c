...
int main(void)
{
    ...
    while (g_running) {
        conn_service();
        /* Immediately after the connection is serviced, because the
           session heartbeat this writes is a statement about the phase
           conn_service just settled - and because the pump reads it as
           proof this application is still pumping its event loop at all
           (act_session.h). */
        now_act_session_service();
        workshop_idle();
        ask_about_replacing();
    ...
    /* Teardown leaves a flushed breadcrumb before each step and closes
       ...
       and then "stopped". */
    /* The act pump goes FIRST, and by being asked rather than by being
       left to time out: it is another process, it outlives this one by
       default, and a faceless application with no window is the hardest
       kind to notice you have orphaned. */
    now_act_session_close();
