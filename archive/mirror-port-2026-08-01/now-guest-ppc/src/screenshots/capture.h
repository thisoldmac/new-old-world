...
typedef struct {
    ...
    short row_bytes;
    long pixel_bytes;
    /* GLOBAL screen coordinate of bounds' top-left. capture_screen_rect's
       `bounds` is 0-based (the GWorld's own frame), so this is the only
       place the rect's true anchor survives — the pixel-island producer
       needs it to map a fetched island back to guest coordinates. 0,0 for
       a whole-screen capture, whose bounds already start at the screen's
       own origin. */
