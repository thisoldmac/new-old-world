                               ...
                               short row_phase, BandedCapture *cap)
{
    ...
    /* world_bounds keeps screen_bounds' own left/top (only bottom moves,
       for the decimated height) — the whole-screen anchor, not 0,0
       unless the screen itself starts there. */
    cap->image.origin_x = world_bounds.left;
    cap->image.origin_y = world_bounds.top;
                        ...
                        CaptureImage *image)
{
    ...
    /* world_bounds is 0-based (SetRect above) — clip is the only place
       the CLAMPED rect's true screen position survives. Reporting the
       CLAMPED corner rather than the caller's requested one matters: a
       rect straddling the screen edge is captured only in its on-screen
       part, and a consumer mapping pixels back to guest coordinates via
       `global = origin + local` must anchor on what was actually
       captured, not what was asked for. */
    image->origin_x = clip.left;
    image->origin_y = clip.top;
    return kCaptureOK;
}
