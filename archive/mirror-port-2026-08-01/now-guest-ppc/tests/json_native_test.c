/* Native (host-side) test for now-guest-ppc/src/core/json.c — NOT built for the Mac.
...
/* now_json_scope_object: isolating one object-valued key's contents from
 * the frame around it. Its whole reason to exist is a shadowed argument
 * - src/input/input_args.c's now_key_args_from_wire is the caller that
 * needed it, because CommandRequest's own "name" (the command's name)
 * and `key`'s own args.name are the same JSON key in the same flat
 * string. */
static void test_scope_object(void)
{
    char scoped[64];

    /* The envelope shape itself: the outer "name" is the command's own
       ("key"), and args carries a DIFFERENT "name" - the caller's key
       name. Scoping to args must return only the inner one. */
    assert(now_json_scope_object(
               "{\"type\":\"command.request\",\"id\":7,\"name\":\"key\","
               "\"args\":{\"name\":\"return\"}}",
               "args", scoped, sizeof scoped) == 1);
    assert(strcmp(scoped, "{\"name\":\"return\"}") == 0);
    {
        char name[16];

        assert(now_json_find_string(scoped, "name", name, sizeof name)
               == 1);
        assert(strcmp(name, "return") == 0);
    }
    /* And the whole point: the SAME lookup against the unscoped frame
       finds the wrong "name" - the one this test exists to stop a
       caller from reading by accident. */
    {
        char wrong[16];
        ...
        assert(now_json_find_string(
                   "{\"type\":\"command.request\",\"id\":7,\"name\":\"key\","
                   "\"args\":{\"name\":\"return\"}}",
                   "name", wrong, sizeof wrong) == 1);
        assert(strcmp(wrong, "key") == 0);
    }

    /* args appearing before the envelope's own name: position is not
       what makes scoping work. */
    assert(now_json_scope_object(
               "{\"args\":{\"name\":\"escape\"},\"id\":9,\"name\":\"key\"}",
               "args", scoped, sizeof scoped) == 1);
    assert(strcmp(scoped, "{\"name\":\"escape\"}") == 0);

    /* Absent key: no args at all. */
    assert(now_json_scope_object("{\"type\":\"command.request\"}",
                                 "args", scoped, sizeof scoped) == 0);
    assert(scoped[0] == '\0');

    /* Present but not an object: refused rather than scanned past. */
    assert(now_json_scope_object("{\"args\":\"oops\"}", "args",
                                 scoped, sizeof scoped) == 0);
    assert(scoped[0] == '\0');

    /* An empty object round-trips as itself. */
    assert(now_json_scope_object("{\"args\":{}}", "args",
                                 scoped, sizeof scoped) == 1);
    assert(strcmp(scoped, "{}") == 0);

    /* NULL safety. */
    assert(now_json_scope_object(NULL, "args", scoped, sizeof scoped) == 0);
    assert(now_json_scope_object("{\"args\":{}}", "args", NULL, 8) == 0);
}
