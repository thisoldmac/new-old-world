...
/* The regression this guards against lived in ext/src/now_content.c, one
   layer up from anything a host cc can reach (the hooks are Toolbox to
   the last line - see the file header above). What IS reachable from
   here is the half of the contract the Toolbox layer depends on: that
   the ring writer stamps each record from block->ticks AS IT STOOD AT
   THAT CALL, not a value latched once and reused. The bug was the
   caller never refreshing block->ticks between draws in Record mode
   (content_stamp() ran only on the count-mode branch); this test pins
   the ring's own half of the contract so a future change here could not
   reintroduce the same symptom from this side too - two records a tick
   apart must not read identically stamped, and must each carry ITS OWN
   moment, not the other's. Measured against a real guest: 19 records
   drained with ticks all 1735, equal to ring.ticks and unchanged across
   a second armed cycle (docs/open-issues.md). */
static void test_ring_ticks_track_the_moment_of_each_put(void)
{
    TestBlock *tb = make_block(4096u);
    NowContentLinePayload pl;
    NowContentU32 pos1;
    NowContentU32 pos2;
    const NowContentRecHeader *h1;
    const NowContentRecHeader *h2;

    memset(&pl, 0, sizeof(pl));

    tb->block.ticks = 1000u;
    pos1 = tb->block.write_cursor;
    check_eq(now_content_ring_put(&tb->block, kNowContentOpLine, 0,
                                  1u, &pl, sizeof(pl)),
             1, "first line commits");

    /* Simulates what content_stamp() now does on every hook entry: the
       block's ticks move on before the next record is put. */
    tb->block.ticks = 1007u;
    pos2 = tb->block.write_cursor;
    check_eq(now_content_ring_put(&tb->block, kNowContentOpLine, 0,
                                  1u, &pl, sizeof(pl)),
             1, "second line commits");

    h1 = (const NowContentRecHeader *)(const void *)&tb->block.ring[pos1];
    h2 = (const NowContentRecHeader *)(const void *)&tb->block.ring[pos2];
    check_eq((long)h1->ticks, 1000, "first record stamped at its own moment");
    check_eq((long)h2->ticks, 1007,
             "second record stamped at ITS moment, not the first's");
    check(h1->ticks != h2->ticks,
          "two records a tick apart must not read identically stamped");
    free(tb);
}
...
int main(void)
{
    ...
    test_ring_ticks_track_the_moment_of_each_put();
