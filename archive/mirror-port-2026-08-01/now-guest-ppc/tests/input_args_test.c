 ...
 *
 * NARROWED, 2026-08-01: a nonzero `mods` is no longer refused
 * UNCONDITIONALLY. `mods_available` is the caller's own answer for
 * whether the act plane's key op can carry it right now - this file
 * cannot ask the Toolbox that itself - and the wall now stands only when
 * that answer is false. Both values are exercised below, because a test
 * that only ever passed 0 would not notice the wall silently coming down
 * for every caller regardless of the extension's state, and a test that
 * only ever passed 1 would not notice it silently staying up.
 */
static void test_key_modifiers(void)
{
    NowKeyRequest req;

    check(now_key_check(NULL, 0, 0, 'n', 1, 0x0100, 1, 0, &req)
          == kNowKeyModifiers,
          "a command modifier is refused when the act plane cannot carry it");
    check(now_key_check("return", 0, 0, 0, 0, 0x0200, 1, 0, &req)
          == kNowKeyModifiers,
    ...
    /* Refused BEFORE any other complaint: a caller that asked for
       ...
       work. */
    check(now_key_check(NULL, 0, 0, 0, 0, 0x0100, 1, 0, &req)
          == kNowKeyModifiers,
          "the modifier refusal outranks every other argument complaint");
    check(now_key_check(NULL, 999, 1, 0, 0, 0x0100, 1, 0, &req)
          == kNowKeyModifiers,
          "including an out-of-range code");

    /* THE OTHER HALF OF THE NARROWING: with a route available, the same
       request is carried rather than refused, and the modifier survives
       into the resolved request for the caller (now_input_run_key) to
       hand to the act plane. */
    check(now_key_check(NULL, 0, 0, 'n', 1, 0x0100, 1, 1, &req) == kNowKeyOk
          && req.mods == 0x0100 && req.ch == 'n',
          "a command modifier is carried when the act plane can take it");
    check(now_key_check("return", 0, 0, 0, 0, kNowKeyModShift, 1, 1, &req)
          == kNowKeyOk && req.mods == kNowKeyModShift && req.code == 36,
          "a named key keeps resolving and carries the modifier too");

    check(now_key_check(NULL, 0, 0, 'n', 1, 0, 1, 0, &req) == kNowKeyOk
          && req.mods == 0,
          "mods 0 is a caller saying `none`, which is what this verb does");
    check(now_key_check(NULL, 0, 0, 'n', 1, 0, 0, 0, &req) == kNowKeyOk
          && req.mods == 0,
          "and an absent mods is the same request");
    /* mods_available is irrelevant when there is no modifier to route -
       the plain path never needs the act plane and must not appear to
       depend on it. */
    check(now_key_check(NULL, 0, 0, 'n', 1, 0, 1, 1, &req) == kNowKeyOk,
          "mods 0 is fine whether or not a route exists - it never uses one");

    /* The shape check runs BEFORE the availability check: a mask this
       verb has never heard of is the caller's error regardless of what
       could carry it. */
    check(now_key_check(NULL, 0, 0, 'n', 1, 0x2000, 1, 1, &req)
          == kNowKeyModRange,
          "a bit outside the modifier mask is bad-request, not unsupported, "
          "even with a route available");
    check(now_key_check(NULL, 0, 0, 'n', 1, -1, 1, 1, &req) == kNowKeyModRange,
          "a negative mods is out of range the same way");
    ...
    check(strcmp(now_key_status_code(kNowKeyModRange), "bad-request") == 0,
          "an out-of-mask mods is the caller's error");
    check(strstr(now_key_status_message(kNowKeyModifiers), "act plane")
          != NULL,
          "the message names the route that was missing, not a blanket "
          "impossibility");
...
static void test_key_resolution(void)
{
    ...
    check(now_key_check(NULL, 0, 0, 0, 0, 0, 0, 0, &req) == kNowKeyNoKey,
          "no name, no code and no char is not a keystroke");
    check(now_key_check("wiggle", 0, 0, 0, 0, 0, 0, 0, &req)
          == kNowKeyUnknownName,
    ...
    check(now_key_check(NULL, 0, 0, '@', 1, 0, 0, 0, &req) == kNowKeyOk
    ...
    check(now_key_check(NULL, 36, 1, 0, 0, 0, 0, 0, &req) == kNowKeyOk
          && req.ch == 13 && req.char_known,
          "code 36 derives the carriage return it types");
    check(now_key_check(NULL, 10, 1, 0, 0, 0, 0, 0, &req) == kNowKeyOk
    ...
    check(now_key_check(NULL, 45, 1, 3, 1, 0, 0, 0, &req) == kNowKeyOk
          && req.code == 45 && req.ch == 3,
          "a caller that sent both halves gets both halves back");

    check(now_key_check(NULL, -1, 1, 0, 0, 0, 0, 0, &req) == kNowKeyCodeRange,
          "a negative code is refused");
    check(now_key_check(NULL, 127, 1, 0, 0, 0, 0, 0, &req) == kNowKeyOk,
          "127 is the last virtual key code");
    check(now_key_check(NULL, 128, 1, 0, 0, 0, 0, 0, &req) == kNowKeyCodeRange,
          "128 is refused rather than masked down to 0");
    check(now_key_check(NULL, 0, 0, 255, 1, 0, 0, 0, &req) == kNowKeyOk,
          "255 is the last character code");
    check(now_key_check(NULL, 0, 0, 256, 1, 0, 0, 0, &req) == kNowKeyCharRange,
...
static void test_key_message(void)
{
    ...
    check(now_key_check("return", 0, 0, 0, 0, 0, 0, 0, &req) == kNowKeyOk,
          "return resolves");
...
/* ---- key, from the WIRE FRAME ------------------------------------------
 *
 * now_key_args_from_wire takes the RAW envelope, not a hand-built args
 * fragment - the exact shape the bug lived in. Every case below drives a
 * FULL command.request string, the shape now_command_run hands every
 * verb, not a fragment that starts at args - a test built from an args
 * object alone would pass whether or not the scoping actually worked,
 * because it would never contain the envelope's own colliding "name" to
 * begin with.
 *
 * MUTATION-WATCH (2026-08-01): reverting now_key_args_from_wire to search
 * request_json directly - i.e. deleting its now_json_scope_object call
 * and reading `args` (the raw parameter) in place of the scoped copy -
 * fails this test's first check: name comes back "key" (the envelope's
 * own name) instead of "return", and feeding that into now_key_check
 * produces kNowKeyUnknownName, now_key_status_message's "that is not a
 * key this verb names". That is the exact refusal measured 0/10 on the
 * emulator, plain and modified keys alike, because the envelope's own
 * "name" is what a flat scan over the whole frame finds first. Confirmed
 * by reverting, watching it fail that way, and re-applying the fix. */
static void test_key_wire_envelope(void)
{
    char name[kNowKeyNameMax + 1];
    long code, ch, mods;
    int  code_present, char_present, mods_present;

    /* THE REGRESSION ITSELF. The envelope's own "name" is "key" - this
       command's name - and it sits BEFORE args.name in the frame the
       wire actually sends. */
    now_key_args_from_wire(
        "{\"type\":\"command.request\",\"id\":7,\"name\":\"key\","
        "\"args\":{\"name\":\"return\"}}",
        name, (long)sizeof name, &code, &code_present,
        &ch, &char_present, &mods, &mods_present);
    check(strcmp(name, "return") == 0,
          "the NESTED name is read, not the envelope's own");
    check(!code_present && !char_present && !mods_present,
          "and nothing else in the envelope is mistaken for an argument");

    /* The fix does not depend on the wire's own field order happening to
       put args last. */
    now_key_args_from_wire(
        "{\"type\":\"command.request\",\"args\":{\"name\":\"escape\"},"
        "\"id\":9,\"name\":\"key\"}",
        name, (long)sizeof name, &code, &code_present,
        &ch, &char_present, &mods, &mods_present);
    check(strcmp(name, "escape") == 0,
          "args before the envelope's own name resolves the same way");

    /* code, char and mods, sent literally, scoped the same way as name. */
    now_key_args_from_wire(
        "{\"type\":\"command.request\",\"id\":3,\"name\":\"key\","
        "\"args\":{\"code\":36,\"char\":13,\"mods\":256}}",
        name, (long)sizeof name, &code, &code_present,
        &ch, &char_present, &mods, &mods_present);
    check(name[0] == '\0', "no name was sent");
    check(code_present && code == 36, "code was sent literally");
    check(char_present && ch == 13, "so was char");
    check(mods_present && mods == 256, "and so was mods");

    /* No args object at all - a typed caller that sent none. Absent, not
       a crash and not a false match against whatever the rest of the
       envelope happens to contain. */
    now_key_args_from_wire(
        "{\"type\":\"command.request\",\"id\":5,\"name\":\"key\"}",
        name, (long)sizeof name, &code, &code_present,
        &ch, &char_present, &mods, &mods_present);
    check(name[0] == '\0' && !code_present && !char_present && !mods_present,
          "no args object is nothing sent, not a search of the rest of "
          "the frame");

    /* An empty args object - the field is there but empty. Same answer
       as absent. */
    now_key_args_from_wire(
        "{\"type\":\"command.request\",\"id\":6,\"name\":\"key\","
        "\"args\":{}}",
        name, (long)sizeof name, &code, &code_present,
        &ch, &char_present, &mods, &mods_present);
    check(name[0] == '\0' && !code_present && !char_present && !mods_present,
          "an empty args object carries nothing either");

    /* Extracted, the values still drive now_key_check exactly as the
       wire face does - the split between extraction and the gate does
       not change the answer. */
    {
        ...
        now_key_args_from_wire(
            ...
            name, (long)sizeof name, &code, &code_present,
            &ch, &char_present, &mods, &mods_present);
        check(now_key_check(name, code, code_present, ch, char_present,
                            mods, mods_present, 0, &req) == kNowKeyOk
              && req.code == 36 && req.ch == 13,
              "the extracted name resolves to the right key - return, not "
              "a refusal");
    }
}
