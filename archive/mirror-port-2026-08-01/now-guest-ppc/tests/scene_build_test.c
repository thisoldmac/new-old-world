...
/* The desktop plane needs no process and no anchor - it is the guest's
   own File Manager, not a foreign-memory read - so it is present the
   moment either walk opens it, absent only when neither does, and an
   invisible item is refused independently of whatever a caller passed
   in. Mutation-watched: refusing `invisible` unconditionally (removing
   the `if (invisible) return 0;` guard) turns this red. */
static void test_desktop_items_present_and_placed(void)
{
    NowScene s;

    /* Nobody opened the plane: absent, not empty. */
    begin(&s);
    check(s.desktop_items_present == 0, "the desktop plane starts absent");
    check(s.desktop_item_count == 0, "and carries nothing");

    /* Opened, nothing added: present, empty - a real claim ("walked,
       found none"), the same shape now_scene_open_controls gives a
       window. */
    begin(&s);
    check(now_scene_open_desktop_items(&s) == 1, "the plane opens");
    check(s.desktop_items_present == 1 && s.desktop_item_count == 0,
          "present with nothing in it");

    /* A real item: placed is DERIVED from x/y, never trusted from a
       caller. */
    begin(&s);
    check(now_scene_add_desktop_item(&s, "Report", "file", 1,
                                     0x54455854UL, 0x74747874UL,
                                     40, 60, 0, 0) == 1,
          "a file at a real position is added");
    check(s.desktop_items_present == 1, "adding one opens the plane too");
    check(s.desktop_items[0].placed == 1, "a nonzero position is placed");
    check(s.desktop_items[0].has_type == 1
          && s.desktop_items[0].file_type == 0x54455854UL,
          "a file carries its type");
    check_str(s.desktop_items[0].kind, "file", "kind is carried verbatim");

    /* {0,0} is never-placed, whether it is a Desktop Folder item the
       Finder has not laid out or a volume this producer never places at
       all - the SAME rule, not a special case for either caller. */
    begin(&s);
    (void)now_scene_add_desktop_item(&s, "Untitled Folder", "folder", 0,
                                     0, 0, 0, 0, 0, 0);
    check(s.desktop_items[0].placed == 0, "{0,0} derives unplaced");
    check(s.desktop_items[0].has_type == 0
          && s.desktop_items[0].file_type == 0 && s.desktop_items[0].creator == 0,
          "a folder carries no type - the key stays absent at encode time");

    /* A volume: kind "disk" LITERALLY (a one-word mismatch cost upstream
       a fix commit, mirror/3b76f79), always unplaced. */
    begin(&s);
    (void)now_scene_add_desktop_item(&s, "Macintosh HD", "disk", 0, 0, 0,
                                     0, 0, 0, 0);
    check_str(s.desktop_items[0].kind, "disk", "a volume's kind is \"disk\"");
    check(s.desktop_items[0].placed == 0, "a volume is never placed here");

    /* Invisible is refused, independently - no truncation flag, no
       count, exactly as if the row had never been offered. */
    begin(&s);
    check(now_scene_open_desktop_items(&s) == 1, "the plane opens first");
    check(now_scene_add_desktop_item(&s, "Backgrounder", "file", 0, 0, 0,
                                     10, 10, 0, 1) == 0,
          "an invisible item is refused");
    check(s.desktop_item_count == 0, "and adds nothing");
    check(s.desktop_items_truncated == 0,
          "a refusal is not the same as a drop");
    check(s.desktop_items_present == 1,
          "but the plane the caller opened stays present - a folder that "
          "IS all invisible items still says it was walked");
}

/* The pool truncates like every other plane here: a bound is hit, the
   scene says so, and the array stays present (not retracted - there is
   only ONE desktopItems array for the notice to be about, the same
   shape windows_truncated already uses). */
static void test_desktop_items_pool_fills_and_says_so(void)
{
    NowScene s;
    int i;

    begin(&s);
    for (i = 0; i < kNowSceneMaxDesktopItems; ++i) {
        check(now_scene_add_desktop_item(&s, "Item", "file", 0, 0, 0,
                                         (short)(i + 1), 1, 0, 0) == 1,
              "the pool accepts up to its cap");
    }
    check(now_scene_add_desktop_item(&s, "Item", "file", 0, 0, 0, 999, 1,
                                     0, 0) == 0,
          "and refuses past it");
    check(s.desktop_items_truncated == 1, "saying so");
    check(s.desktop_items_present == 1, "the plane itself stays present");
}
...
int main(void)
{
    ...
    test_desktop_items_present_and_placed();
    test_desktop_items_pool_fills_and_says_so();
    test_verdict_tokens();
