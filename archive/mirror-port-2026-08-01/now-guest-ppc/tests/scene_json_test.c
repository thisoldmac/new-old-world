 ...
 * `display`, `items` (the window kind) and `meta.bytes` must still not
 * appear AT ALL: this producer does not report them, and an empty array
 * would assert "this window has no controls" where absence says "this
 * producer does not report controls".
 *
 ...
 *
 * `desktopItems` moved the same way on 2026-08-01, but conditional on a
 * SCENE, not a window: it needs no process and no anchor (it is the
 * guest's own File Manager, not a foreign-memory read), so it is present
 * once either the Desktop Folder walk or the volumes walk ran and absent
 * only when a scene never touches either - `build_small` below is that
 * scene, and `test_desktop_items_is_the_third_conditional_plane` is where
 * the three states get built in one.
 *
...
/* Every retraction reaches meta.errors. A sub-plane that vanished with
   ...
   exists to prevent. */
/* The desktop plane's three states, the same shape test_conditional_planes
   proves for controls - except the "owner" here is the whole scene, not
   one window, so absent/empty/populated are three different SCENES
   rather than three rows of one. */
static void test_desktop_items_is_the_third_conditional_plane(void)
{
    NowScene s;
    char out[4096];

    /* Absent: a scene that never touches the desktop plane at all - the
       state every scene before 2026-08-01 was in, and still is unless
       something calls into it. */
    build_small(&s);
    (void)now_scene_encode(&s, out, sizeof out, NULL);
    check_absent(out, "desktopItems");

    /* Present, empty: the walk ran and genuinely found nothing - a
       Desktop Folder with every entry invisible looks exactly like this,
       and it is a real claim, not silence. */
    now_scene_begin(&s, 1, 0.0, "peek", 640, 480, 0, 0);
    check(now_scene_open_desktop_items(&s) == 1, "the plane opens");
    (void)now_scene_encode(&s, out, sizeof out, NULL);
    check_present(out, "\"desktopItems\":[]");

    /* Populated: a folder, a file with a type, and a volume - the three
       kinds this producer writes, "disk" spelled exactly that way. */
    now_scene_begin(&s, 2, 0.0, "peek", 640, 480, 0, 0);
    (void)now_scene_add_desktop_item(&s, "Lab", "folder", 0, 0, 0,
                                     20, 40, 0, 0);
    (void)now_scene_add_desktop_item(&s, "ReadMe", "file", 1, 0x54455854UL,
                                     0x74747874UL, 90, 40, 0, 0);
    (void)now_scene_add_desktop_item(&s, "A Link", "file", 1, 0x61647270UL,
                                     0x4D414353UL, 160, 40, 1, 0);
    (void)now_scene_add_desktop_item(&s, "Macintosh HD", "disk", 0, 0, 0,
                                     0, 0, 0, 0);
    /* Invisible: refused before it ever becomes a row. */
    (void)now_scene_add_desktop_item(&s, ".hidden", "file", 0, 0, 0,
                                     5, 5, 0, 1);
    check(now_scene_encode(&s, out, sizeof out, NULL) == kNowSceneEncodeOk,
          "the populated desktop encodes");
    check(well_formed(out), "and is well formed");

    check_present(out, "\"name\":\"Lab\",\"kind\":\"folder\",\"x\":20,"
                  "\"y\":40,\"placed\":true,\"alias\":false,"
                  "\"invisible\":false");
    check_absent(out, "\"name\":\"Lab\",\"kind\":\"folder\",\"type\"");
    check_present(out, "\"name\":\"ReadMe\",\"kind\":\"file\","
                  "\"type\":\"TEXT\",\"creator\":\"ttxt\"");
    check_present(out, "\"name\":\"A Link\",\"kind\":\"file\","
                  "\"type\":\"adrp\",\"creator\":\"MACS\",\"x\":160,"
                  "\"y\":40,\"placed\":true,\"alias\":true");
    check_present(out, "\"name\":\"Macintosh HD\",\"kind\":\"disk\","
                  "\"x\":0,\"y\":0,\"placed\":false");
    check_absent(out, "hidden");
    /* Four rows in, not five - the invisible one never became one. */
    {
        const char *p = out;
        int count = 0;
        ...
        check(count == 4, "the invisible item is not a fifth, dropped row");
    }
}
...
int main(void)
{
    ...
    test_desktop_items_is_the_third_conditional_plane();
