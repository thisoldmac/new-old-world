...
- **The act pump deploys BESIDE `New Old World`, or clicks cannot work at
  all.** `act_session.c :: session_find_pump` finds `now-pump` (built by
  `scripts/build-guests pump`) by walking the application's own folder
  for creator `'NWpu'` — never by name. Nothing in this repository
  automates that walk's other half onto real hardware: there is no
  scripted PowerBook deploy for the PPC guest today, so a hand FTP of
  `New Old World` must carry `now-pump.bin` up beside it in the same
  folder, or the machine has no click plane and nothing about the app's
  own deploy says so (docs/open-issues.md, "The act pump exists and has
  never run"). `tools/stage-ext.py` and `scripts/spin-up-ppc` enforce
  this for the emulator; there is no equivalent enforcement for metal yet.
