...
enum {
    ...
    /* V2. Served outright, exactly like Text and Move above: neither
       arms a patch, so both are done by the time this call returns. */
    kNowActServeKey = 6,       /* the hook must post the keystroke       */
    kNowActServeMenuGeom = 7   /* the hook must read the menu's MDEF     */
};
...
/* ---- the click's posting context (V3) ---------------------------------
 *
 * Is a click owed, and is this a pass that may queue it? The request and
 * the click are two different passes in two different processes now
 * (contract/peek_table.h, the click_not_a5 block): the target's pass
 * arms, and any OTHER process's pass posts. This clause
 * is deliberately independent of `status` and of the arm - it is not a
 * request being served, it is one word the application asked another
 * pass to act on, and it must survive a cell whose request has already
 * been published as done.
 *
 * Nothing here checks the arm, on purpose: the arm belongs to a
 * different process's context and re-checking it from this one would
 * make the click depend on winning a race it does not need to win. The
 * guard that keeps a stray click from acting is the patch's own identity
 * check, which has not moved. */
int now_act_click_due(const NowPeekActCell *cell, unsigned long current_a5);

/* Answer the ask: `posted` is what PPostEvent said. Clears the flag last,
 * so an application that sees it clear can trust click_posted. */
 ...
 *
 * now_act_trap_hit's analogue for MenuSelect and TrackControl, and it
 * takes the TABLE rather than the cell because the counters had to be
 * appended at the end of the table to keep every existing offset
 * (peek_table.h, P4b).
 *
 * Called before now_act_armed_cell, so it counts an entry even while the
 * plane is DISARMED - which is the state the counters are most useful
 * in: a patch that is entered constantly with the plane off is a patch
 * that installed correctly, and that is the fact "0/10 menuact" could
 * not establish. `which` is 0 for MenuSelect, 1 for TrackControl. */
enum {
    kNowActVerbMenu = 0,
    kNowActVerbControl = 1
};
void now_act_verb_trap_hit(NowPeekTable *table, int which,
                           unsigned long current_a5);

/* ---- the pump handshake (P4b, V4) --------------------------------------
 *
 * The decisions behind the click route, the session lifecycle and the
 * pump's own self-exit. Here for the reason everything else in this file
 * is here: one half runs inside every process that pumps events and the
 * other half runs in a faceless application with no console, so neither
 * is a place to find out a comparison was backwards.
 *
 * The pump's TOOLBOX code (now-pump/) cannot be tested here and is not
 * pretending to be: what is testable is every decision it makes.
 *
 * NAMING. The pump's two click calls are now_act_pump_click_*, not
 * now_act_click_*, because V3's cell-level pair above already owns those
 * names and the two are different levels of the same ask - the cell's
 * "a press is owed by some other pass" and the pump's "ticket N is
 * yours". Collapsing them into one name would have silently redefined
 * the V3 helpers the click split is measured through. */

/* The pump cell, or NULL when this table has no room for it. Gated like
 * now_act_plane_state - exact act format, and a length that covers the
 * appended region - but NOT on arm_request, because the pump reads this
 * while the plane is idle and the application writes its heartbeat
 * before it has armed anything. */
NowPeekActPump *now_act_pump(NowPeekTable *table);

/* Is a stamp still fresh, `window` ticks later? A stamp of 0 is ABSENT
 * and never fresh - "no session has ever opened" is a different state
 * from "one opened and stopped", and only the second is an orphan.
 *
 * Unsigned tick arithmetic throughout, so the 32-bit wrap costs one
 * window at worst rather than reading as an eternity of staleness. A
 * stamp in the FUTURE (a wrap, or a clock a debugger moved) subtracts to
 * an enormous difference and is treated as stale, which is the safe
 * direction for every caller here: the pump exits, the route falls back
 * to V3's, and both are recoverable on the next beat. */
...
/* Who posts the click for a request that has just armed? */
enum {
    /* No live pump: V3's route stands unchanged - the filter posts from
       the first pass it gets in a world that is not the target's. It is
       the path the ledger's act-click-no-pass describes and it is kept,
       not repaired, so a machine without the pump degrades to a known
       state rather than an unknown one. */
    kNowActClickInline = 0,
    kNowActClickPump = 1
};
int now_act_click_route(const NowPeekActPump *pump, unsigned long ticks);

/* Filter side. Publishes a click for the pump and returns the ticket it
 * committed. Point and modifiers are written BEFORE click_pending, which
 * is the commit word - the same discipline as the anchor slot's stamp.
 *
 * It does NOT wait, and must not: this runs inside a jGNE pass under
 * cooperative scheduling, so the pump cannot get the processor until the
 * filter has returned and its host has yielded. Waiting here would
 * guarantee the timeout it was waiting to avoid. The caller that waits
 * is the application, on its own stack, for `fired`. */
unsigned long now_act_click_request(NowPeekActPump *pump, long h, long v,
                                    long mods, long count);

/* What became of a ticket. For the APPLICATION's diagnosis after a
 * request went unanswered, and for the filter to close the CELL's own
 * ask with - never for either to spin on. */
enum {
    kNowActTicketWaiting = 0,    /* no pump has served this ticket yet  */
    kNowActTicketPosted = 1,
    kNowActTicketRefused = 2     /* the pump tried; the queue said no   */
};
int now_act_click_state(const NowPeekActPump *pump, unsigned long ticket);

/* Pump side. What the pump needs to post one click, copied out under no
 * lock: every field here has exactly one writer and click_pending is
 * read first and last, so a request that changed underneath us is
 * discarded rather than half-served. */
typedef struct {
    unsigned long ticket;
    long h;
    long v;
    long mods;
    long count;                 /* always 1..3 */
} NowActClickOrder;

/* 1 when there is a click to post. */
int now_act_pump_click_due(const NowPeekActPump *pump, NowActClickOrder *out);

/* Pump side. Publishes the outcome: click_error first, then click_posted
 * - which is what a reader keys off, so the error is never read before
 * it is written. `error` is 0 or kNowPeekActErrPostFailed. */
void now_act_pump_click_done(NowPeekActPump *pump, unsigned long ticket,
                             unsigned long error);

/* Application side: one beat per event-loop pass while a host session is
 * open, and 0 (now_act_session_end) when it closes. The pump watches it. */
void now_act_session_beat(NowPeekActPump *pump, unsigned long ticks);
void now_act_session_end(NowPeekActPump *pump);

/* Pump side. Called ONCE at startup, before the first pass.
 *
 * It adopts whatever click_pending currently says as already served.
 * That is deliberate and it is the difference between a pump and a
 * replay: a request published while no pump existed was posted by the
 * filter (or was refused) minutes ago, and a pump that started later has
 * no business queueing a mouse click at a coordinate from a session that
 * has ended. */
...
/* Pump side. Written before the process quits so a reader can tell a
 * clean exit from a crash: a crash leaves Running behind a stale beat. */
void now_act_pump_detach(NowPeekActPump *pump);

/* Pump side: should this pump exit now, unasked?
 *
 * `started` is the tick the pump began at and `seen_session` is whether
 * it has ever observed a live heartbeat. The two together are what make
 * the grace window honest: before the first beat the pump waits out
 * kNowPeekActPumpGraceTicks (it is launched by the application and may
 * well be running first), and after one it exits as soon as the beat
 * goes stale. A pump with no table at all exits: it has nothing to
 * serve and no way to be asked. */
