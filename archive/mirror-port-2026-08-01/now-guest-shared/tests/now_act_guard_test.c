...
static void test_serve_begin(void)
{
    ...
    /* V2: key and menugeom are served OUTRIGHT, like text - no patch to
       check, because neither answers a trap the application calls. */
    memset(&cell, 0, sizeof cell);
    ...
    cell.key_code = 45;
    cell.key_char = 'n';
    cell.key_mods = 0x0100UL;            /* cmdKey */
    check_long(now_act_serve_begin(&cell, kTargetA5, 1UL), kNowActServeKey,
               "key is served outright, not armed");
    check_long((long)cell.key_code, 45, "the request's key fields survive begin");
    ...
    cell.status = kNowPeekActStatusPending;
    cell.menu_id = 130;
    cell.menu_item_count = 999;           /* stale from a previous request */
    cell.menu_width = 999;
    cell.menu_height = 999;
    check_long(now_act_serve_begin(&cell, kTargetA5, 1UL), kNowActServeMenuGeom,
               "menugeom is served outright, not armed");
    check_long((long)cell.menu_item_count, 0,
               "begin clears a stale reply before the hook fills it in");
    check_long((long)cell.menu_width, 0, "and clears width too");
    check_long((long)cell.menu_height, 0, "and height");
...
static void test_plane_state(void)
{
    ...
    check_long(now_act_plane_state(&table), kNowActPlaneReady,
               "a current (V4) extension is ready");

    /* THE FORMAT GATE, as its own case per superseded format rather than
       folded into the "format this build does not know" check below: a
       V1 resident is not an ARBITRARY unknown format, it is a specific,
       previously-valid one, and the length check above would happily
       pass it (a V1 table is not short - it simply has no key_code,
       key_mods or menugeom rects). Only act_format's exact match catches
       it. MUTATION WATCHED FAILING (revert after use): change the
       comparison in now_act_plane_state to `< kNowPeekActFormatV4` (an
       accretive-looking `>=`) and these lines answer Ready for a resident
       that has no room for a V4 request - the exact silent-corruption
    ...
    check_long(now_act_plane_state(&table), kNowActPlaneWrongFormat,
               "a V1 resident is refused, not accepted as a shorter V4");

    /* The V2 resident posts the click inside the TARGET's own pass and
       has no click_not_a5 to be told otherwise, so pairing it with a
       later application arms every request and clicks none of them. */
    table.act_format = kNowPeekActFormatV2;
    check_long(now_act_plane_state(&table), kNowActPlaneWrongFormat,
               "a V2 resident is refused: it would arm and never click");

    /* And the V3 resident, which is the one THIS bump exists for and the
       one the accretive-looking gate would let through. A V3 resident
       knows the click split but has no pump cell, so the ask it publishes
       waits for a process that measured as not existing - it would arm
       every request and click none of them, by the other of the two
       routes. */
    table.act_format = kNowPeekActFormatV3;
    check_long(now_act_plane_state(&table), kNowActPlaneWrongFormat,
               "a pre-pump (V3) resident is refused, not half-accepted");
    ...
    table.caps = kNowPeekTableCapAnchors | kNowPeekTableCapAct;
    table.act_format = kNowPeekActFormatV4 + 1;
    check_long(now_act_plane_state(&table), kNowActPlaneWrongFormat,
...
static void test_click_context(void)
{
    NowPeekActCell cell;

    memset(&cell, 0, sizeof cell);
    check(!now_act_click_due(NULL, 0x1000), "no cell owes no click");
    check(!now_act_click_due(&cell, 0x1000),
          "an idle cell owes no click, whoever is pumping");

    cell.click_not_a5 = 0x1000;
    cell.click_pending = 1;
    check(!now_act_click_due(&cell, 0x1000),
          "the target's own pass never posts the press it is about to get");
    check(now_act_click_due(&cell, 0x2000),
          "any other process's pass does");
    check(!now_act_click_due(&cell, 0),
          "no A5 is nobody's world, not merely 'not the target'");

    /* Excluding nobody is how the application asks for "the next pass,
       whoever it belongs to" - which is what it asks for today, because
       on this machine the target is the only process whose loop reaches
       the hook while the wire waits. */
    cell.click_not_a5 = 0;
    check(now_act_click_due(&cell, 0x1000),
          "excluding nobody lets the next pass post it");

    /* THE POINT OF THE WHOLE CHANGE, as an assertion: the ask does not
       depend on the request's own state. The arm lives in the TARGET's
       context and is published as done before this flag is ever set, so
       a clause that also required status == pending, or armed == ready,
       would never fire in the application's process at all - which is
       the shape of the bug this replaced, arrived at from the other
       direction. MUTATION WATCHED FAILING: add `cell->status ==
       kNowPeekActStatusPending` to now_act_click_due and this fails. */
    cell.status = kNowPeekActStatusDone;
    cell.armed = kNowPeekActArmNone;
    check(now_act_click_due(&cell, 0x2000),
          "a published, disarmed request still owes the click it asked for");

    cell.click_passes = 3;              /* the hook saw it three times */
    now_act_click_done(&cell, 1);
    check_long((long)cell.click_posted, 1, "the answer is recorded");
    check_long((long)cell.click_passes, 3,
               "the pass count is the hook's to keep, not the answer's");
    check_long((long)cell.click_pending, 0, "and the ask is cleared");
    check(!now_act_click_due(&cell, 0x2000), "one ask, one click");

    /* A refusal is not a silence: PPostEvent declining and this
       application's own hook never running are opposite repairs, and the
       caller can only tell them apart if the failure writes something. */
    cell.click_pending = 1;
    now_act_click_done(&cell, 0);
    check_long((long)cell.click_posted, 0, "a refused post says so");
    check_long((long)cell.click_pending, 0, "and still clears the ask");
}
