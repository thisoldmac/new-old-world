...
| `now-pump/` | The act plane's posting context: a faceless 68K background application that queues the act plane's mouse press from a classic process's own context, because a Carbon application cannot and because nothing else on the machine was pumping. Optional in the same sense `ext/` is, built and **never run** — see docs/open-issues.md. |
