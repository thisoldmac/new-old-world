...
| `capture.region` | ✅ | ❌ | side-effect-free rect capture (no PSN, no SetFrontProcess) — the pixel-island fallback's fetch, added 2026-08-01; NOW-68K has no scene/mirror consumer to feed |
...
PPC handles 38 inbound types; NOW-68K handles 23. **That count
...
The registry is `x-commands` in the contract: **38 verbs.** Sixteen of
...
plane's reader. The thirty-eighth, `actstate`, landed 2026-08-01 with
the act pump: it is the plane's own instrument panel, and it exists
because every number this plane had produced until then had to be
parsed out of a failure sentence.
...
| `menugeom` | one menu's per-item geometry, read from its own MDEF | ✅ | ❌ |
| `activate` | bring one process forward, by serial number | ✅ | ❌ |
| `actselftest` | prove the act plane's trap ABI in one process | ✅ | ❌ |
| `actstate` | the act plane's own counters, read rather than narrated | ✅ | ❌ |
| `key` | post one keystroke, with a modifier route via the act plane | ✅ | ❌ |
...
Eleven of those seventeen — the act plane and the reference layer — are
one mechanism and are served together or not at all. They are PowerPC-only
...
**PPC serves 35 of 38.** `put` is console-only there and `cancel` is
not a verb at all, both deliberately: the host reaches those
capabilities through the `file.*` families and that guest's own
Workshop. `shotdiag` is the third, and the newest before `menugeom`: it
diagnoses a raw framebuffer walk the PowerPC guest does not have. (`key`
was missing its own row here until 2026-08-01 - present in `x-commands`
and served since before this file's last derivation, but never counted.
Fixed in the same commit that added `menugeom` beside it, since both
touch this exact table; a fuller audit of this file for the same gap
elsewhere has not been done and is not claimed.)

`menugeom`, declared 2026-08-01 beside `key`'s modifier route, is an
ELEVENTH member of the act-plane family above, not a twelfth mechanism:
it is served outright by the same extension (kNowPeekActOpMenuGeom),
gated by the same V2 plane-state check, and carries the same **served
is not proven** caveat every act verb above it does — nothing has
watched a real menu's MDEF answer this yet.

**NOW-68K serves 13 of 38** — `help`, `ls`, `sw`, `census`, `put`,
`cancel`, `vprobe`, `screenshot`, `shotdiag`, `ps`, `launch`, `quit`,
`front`. The twenty-five it does not: `gestalt`, `catsearch`, `tail`,
`reveal`, `vers`, `putstat`, `key`, the eleven of the act plane and the
reference layer, and the six registered on 2026-07-31 — `activate`,
`actselftest`, `mouseloc`, `script`, `aesend`, `qdtrace` — and
`actstate`, which reads a table that guest has no extension to publish.
Most of those
are not a 68K debt: several reach for OSA, Apple Events, a content-plane
ring or a resident MDEF call that guest's own act plane does not carry,
