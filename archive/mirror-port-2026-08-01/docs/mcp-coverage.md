...
| `now_wait_for` | `process.list` | `process.list` | message family |
| `now_observe_elements` | `elements` | `elements` | command |
| `now_find_elements` | `elements` | `elements` | command |
...
| `now_scene` | `scene.request` | `scene.request` | message family |
...
| `capture.region` | message | ppc | unnoticed | The side-effect-free sibling of `process.shot`: names a GLOBAL rect directly, so no process is fronted — built as the pixel-island fallback's fetch (`SceneIslands.attach`'s `Capture` closure, docs/mirror-knowledge.md's "Lane A-fallback"), not as a capability a caller asks for by itself. Nobody has decided whether it belongs on this surface. What a row would have to settle first: `now_capture_screen` already projects `capture.request` and takes no rect, so a row here is either a second, narrower route to the same underlying capability (a crop `now_capture_screen` could in principle grow a parameter for instead) or a distinct one ("capture exactly this rect, side-effect-free" is not quite "capture the whole screen"); and whether an agent has any use for a raw rect crop that is not also served by `process.shot`'s window-relative one — which is itself `deliberate`-excluded above. The wire half only exists to feed a renderer, the same shape `scene.request` was in before its own row was argued. |
...
| `actstate` | command | ppc | deliberate | The act plane's own counters — how often each patched trap was ENTERED, how many of those entries were ours, and whether a click would go through the act pump or by V3's inline route. **An instrument, not a capability**, and the same argument `mouseloc` makes: an agent driving an application has nothing to do with the number of times some process entered `MenuSelect`, and the reader who does is a person diagnosing this plane at the machine or a wave measuring it. It is on both guest faces for exactly that reason, which is the same "one capability, one route per face" reasoning [command-parity.md](command-parity.md) applies to `mouseloc`; what the counters are FOR, and the trials that must read them, is in [open-issues.md](open-issues.md) under the act pump. Revisit only if an agent is ever expected to decide whether to attempt an act from these numbers — which would be a different verb's job, since deciding needs a verdict and this deliberately reports readings. |
...
| `menugeom` | command | ppc | unnoticed | One menu's per-item geometry, read from its own MDEF rather than assumed at a fixed row height — landed 2026-08-01 beside `key`'s modifier route, served outright by the same extension. Nobody has decided whether an agent driving this surface needs real menu geometry at all: `menuact` already performs the command by identity without ever drawing the menu, so the only caller for this row's ANSWER (as opposed to its mechanism) is the host's own dropdown rendering, which reads it directly rather than through a tool the way `mouseloc` does. **That caller landed 2026-08-01, lane L7**: `MirrorModuleModel.openDropdown(at:menu:)` fetches it on a menu-title press through `AgentIntegrationActControl.menuGeom`, and `SceneRenderer`'s dropdown drawing/hit-test consume the real per-item rects when one matches, falling back to the uniform 16px row otherwise — unverified on a paired guest, same as `key`'s modifier route above. What a row on THIS surface (an agent-facing tool) would still have to settle first: whether a projection exists for an agent that wants to describe a menu's layout to a person, or whether that is always a host-rendering concern with no agent-facing use — nobody has asked. |
...
**`axsnap`, `handle`, `actselftest`, `aesend`, `script`, `menugeom` and
`capture.region`** — all served by the PowerPC guest, none decided either
way. Their rows above say what a decision would have to settle.

**`capture.region` joined 2026-08-01**, alongside the pixel-island
producer it feeds. It differs from the others in shape: those are
verbs an agent might plausibly call directly, where this one exists to
supply a renderer and was never built with a caller other than
`SceneIslands.attach` in mind — which is precisely why the question
belongs here rather than being answered by whoever wrote the wire code.
...
`menugeom` joined a lane later, 2026-08-01, beside `key`'s modifier route
— its own row above says what makes it a different SHAPE of undecided
question from the other five: the mechanism is proven reachable the same
way every other act-plane verb is, but the case for a tool at all is
weaker, because the one caller anyone has named for its answer is the
host's own rendering rather than an agent.
