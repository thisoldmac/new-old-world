...
## The act pump exists and has never run (2026-08-01)

**Unverified, in the strongest sense: a whole new process, built and
never launched.** `now-pump/` is a faceless 68K background application
whose only job is to post the act plane's click from a CLASSIC process's
own context. It cross-compiles; nothing else about it is known.

**What it answers, and it is a measurement rather than a theory.** V3
moved the press off the arming pass and onto a pass belonging to any
process but the target's, and measured 0/5 — with 293–303 act passes in
five seconds and **every one of them in the target's own A5 world**.
V3's rule was not wrong; it had nobody to obey it. There was no other
classic process pumping, because a background Carbon application's
`WaitNextEvent` does not fall through to the classic Event Manager.
The pump is that missing process.

**What it does not fix by existing.** The plane still arms in the
target's own jGNE pass exactly as before, and V3's route is untouched.
The only change is WHO posts the press: `now_act_click_route()` sends the
ask to the pump when one is beating and falls back to V3's inline route
when there is not. A machine without the pump therefore degrades to
precisely today's behaviour, deliberately, so a failure after installing
it is attributable to the pump rather than to a rewrite.

**The two asks are not two answers.** The cell's `click_pending` (V3)
stays the word the application waits on; the pump's ticket is how the
filter gets it served, and the cell's ask is closed with the pump's
reply through the same `now_act_click_done` V3 already used. Whichever
route serves a press, the caller reads one story.

**The format gate is now exact at V4**, so an extension built before this
handshake leaves the whole act plane dark rather than half-accepted.
Extension, application and pump are built from one header and must be
deployed together. A machine carrying a V3 extension will report a wrong
format on every act verb, and that is the intended reading.

**What the emulator wave has to measure**, and it should not be inferred
from a reply sentence — `actstate` reports every number below directly,
arms nothing, and should be read immediately BEFORE and AFTER each
trial:

- `Pump` / `Pump beat` / `Session beat` — that the application launched
  it, that it is beating, and that the session heartbeat is being
  written. If `Pump` reads `never attached` after a session opens, the
  failure is the launch (wrong folder, wrong creator, or
  `LaunchApplication` refusing), not the plane.
- `Click route` — it must say the pump. If it says V3 inline, nothing
  below is a test of anything new.
- `Ticket asked` / `Ticket served` / `Last click error` — asked without
  served is a pump that is not running or not looping; served with an
  error is `PPostEvent` refusing, which is a full event queue and is a
  different repair from anything to do with the guard.
- `Click passes` / `Click last A5` — V3's own instrument, and still the
  one that says whether the FILTER is being reached at all. A run where
  it still shows one world is saying the ask never left the target,
  which the pump's rows cannot distinguish from a pump that never
  started.
- `MenuSelect hits` / `MenuSelect ours` and `TrackControl hits` /
  `TrackControl ours` — **the numbers this whole instrument exists for.**
  Global hits at zero across ten trials means the patch is never entered
  and the repair is installation. Global hits rising with `ours` at zero
  means the press is arriving in the wrong process, or at the wrong
  point, and the repair is the request. Until 2026-08-01 those two were
  the same symptom, which is why `0/10 menuact` settled nothing.
- The four window traps' pairs, the same way, for `winact`.

Ten trials per case, and the cases are: `menuact` on an item with no
keyboard shortcut; `ctlact` on a push button; `winact` close, zoom and
resize. Each against a plain application that is NOT the guest itself,
with the guest running and connected so a session is open.

**Three things nobody has watched.** The pump's Toolbox half — the SIZE
flags that keep it out of the foreground, `PPostEvent` with `evtQWhere`
stamped, the broad event mask Mirror measured as load-bearing — has no
test in this tree and cannot have one; `act_pump_test.c` says so at the
top. The launch path in `act_session.c` — a catalogue walk for a creator
signature, then `LaunchApplication` — has never run: nothing in this
product had launched a process on a Macintosh before it. And a pump that
DOES come to the front would break every act it exists to enable,
silently, by taking the front layer from the application the click is
aimed at. If actuation is 0/10 with the counters showing entries in the
wrong process, suspect that first.

**What is deliberately NOT claimed.** Mirror's 20/20 is Mirror's. This
port copies the SHAPE that produced it and inherits none of the number.

### 2026-08-01, emulator: it has now run. The launch half works.

**Emulator-verified** on mac99/OS 9.1, build `4c4930f22c97`
(`2026-08-01T19:40:48Z`, confirmed against this build's
`build_stamp_gen.h` in the guest's own hello). The first three sentences
of this entry are no longer true, and the rest of it still is.

With a wire session open, `actstate` reads:

    Plane            ready          Act format      4
    Pump             running        Pump beat       2 ticks ago
    Session beat     7 ticks ago
    Click route      the pump (its own classic context)

and the guest's own `ps` shows `now-pump` as `background, 109 KB` with
`New Old World` still `front`. So all four of the things this entry said
nobody had watched — the catalogue walk for creator `'NWpu'`,
`LaunchApplication`, the heartbeat handshake, and `launchDontSwitch`
keeping the pump out of the front layer — did what they were written to
do. `Click route` naming the pump means the act measurements below are
tests of the new route rather than of V3's.

**And clicks land.** `Ticket asked` and `Ticket served` were both 0 at
that first reading, because nothing had asked yet. Across the act runs
below they moved together, one for one, and `Last click error` stayed 0
throughout — every ask the application made, the pump served.

    winact close   10/10   oracle: the window's absence from a fresh walk
    winact move    10/10   oracle: the window's own rect
    ctlact         10/10   oracle: the scroll bar's own value
    menuact        10/10   oracle: an 'untitled folder' on disk
    key cmd-N      10/10   oracle: an 'untitled folder' on disk
    key cmd-W      10/10   oracle: the window's absence
    key plain      10/10   ACCEPTED only — see the caveat below

Per-trial, `winact close` was arithmetically clean: each trial added
exactly +2 `FindWindow hits`, +1 `TrackGoAway hits`, +1 `Ticket asked`
and +1 `Ticket served`, with `TrackGoAway ours` reading 1 every time.
So the press was entered, and entered in the target, on all ten.

**`winact move` moved nothing in the act plane, which is the correct
result and worth stating separately.** No trap counter changed across
its ten trials — move calls `MoveWindow` in the target's own pass and
uses no click at all. It would therefore pass on a machine where the
click family failed entirely, so pooling it with close would have
manufactured a number. It is reported apart for that reason.

**One instrument asymmetry, found here and not chased.** The cell's
`trap_hits_target` (the four window traps) RESETS per arm — it read 1 on
each of the ten close trials rather than climbing. The table-root
counters `act_control_hits_target` / `act_menu_hits_target` do NOT: they
accumulated 0 -> 19 over the ctlact run. Both readings are usable once
you know which you are looking at, and nothing above depends on the
difference, but a reader comparing a window pair against a control pair
will otherwise conclude one of them is broken.

**`menuact` is the one to read twice, because it is the case that used
to settle nothing.** It went 10/10 with counters that are monotone and
unambiguous: every trial added exactly +1 `MenuSelect hits`, +1
`MenuSelect ours`, +1 `Ticket asked` and +1 `Ticket served`, ending 11 /
10 / 40 / 40. The old 0/10 could not distinguish "never entered" from
"entered in the wrong process"; this run is `ours` climbing one per
trial, which is the target's own `MenuSelect` and nobody else's. The
menu id (257), the item index (1) and `titleLeft` (38) were taken from
the scene's own menubar rather than guessed, which matters because
`titleLeft` IS the identity check.

**The two key families do NOT share a route, and only one is the act
plane's.** cmd-N and cmd-W both went 10/10, and across all twenty trials
`Ticket asked`/`Ticket served` never moved and `MenuSelect ours` stayed
0. So the modified-key path posts the keystroke and the Finder's own
`MenuEvent` dispatches it — the documented Mirror keycode fix (a virtual
KEYCODE beside the char) working — and it is independent of the click
family the pump serves. That is worth knowing before anyone reads the
key numbers as evidence about the pump: they are not.

**`key plain` is the weakest number on this page and is listed as 10/10
only because the verb claims nothing more.** `mods 0` never reaches the
act plane, so there is no counter to read and no side effect to look for
— the "oracle" is that the guest accepted and posted. It is 10/10
ACCEPTED, not 10/10 observed, and it should not be quoted beside the
other six.

**And it GOES when the wire drops, which closes the lifecycle.** The
oracle here cannot be the wire — dropping it is the stimulus, and asking
the guest means opening a session, which relaunches the process being
looked for. So this was read from the anchor worker's own `observe`, a
channel sharing nothing with NOW's wire: `now-pump` present before the
drop, GONE at the first sample five seconds after. Opening a fresh
session brought it back. Launch, serve, quit, relaunch — all four
watched.

**The spin-up does not install the pump, and that is why it had never
run.** `tools/stage-ext.py` stages the extension and the application and
stops there; `scripts/spin-up-ppc` calls it and adds nothing. But
`act_session.c :: session_find_pump` walks the APPLICATION'S OWN FOLDER
for a file with creator `'NWpu'`, so on a stock spin-up the pump is
resident nowhere and the launch cannot succeed — it fails to the warning
"act pump not found beside this application". `scripts/build-guests`
grew a fourth artifact and the staging script did not, which is the same
shape as the gate-that-never-invoked-a-cross-compiler in AGENTS.md. The
run above staged `now-pump.bin` into `Macintosh HD:TimBotTu:now-dev:`
by hand to get a measurement at all.

**Closed for the emulator, same day.** `tools/stage-ext.py` now REQUIRES
`NOW_PUMP_BIN` whenever `NOW_APP_BIN` is given and stages it beside the
application, verified the same way the app and extension already are —
by fork size read back off the guest, never by the push's exit code.
`scripts/spin-up-ppc` builds and checks for it before booting anything,
passes it through, and — because a staged file surviving a reboot is not
proof the pump LAUNCHED — reads the `actstate` verb it already asks over
the wire and fails loudly, naming the pump, if the pump region is absent
(an extension built before the handshake) or `Pump` still reads `never
attached` after the same settle window `actselftest` gets. A stock
`scripts/spin-up-ppc` run can no longer produce the silent half-machine
this entry describes.

Two things this fix does NOT cover, both worth naming rather than
letting the gate's own confidence imply otherwise:

- **Untested on a machine.** Nothing above has been run — not against
  the emulator, and certainly not against the PowerBook 1400c. It is
  reasoned from `session_find_pump`'s creator/type walk and from
  `mach_actstate.c`'s row words, both read, not from a watched pass. The
  next session to run `scripts/spin-up-ppc` for real is the first actual
  evidence.
- **The PowerBook has no automated deploy path at all, pump or
  otherwise.** `scripts/deploy-68k` deploys NOW-68K — a different guest,
  built from `now-guest-68k/`, with no pump concept — to the 68K
  PowerBook 180c; nothing in this repository automates getting `New Old
  World` and `now-pump.bin` onto real PowerPC hardware over FTP. Until
  such a script exists, a hand deploy to metal must stage the pump beside
  the application itself, by the same creator/type identity
  `tools/stage-ext.py` now checks — and has exactly the same way to get
  it wrong silently that this entire entry describes.

**A second thing that run needed, and that nothing in the tree provides.**
The wire port is not collision-safe the way the anchor port is.
`spin-up-ppc` probes and advances for the anchor, but requires 5250 to be
free and exits if it is not — and on this Mac another session's host app
held it. The guest's compiled-in default is 10.0.2.2:5250, so a guest
left alone would have dialled THAT session's host: the two-guests-on-one-
port failure from 2026-07-28, pointing the other way. The run above
worked around it by staging a preferences file (`prefs.c`'s v1 record —
magic, format, port, `host[64]` — is 72 bytes and a format below 3 keeps
every other default) that pins host 10.0.2.2 and a port no one else
holds. Worth making a real option rather than leaving the next session to
rediscover it.

## The act plane's neighbours, measured in the same pass (2026-08-01)

**Emulator-verified**, same run, same build `4c4930f22c97`, mac99/OS 9.1.
Three things the act numbers above sit beside.

### `capture.region` produces real pixels, and is far faster than its help text

Its first run anywhere. A 300x200 rect over a Finder folder window at
depth 8 with PackBits answered:

    width 300  height 200  depth 8  rowBytes 304  paletteBytes 768
    originX 13  originY 47   (the window's own top-left, exactly)
    encoding packbits   bulk 8715 bytes, matching capture.begin's `bytes`

The **decode was checked rather than assumed**, because a wrong stride
produces exactly the right byte count: walking the per-row big-endian u16
lengths and unpacking consumed all 7966 row bytes with nothing left over
and produced exactly 60000 pixels, and the image renders as the window's
real content (`docs/local/wave6-03-capture-region-decoded.png`). Repeating
the same rect against an unchanged screen returned an identical CRC.

A rect past the documented 1024 px side cap (1600x1200) **refuses
honestly** — `capture.end ok:false`, no `capture.begin`, no bytes, in
109 ms. It refuses; it does not band.

**The timing contradicts the toggle's help text.** These took 9-110 ms
wall, not the ~1 s/window figure that text quotes. One emulated machine
is not a PowerBook 1400c and this does not refute the figure on metal —
but the help text is currently the only number a user sees, and nothing
has measured it on hardware.

### `bytes` in `capture.begin` does not mean what the contract says

The contract describes it as "Total bulk bytes to expect (rowBytes *
height)". With `encoding: packbits` that product is 60800 and the field
carried 8715 — the COMPRESSED total, which is also what actually
arrived. The implementation is the sensible one and the prose is wrong
for every packed capture. A host that trusted the parenthetical to
size a buffer would over-allocate by 7x, and one that used it to detect
a short transfer would call every successful packed capture short.

### The scene carries the controls the host-side role inference needs — for one of the two scroll bars

With a Finder folder window open, `scene.request` returned 12184 bytes in
43 ms: `desktopItems` 18 (populated, with names, kinds, types and
positions), `menubar` with 8 menus carrying ids, titles, `left` and full
item lists, and 2 windows. The folder window's 3 controls include the
vertical scroll bar as a genuine ranged control — `value 55, min -4,
max 188`, rect 16x184 — which is what the host's scrollbar inference
needs, and it is real guest data rather than a host default.

**But the horizontal scroll bar of the same window reports `min -52,
max -52, value -52`.** Any host-side inference keyed on `max > min` will
classify it as not-ranged and miss it. That is one to check against
whatever rule the host actually uses before the inference is called
proven.

Two smaller gaps in the same document, stated because they are easy to
mistake for a decode failure: the folder window's `items` array is
EMPTY (0 items for a window showing 15 files — the per-window item route
is the separate folder-items script path, not this document), and `New
Old World`'s own window reports 0 controls, so the guest does not walk
itself.

### `qdtrace` records real drawing, and every record carries the same tick

Arming the front process resolved the Finder's A5 (`0x1f50f550`),
reported `hookedPorts: 2`, `installs: 2`, `repairs: 1`, and `start`
correctly answered `requested: true, armed: false` — the documented
"nothing is hooked until the extension agrees inside the target". Ten
window moves later the ops counters had moved (`rect` and `bits` both
climbing, 8 ops for that provocation) and a drain from the pre-arm cursor
returned 19 records — 11 `state`, 4 `bits`, 4 `rect` — with real
contents: blit src/dst rects, `srcRowBytes 128`, clip rects, origins and
RGB fore/background. `more`, `resync`, `torn` and `busy` were all false
and `lostBytes`/`dropped`/`wraps` all 0, so the short-drain guard had
nothing to report and said so.

**The defect in it: the tick stamp never advances.** All 19 records
carried `ticks: 1735`, the identical value `ring.ticks` reports, and the
same value seen in a status read minutes earlier and in a separate armed
cycle before it. So a record cannot be ordered against another record,
aged, or correlated with anything — which is most of what a timestamp on
a draw record is for, and it matters specifically for the island
recapture signal this plane now feeds. The application's own `TickCount`
is fine (`actstate` reports "Pump beat 2 ticks ago" and it moves), so
this is the resident half's stamp, not the clock.

## A real scene with menus or controls fails to decode in the app (2026-08-01)

**CRITICAL, found during wave 3's pane-render attempt (check 7),
`mac99-os91`, build `67368e24beb2 2026-08-01T08:58:22Z`.** The production
decode path — `MirrorScene.decode(result:)`
(`now-host/Sources/MirrorKit/IRVersion.swift:100`), a plain
`JSONDecoder().decode(Scene.self, from: data)` with no pre-processing —
**throws on a real scene the moment its front process has a populated
menu bar or a window has controls**, because `Scene`'s frozen v1 Codable
requires three fields the PPC guest's producer either never emits or
emits only conditionally:

| field | required by `Scene.swift` | what the guest actually does |
|---|---|---|
| `menus[].apple` | non-optional, no default (`Scene.swift:373`, plain `c.decode`) | **never emitted, by explicit design** — `scene_json.c:166` says outright: *"nothing this walk reads says which menu is the Apple menu … so the key stays absent rather than becoming a plausible false claim"* |
| `items[].cmd` (a `MenuItem`'s shortcut) | non-optional | only emitted when the item HAS a shortcut — 65 of the real File/Edit/View/Special menu's items in this one capture had none |
| `controls[].role` | non-optional (`Scene.swift:233`) | **never emitted at all** — zero occurrences of `"role"` anywhere in `now-guest-ppc/src/scene/scene_json.c` |
| `controls[].checked` | non-optional | not emitted in this capture either |

Reproduced three times in sequence, live: decoding an actual scene
fetched from this session's guest (Finder front, `File` menu open,
`Macintosh HD` window's controls present) threw
`DecodingError.keyNotFound` for `apple`, then (after patching that key
in) for `cmd`, then (after patching that in) for `role` — three
independent, real gaps in the same code path, not one. Only after all
three were patched into the captured JSON did `JSONDecoder` succeed and
`RenderShot.png` produce a correct-looking frame
(`docs/local/wave3-pane.png` beside the guest's own screenshot,
`docs/local/wave3-03-final-state.png` — same two windows, same relative
layout).

**Why this was invisible until now**: every existing test that builds a
`Scene` (`IslandRenderTests`, `MirrorFolderItemsJoinTests`'s `scene(windows:)`
helper, etc.) constructs one directly in Swift, which always sets every
non-optional field — there is no way to *forget* `apple`/`role` when
you're writing a Swift literal. The only way to see this gap is to feed
the decoder something the real guest actually sent, which is exactly
what `MirrorScene.decode` does at runtime and no test before this one
did. This is another instance of the defect class this project already
has a name for — `two-halves-never-met-in-a-test` — one layer further in
than the wire message shape: here both sides agree on the MESSAGE
(`scene.request`/`scene.begin`/`scene.end`) and disagree on the BODY's
schema.

**Consequence if unfixed**: the real host app's scene pipeline — poller,
renderer, the Mirror pane itself — will throw and (depending on the
poller's error handling, not audited here) most likely stop updating or
silently drop the scene, on the **ordinary case** of a front process with
a menu bar, which is nearly every process, every time. This was not
caught by wave 3's checks 1–6 only because those went through a raw
Python dict (bypassing Swift's `Codable` entirely) rather than through
the app's own decoder.

**Not fixed here** — this pass is validation, not repair; the fix is
either loosening the three fields to `decodeIfPresent`-with-default (the
same three-states treatment `Scene.swift`'s own header already argues
for `appsPresent`/`windowsPresent`/etc.) or having the guest emit them.
Either is a real behaviour change and belongs to an owning agent, not to
whoever tripped over it during an emulator pass.

## Folder windows are modelled, and none of it has run against a Macintosh (2026-08-01)

**Partially emulator-verified 2026-08-01, wave 3** (`mac99-os91`, build
`67368e24beb2 2026-08-01T08:58:22Z`) — the fetch half, not the click half.

- **The exact `FinderItems.windowsScript()` body, run against a real
  Finder with two open windows** (`Macintosh HD`, then `TBTRunner`
  opened inside it), returned a quote-wrapped `W|`/`I|` record stream
  that matches `FinderItems.parse`'s assumed shape byte for byte — no
  diff, no CRITICAL finding here. The raw capture is now a committed
  fixture (`now-host/Tests/HostTests/MirrorFolderItemsJoinTests.swift`,
  `testParsesARealFinderReplyVerbatim` / `testJoinsARealFinderReplyOntoAMatchingScene`),
  labelled with its provenance.
- **`MirrorFolderItemsAim`'s mechanism — `aesend odoc` at the Finder's
  PSN with a composed HFS path — opens a named item reliably.** 10/10
  trials against `"Macintosh HD:TimBotTu"` (not open beforehand) each
  got `sent:true` and the `TimBotTu` window present in the next scene.
  **Caveat: not closed between trials.** No working close mechanism
  existed this session — `key` is unconditionally broken (see the `key`
  entry below) and the act plane's arm-then-await family refused every
  attempt (`menuact` File→Close Window and `winact` `action:close` both
  answered `act-not-taken`, "the application never called
  MenuSelect/FindWindow for our click", even with Finder confirmed
  front and a correct menu id/titleLeft or window ref) — so trial 1 is
  the genuine open and trials 2–10 are idempotent re-assertions of an
  already-open window, not ten independent open/close cycles. Whether
  that act-plane refusal is an emulator artifact or a real defect in
  the synthesized-click mechanism is unknown; it was not this pass's
  scope to chase, but it blocks any future check that needs a
  synthesized close, drag, or control track on this guest.

  **Wave 4 follow-up, same day, same VM (`mac99-os91`, build
  `67368e24beb2`).** Front-process discipline is NOT the cause. `activate`
  against Finder's own PSN (`0.29949953`) answered `already-front`, `ps`
  confirmed `Finder ... front` before and throughout, and `winact` close
  against a real window (`TBTRunner`) still refused 0/10, byte-for-byte
  the same message every time — `answers=0; this request saw find=0
  grow=0 box=0 goaway=0; the machine saw find=1` (that single `find=1` is
  a stale count from earlier in the day, not from these ten calls: it
  never moved across all ten). So the counters say something more
  specific than "the click didn't land": `g_snap.armed ==
  kNowPeekActArmReady` came back true every time, which only happens
  after `now_act_serve_begin` has already matched `current_a5` against
  Finder's own A5 **while Finder itself was pumping** and then run
  `act_post_click` successfully — i.e. the extension's jGNE filter did
  catch Finder mid-`WaitNextEvent` and did `PPostEvent` the mouseDown/
  mouseUp pair from inside that exact call. What never happens
  afterwards is Finder's own application code calling `FindWindow` in
  response — not once, on any of the ten attempts, over roughly a
  minute of wall time.

  `ctlact` against a real, in-range vertical scrollbar (TimBotTu's,
  `min=-4 max=252`) fails identically (`armed, and the application never
  called TrackControl`), which rules out anything specific to
  `FindWindow`/window content: the whole click-driven arm-then-await
  family is affected, not one trap. A controlled before/click/after
  triplet confirmed the control's `value` does **not** move as a result
  of the call (162 before, 162 after) — an earlier apparent jump from
  `-4` to `162` between two unrelated reads was a false lead, not
  evidence of a delivered click; it turned out to be unrelated Finder
  activity between two `elements` snapshots, not caused by `ctlact`.

  `menuact` (File id 257, item 5 "Close Window", `titleLeft=38` read
  from the same day's committed scene capture,
  `docs/local/wave3-live-scene.json`) surfaced a second, separate
  finding: only the *first* call in a run of ten got a wire-level reply
  at all (`act-not-taken`, same shape as the others); calls 2 through 10
  each ran out the full 20s reply-timeout with **no reply whatsoever** —
  not a refusal, the wire simply went quiet — and only recovered
  afterwards (`ps`/`mouseloc` answered normally again once the run
  ended). `winact` and `ctlact`, hammered back-to-back the same way,
  never did this. Whether repeated `menuact` specifically leaves the
  Menu Manager (or this plane's menu patch) in a state that stalls the
  wire's own pump for many multiples of one call's deadline, or this is
  an artifact of clicking at `v=10` in the system menu bar specifically,
  is unknown and untested further this pass.

  **Narrowed mechanism, not yet a root cause**: the arm and the post both
  provably happen in the confirmed-front target's own context (the
  counters distinguish "never called" from "called and declined," and
  every reading says "never called"), so this is not the front-process
  hijack the diagnosis brief's leading hypothesis named, and it is not a
  reference-resolution or ABI problem (`actselftest` still reports
  abi-agreed, `menugeom` still serves outright with no click involved).
  The defect is somewhere between "the resident extension posts a
  mouseDown/mouseUp pair into the OS event queue from inside the
  target's own `WaitNextEvent` call" and "the target's own application
  code dispatches that pair to the trap the request armed" — a gap this
  pass instrumented but did not close. Full trial data, the `activate`
  confirmation, the ctlact before/after snapshots and the menuact
  wire-stall transcript are `docs/local/wave4-*.txt`.

  **Wave 5, same day, fresh clones per run, builds `256914942954`
  through `0b9c5d295e6b`.** Still broken, and now narrowed to one
  sentence: **a `mouseDown` queued by `PPostEvent` is never delivered to
  any application on this guest, while a `keyDown` queued the same way
  from the same place is.** Everything below was measured against guest
  state, never the verb's own answer.

  The load-bearing control: NOW's `key` verb has two halves that queue
  into the same event queue by two different routes — `mods: 0` posts
  from the Carbon application (`PostEvent`), `mods != 0` posts from the
  resident extension inside the target's own jGNE pass (`PPostEvent`).
  The extension half **actuates**: `key char=110 mods=256` really opened
  five new SimpleText windows (5/10, the later trials confounded by
  SimpleText's own window budget rather than by delivery). So the
  resident context is not inert and `PPostEvent` is not refused there.

  A second oracle worth having, because it needs no patch of ours at
  all: a `mouseDown` delivered into the **content of a background
  window** brings that window forward — that is the Window Manager's own
  behaviour. Across every run below, the target window sat at z=1 before
  and z=1 after, and the global `FindWindow` entry counter (bumped
  unconditionally at the top of the patch, before any guard) did not
  move once. The application is not declining our press; it never sees
  one.

  Four mechanisms tested and **ruled out**, each on a fresh boot:

  1. *Event-queue exhaustion* (upstream's `A-AAAAAAAA----------`
     signature, the leading theory). Fresh boot with the click case run
     FIRST, nothing posted before it: 0/8.
  2. *The click posted from the application's own process* — upstream's
     literal topology. 0/6, and the ask was never served at all.
  3. *The click posted from any process but the target's.* 0/5, with
     293–303 act passes in five seconds and **every single one in the
     target's own A5 world**.
  4. *The click posted on a LATER pass than the one that armed* —
     upstream's ordering, which this port had never reproduced. The
     press is queued, `PPostEvent` returns a real queue element, and the
     application still never calls `FindWindow`. 0/6.
  5. *The low-memory mouse writes.* The whole sequence run once with the
     four `LMSet*` calls removed — byte for byte the sequence that DOES
     deliver a keyDown here — also 0/6.

  A **new CarbonLib fact** fell out of (2) and (3) and is worth more
  than the negative results: a background Carbon application's
  `WaitNextEvent` does not fall through to the classic Event Manager, so
  **the resident jGNE filter never runs for it while the wire waits** —
  measured with mask zero, with `networkMask`, and with `everyEvent`,
  all three giving zero passes in our own world. Upstream's topology (an
  agent process queueing the press for a different target) is therefore
  not reachable from a Carbon binary at all. That is the real answer to
  "why can't NOW just do what Mirror does", and it is not a bug to fix
  in this plane.

  **What the code now carries** (act_format V2 → V3): the arm and the
  press are separate passes with `click_pending` / `click_not_a5`, so
  the press goes out after the arm has been published rather than during
  it; `click_passes` and `click_last_a5` count who reached the hook
  while a press was owed; and `act-click-no-pass` / `act-click-refused`
  / `act-not-taken` are three statuses for three opposite repairs. None
  of that fixes the plane. It is what made the five results above
  measurable in an evening instead of guessable.

  **What the next session needs**, in order of value: (a) an instrument
  inside the guest that can say whether the queued `EvQEl` is still in
  the OS event queue after the target has pumped — the extension can
  walk `LMGetEventQueue()` and count mouseDowns, which distinguishes
  "queued and never dequeued" (a routing problem) from "dequeued by
  somebody" (a consumer problem), and is the one reading none of this
  evening's work could take; (b) the same probe run against a NON-front
  target and against a second application, since every trial here used
  Finder; (c) a spin of upstream's own stack with `winact` on it —
  neither Mirror VM running on this Mac carries a Portal new enough
  (v1 and a pre-`winact` agent), so upstream's 20/20 is quoted from
  `mirror/docs/STATUS.md` and was NOT re-taken here. Wave 5 transcripts
  are `docs/local/wave5-*.txt`, and `docs/local/wave5-01-simpletext-cmd-n.png`
  is the guest with `untitled 11` open — every one of those windows was
  opened by an extension-posted Cmd-N, which is the positive control this
  whole diagnosis rests on.

  **Final numbers on the landed build** (`996f0fbcd7ba`, fresh clone,
  oracle = guest state): `winact close` **0/10**, `menuact` **0/10**,
  `ctlact` not measurable this run (the fresh desktop had no in-range
  visible control; 0/10 on the earlier build that had one), `key` with
  modifiers **5/10** — the same `AAAAA-----` shape twice, and it is the
  guest's own window budget rather than decay, since a later run on the
  same machine opened five more. `key` with `mods: 0` was **not**
  measured: the oracle chosen for it (a SimpleText window's text length)
  is not reported by `elements` for that window, so the run measured
  nothing, and it is recorded as unmeasured rather than as 0/10.

  One wave-4 observation did **not** recur: ten back-to-back `menuact`
  calls all replied on the wire. The wave-4 stall (calls 2–10 silent for
  the full reply timeout) did not reproduce on any wave-5 build.

  **Two caveats on wave 5's own evidence, stated because they weaken
  it.**

  *The z-order oracle is weaker than it reads.* "A background window
  clicked at its centre does not come forward" is only conclusive if
  that centre is not covered by the window in front of it. The trials
  clicked the centre of `Macintosh HD` (z=1) while `System Folder` sat
  at z=0, and nothing checked whether the two overlap. If they do, a
  delivered click would have landed in the FRONT window's content and
  moved nothing, which is the same picture. The frozen global
  `FindWindow` counter is the stronger half of that argument and does
  not depend on geometry — but it depends on the next caveat.

  *`actselftest` refuses against Finder, and answered `abi-agreed`
  against this application at spin-up on the same build.* That is
  unexplained and it matters, because the selftest is the only oracle
  saying the trap patches work at all. It makes its own `MenuSelect`
  from the resident 68K half, so it should not care which process it
  runs in. Until it is explained, "the global FindWindow counter never
  moved" has a second reading that this pass did not eliminate: that a
  PowerPC-native application's Toolbox calls may not reach the 68K trap
  table the patches live in, in which case the press could be arriving
  and being handled natively, with our patch never in the path and its
  counter correctly reading zero. That reading would fit every number
  here, and it is the FIRST thing the next session should test — it is
  cheap: call `actselftest` against several processes and see which
  answer, and compare a 68K application with a PowerPC one.
- **The one thing NOT exercised this pass**: a person clicking the
  rendered pane. See "The last functional gap" below — the click path
  now exists and is wired, but this session drove everything through
  the wire's own verbs, not through a running host app's UI.

**Unverified, in the strongest sense on this list.** *(True of the
click-and-scroll checks below; the fetch mechanism above is now real.)*
A Finder folder window
now draws the Finder's own live items and can have one opened by name —
`MirrorFolderItemsJoin` (`now-host/Sources/Host/MirrorFolderItemsJoin.swift`)
joins a `script`-fetched window walk onto `Scene.Window.items` after every
landed scene, and `MirrorFolderItemsAim`
(`now-host/Sources/Host/MirrorFolderItemsAim.swift`) opens one named item by
re-running that same walk and sending an `aesend odoc` addressed at the
Finder's own PSN with the item's composed HFS path. Both are unit-tested
against fixture text transcribed from `FinderItems`'s own measured numbers
(2026-07-31, mac99/OS 9.1) and neither has been asked of a real Finder.

**What the emu-validation wave should exercise, the 40/40-style check:**
against a live Finder with at least one folder window open —

1. Open the Mirror pane; the folder window should draw named, positioned
   items rather than an empty content rect.
2. Double-click one. The Finder should open it (a document in its owning
   application, a folder in a new Finder window) — the same effect a real
   double-click has, because that is what `odoc` addressed at the Finder
   asks it to do to itself.
3. Scroll the window, then double-click an item that is now off-screen in
   the RENDERED drawing but the Finder still reports at some position:
   confirm the open either succeeds against where it actually is or refuses
   "scrolled out" — never opens whatever used to be at the clicked pixel.
4. Open two Finder windows on folders of the same name (nest one inside a
   same-named folder, or rename a second window's folder to match): confirm
   neither draws items and the page says why, rather than one silently
   winning.
5. **The known-open cost**: this join runs once per landed scene whenever a
   Finder folder window is open, and `FinderItems`'s own header measures
   that call at roughly 1-2s of Finder time. Confirm this does not visibly
   stall the watch loop on a PowerBook-class machine; if it does, the fix is
   likely to make this join fetch-on-ask like `MirrorContentJoin` rather
   than automatic — a product decision, not a bug, but one that wants a
   real machine's number to make.

**What is deliberately NOT built, and why it is not a gap:** selecting an
item without opening it (`.finderSelect`) still has no host lane —
`MirrorActionDriver` still refuses it by name, unchanged, because the
Finder's own `select item "X" of window "Y"` needs the `script` command run
as an ACT and this host has no lane for `script` as one (only as the fetch
`MirrorFolderItemsJoin`/`MirrorFolderItemsAim` already use). Building that
lane was out of scope for this pass and is a `script`-as-act host lane away,
not a guest verb — the guest already serves `script`.
...
**Retracted 2026-08-01, commits 5985a5c and ce8b453 (`audit/pane-icons` and `audit/pane-keys` merges).** The gesture path landed and is tested; the pane is wired and connected.
...
**`key` no longer refuses every modifier outright — the wall narrowed,
2026-08-01, and neither half of it has run.** An event's modifiers live
on the Event Manager's **queue element**, not in the message, and the
only call that hands that element back is `PPostEvent` — which CarbonLib
does not have (`CALL_NOT_IN_CARBON`). NOW's application is Carbon and
still cannot post one itself. But `ext/src/now_ext_act.c` is **68K
resident**, not Carbon, and already proved the same reach for a mouse
press (`act_post_click()`, `now_ext_act.c:497-533` — sets
`LMSetMouseLocation` and calls `PPostEvent` for the press and release,
stamping `evtQWhere` and `evtQModifiers`). `kNowPeekActOpKey` (act plane
V2) is the same mechanism for a key: `PPostEvent(keyDown)`, stamp
`evtQModifiers` on the queue element, a short bounded wait, then
`PPostEvent(keyUp)` — ported as a MECHANISM, not a measurement, from the
sibling Mirror project's `verb_key` (fix commit f4b4742), which is parked
and metal-proven there.

So `mods != 0` now ROUTES through that op instead of being refused
unconditionally: `now_input_run_key`
(`now-guest-ppc/src/input/input_cmds.c`) asks whether the act plane is
Ready only when a modifier was actually requested, and `now_key_check`
(`now-guest-ppc/src/input/input_args.c`) carries the decision as an
explicit `mods_available` flag rather than querying the Toolbox itself.
`mods` outside the extension's absent/stale/dark states, or naming no
process pumping, is still `unsupported` and names which of those it was;
`mods: 0` is unchanged and never touches the act plane at all. Stated at
`contract/asyncapi.yaml`'s `key` and `menugeom` entries, and in the guest
at `now-guest-ppc/src/input/input_args.c` and
`now-guest-ppc/src/act/act_cmds.c`.

**CRITICAL, found 2026-08-01, wave 3: the wire's `key` verb cannot succeed
at all, for any request, modified or not.** `now_command_run(name,
request_json, …)` (`now-guest-ppc/src/core/wire.c:4864`) passes the
**whole raw envelope** — `{"type":"command.request","id":N,
"name":"key","args":{…}}` — as `request_json` into every command handler.
`now_input_run_key` (`input_cmds.c:295`) then does
`now_json_find_string(request_json, "name", name, …)`, and
`now_json_value` (`json.c:7`) is a bare `strstr` over that WHOLE text —
not scoped to `args`. Since the envelope's own `"name":"key"` appears
before any `args.name` (which our calls never set), the scan always
finds `"key"` and hands it to `now_key_check` as the caller's named-key
selector. `now_key_named("key", …)` fails (it is not one of
`return/enter/tab/…`), so `now_key_check` returns `kNowKeyUnknownName`
**before ever inspecting `code`/`char`/`mods`** — this is unconditional,
independent of whether `mods` is 0 or non-zero.

Confirmed live against the `mac99-os91` guest (build
`67368e24beb2 2026-08-01T08:58:22Z`), independently through both
`scripts/probes/nowwire.py`'s `GuestLink` and the unrelated
`tools/askguest.py`, on 20 separate calls (10× `key:char=87,mods=256` for
⌘W, 10× `key:char=119,mods=0` plain `w`) — **0/10 and 0/10**, every one
`{"code":"bad-request","message":"that is not a key this verb names. …"}`
verbatim. The target window (`Macintosh HD`) never closed once. This is
not the `mods`-availability wall described above; it fires even for
`mods:0`, and it fires whether or not the extension is present. The
console face is unaffected — `now_input_key_console` calls
`now_command_run(name, NULL, …)` (`console_model.c:716`), so a person
typing at the machine hits none of this; **the wire is the only face
this breaks**, which is exactly the shape `docs/command-parity.md`
exists to catch and this defect is not yet in it.

**`menugeom` ran clean the same session and is the contrast case.**
`menugeom(menu:257)` against Finder's real `File` menu (fetched from a
live scene, `id 257, left 38, 20 items`) answered `item_count: 20` with
per-item rects whose heights are **not uniform**: `16` for an ordinary
item, `6` for each of the three separators (rows 6, 14, 18) — matching
the File menu's own three separators exactly. `menugeom`'s own args
(`menu`, `serialHi`, `serialLo`) share no name with the envelope, so it
never triggers this collision; `key` and any future verb whose own name
doubles as one of its own argument keys would.

**NOTHING ABOVE HAS RUN.** `scripts/test-native` covers the decision half
(`now_key_check`'s narrowed wall, both branches) and a real cross-build
of both guests and the extension succeeds, but a build proves compilation
only. No emulator or metal session has watched a modified keystroke
arrive, and the host (`MirrorAction`/`ActionModel.swift`) has **not**
been updated to use this route — its `key` availability table still
marks every `mods != 0` request `.unavailable` unconditionally (see the
table above), so nothing on the host side will attempt it yet. Wiring the
host is a separate, later step; this entry exists so the gap between "the
guest can" and "the host will" is not silently read as closed.

**`menugeom` (new 2026-08-01) has the identical shape and the identical
gap.** One menu's real per-item rects, read from its own MDEF
(`mCalcItemMsg`) rather than assumed at a fixed row height — served
outright by the same extension, gated by the same V2 plane-state check
as `key`'s modifier route, and equally unrun: `scripts/test-native`
passes and both guests plus the extension cross-build clean, and nothing
more. `SceneRenderer.swift`'s dropdown drawing (`dropdownFrame`,
`drawDropdown`, `dropdownItem`) still assumes a uniform 16px row and a
6px separator sliver and has **not** been changed to consume this verb —
that host-side wiring (a wire call per open menu, a fallback to the
constant when the data is not there) is explicitly left for a later
step rather than attempted under this lane's time budget, so it is not
claimed done.

**Update — 2026-08-01, lane L7: both host-side gaps above are closed on
this branch, and neither has run any closer to metal than the guest half
already had.** `ActionModel.availability(.key)` no longer marks every
`mods != 0` request `.unavailable` — it answers `.available(command:
"key")` unconditionally, and `MirrorActionDriver` forwards `mods` exactly
as resolved; the guest's own reply (armed dispatch, or a named refusal —
`act-plane-absent`/`-stale`/`-dark`/`act-no-anchor`) is what a caller
sees, never a host-side guess made in advance (`ActionModel.swift`,
`MirrorActionDriver.swift`, both with `RETRACTED 2026-08-01` comments at
the changed case). `SceneRenderer`'s `dropdownFrame`/`dropdownItem`/
`drawDropdown` now take an optional `MirrorKit.MenuGeometry` and consume
real per-item rects when one matches the open menu (right menu id, a rect
for every item the menu currently has), falling back to the uniform 16px
row otherwise — never a mix of real and invented rects for the same menu.
The wire call this entry asked for is `AgentIntegrationActControl
.menuGeom` / `AgentIntegrationHostAdapter.menuGeom`, fetched by
`MirrorModuleModel.openDropdown(at:menu:)` on a menu-title press — which
also had to be built from nothing: `MirrorModuleModel` had NO menu-open
state or click handling at all before this (`ActionModel.click`'s own
`.menuTitle` case returned `[]` with the comment "opening a menu is UI
state, not a guest action", and nothing downstream ever set one). Tested
against a fake guest (`MirrorActionDriverTests`, `MenuGeometryTests`,
`DropdownGeometryTests`, `AgentIntegrationMenuGeomTests`,
`MirrorDropdownModelTests`) and mutation-checked (breaking the
real-geometry path in `dropdownItem` was caught by
`DropdownGeometryTests.testDropdownItemAimsAtTheRealRowWhenGeometryMatches`
naming the wrong row). **Nothing here is metal-verified** — no emulator or
PowerBook session watched a modified keystroke arrive or a real dropdown
selection land, exactly the gap this entry existed to keep from being
silently read as closed, and it still is not.
...
**Update (2026-08-01, later the same day): the supplier now exists, and
still has no caller.** Both halves of the producer this entry called
missing are built and tested:

- Guest: `capture.region` (`contract/asyncapi.yaml`, served by
  `serve_capture_region` in `now-guest-ppc/src/core/wire.c`) — the
  side-effect-free sibling of `process.shot`: a GLOBAL rect, no PSN, no
  `SetFrontProcess`. It wraps the existing `capture_screen_rect` /
  `gather_shot_rect` (already in the tree before this change) and rides
  the existing capture transfer lane. `capture.begin` now also carries
  `originX`/`originY`(/`scale`, unused by this guest) on every capture,
  not only regions — the field process.shot's own window crop was
  silently missing too.
- Host: `GuestListener.requestCaptureRegion` (mirrors
  `requestProcessShot`), and `GuestListener.CaptureDelivery
  .makePixelIsland()` (`Host/CaptureIslandAdapter.swift`) — a CGContext
  raster of the capture's already-decoded `CGImage` into the tight RGBA8
  `PixelIsland.rgba`, paired with `format.originX/originY/scale`. Not a
  second wire decoder: `CaptureDecoder` already turns every depth this
  guest sends into a `CGImage`, and re-deriving Mirror's rgb555be /
  gray8 / mono1 math here would repeat the exact defect this file's own
  porting note (`PixelIsland.swift`) named for the upstream `WireClient`
  fetch this project already once rejected.

**Still nobody's caller.** `requestCaptureRegion` is completion-based —
a network round trip with a 20-second watchdog. `SceneIslands.Capture`
is `(Rect) throws -> PixelIsland`, synchronous. Bridging the two is not
a wiring job in the sense "plug A into B": it is a design decision about
which thread blocks and for how long, because a naive semaphore wait
inside `attach` risks deadlocking whatever run loop drives the poll that
called it — `GuestListener` very plausibly needs that same run loop free
to receive the reply the wait is blocking on. That decision, plus the
actual call site inside whatever polls the scene (`MirrorModuleModel` or
its successor), is squarely a wiring job for whoever owns that file —
this entry is not it, on purpose.

## The content plane has never run anywhere (2026-08-01)

**Emulator-verified 2026-08-01, wave 3 (`claude/mirror-parity-overnight`,
`mac99-os91`, build `67368e24beb2 2026-08-01T08:58:22Z`).** `qdtrace start
front:true mode:record` armed the front process (`resolvedVia: "front"`,
a5 `0x1f50f550`) against a live Finder; two `aesend odoc` calls (opening
`TBTRunner`, then re-asserting `Macintosh HD`) drew into it while armed,
and `qdtrace status` moved `ops.total` from 20 → 44. `qdtrace drain`
returned 24 real records — `state` (origin/clip/fg/bg), `rect` fills and
two `bits` (CopyBits) blits, all tagged with real QuickDraw ports
(`0x00a5ff30`, `0x00a601f0`) and tick stamps. `qdtrace stop` disarmed
cleanly. **This is the writer's first execution on any Macintosh**, and
it worked exactly as designed — the paragraph below stands as history,
not as the current state.

**Unverified in the strongest sense on this list, and not a fault to
chase.** *(True until the update above.)*
...
- **2026-08-01 runtime observation: the module was connected and chrome-only** — menu bar and window frames drew; window interiors empty; desktop bare; process shelf showed monogram tiles. The state most worth looking at is `.neverAttached`, because it is what the pane says on most machines for most of their lives.
