...
## Update — 2026-08-01, lane L7 (mods-gated no longer, on this branch)

**The claim this lane's whole design rested on — that NOW has no code path
for a modified keystroke at all — was true when this document was written
and is retracted here, not by editing the paragraphs below but by this
note in front of them.** The guest's `key` verb gained an act-plane route
for `mods != 0` on this branch after this lane closed
(`contract/asyncapi.yaml:key`, "THE ROUTE, AS OF 2026-08-01"; guest side
`now-guest-ppc/src/input/input_cmds.c:run_key_via_act`; resident side
`ext/src/now_ext_act.c`'s `PPostEvent` reach, the same reach `winact` and
`menuact` already used). That did not touch this lane's own conclusion
that **CarbonLib itself has no `PPostEvent`** — this application's own code
still cannot post a modifier — the new fact is that a 68K resident filter
CAN, when the NOW Extension is present, current, and the target process is
pumping.

L7 lifted the two HOST-SIDE gates this lane built to enforce the
now-superseded fact: `ActionModel.availability(.key)` and
`MirrorActionDriver`'s `.key` case both used to refuse `mods != 0` before a
request was even built (item 4, "What this lane built"). Both now send it
and let the GUEST's own reply decide — dispatched when the act plane is
armed, refused by name (`act-plane-absent`, `-stale`, `-dark`,
`act-no-anchor`) when it is not — the same discipline every other act on
this lane already held for a stale window reference or a busy target.
`MirrorKeyCaptureView` and `ActionModel.paneKeystroke` needed no change at
all: they already forwarded Command/Option/Control into `mods` honestly
(see "What this lane built" item 5) — it was only the two AVAILABILITY
gates sitting in front of the wire that assumed the fact this update
retracts.

Section 5 below ("Hold ⌘ and press `n`... **Expected**: `ActionModel
.availability` refuses it locally") is the one concrete casualty: that
step's expectation is from before this update and is no longer what
happens. A held ⌘ now reaches the guest exactly like any other keystroke,
and the honest expectation is whichever of "posted" or a named refusal the
paired guest's own extension state produces — see
`MirrorActionDriverTests.testAModifiedKeystrokeReachesKeyWithModsForwarded`
and `.testAModifiedKeystrokeIsRefusedHonestlyWhenTheActPlaneIsAbsent` for
both outcomes, unverified on real hardware exactly as the rest of this
document's plain-keystroke path is.
...
**2026-08-01 runtime observation: the built host was run against a live
guest and the pane was connected and chrome-only** — menu bar and window
frames drew; window interiors empty; desktop bare; process shelf showed
monogram tiles. Unverified, specifically:
