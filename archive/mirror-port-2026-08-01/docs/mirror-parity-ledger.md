...
| Resolution autodetect (scene envelope carries display bounds) | upstream measurement in prior art | **crossed, resolved BETTER** | `now-guest-ppc/src/scene/scene_collect.c:52-64` reads `GetMainDevice` gdRect into every scene envelope on collection; NOW embeds the resolution in every frame. Upstream needed a separate `video` verb to probe the display — NOW's walk answers it once per scene | Asymmetry resolved by design: guest-side walk reports resolution; host reads one value per frame, no verb overhead |
...
| `FindWindow` patch answering at either stage | `6f3c228` "FindWindow answers at either stage, and the counters scope to the request" | crossed | Guest: `now_act_patch_findwindow` at `ext/src/now_ext_act.c:660-679` is the patch vector; `now_act_findwindow_answer` at `now-guest-shared/src/now_act_guard.c:362-415` answers at both ready AND stage-2 (same upstream strategy per header note at `contract/peek_table.h:246-258`) | The shared source file audit grepped now-guest-ppc only; resident code spans both |
...
| Drag / shortcut-less menu-drag (emu-only, QMP closed loop) | `8383019`, `40aa89f`, `4cfe05b`, `48ea6fd` (nohijack drag-learning series) | **refused** | `now-host/Sources/MirrorKit/PORTING.md:44-76` documents deletion of `QmpClient` + the `drag`/`menuDrag`/`thumbDrag` executors — "emulator mouse events injected from outside the guest CPU; host-side cheating by this project's rule, and there is no emulator on the other end of a NOW connection by assumption" | QMP-layer deliberately deleted; no emulator on the other end of a NOW connection |
...
| Scene refs — window and control `ref` tokens minted in the walk | part of reference-layer work | crossed | `scene.h:178-189` (`NowSceneControl.ref`), `scene_walk.c:49-73` (`name_window`, `name_control` call obs walk to mint tokens) — **landed 2026-08-01** | Every window and control now carries an opaque reference token; previously blank and carried in identity-only mode |
