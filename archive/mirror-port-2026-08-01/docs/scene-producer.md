...
| `desktopItems[]` | **conditional** — present once the Desktop Folder walk or the volumes walk ran, whichever gets there; no anchor, no process — the guest's own File Manager |
...
*Landed 2026-08-01.* `desktopItems[]` is the one conditional plane with no
process and no anchor behind it — it is the guest's own File Manager, not a
foreign-memory read, so `now_scene_open_desktop_items` /
`now_scene_add_desktop_item` (`scene_build.c`) answer to nothing but the
walk that calls them (`scene_desktop.c`).

Two independent walks feed it. The **Desktop Folder**'s own entries carry the
Finder's *saved* `fdLocation` — a value `mirror/docs/FOLDER-ITEMS.md` found is
the *wrong* quantity the moment a Finder window scrolls, but the desktop is the one
surface with no scroll frame to disagree with it, so the saved position **is**
the live one there. **Mounted volumes** are not Desktop Folder entries at
all — `PBHGetVInfo` can name one cheaply, but where the Finder actually
*drew* it lives in the Finder's own desktop database, a fact no File Manager
call reaches — so every volume this producer reports carries `placed:false`,
always, and a host lays it out by the Finder's own default (top right,
stacked down: `SceneGeometry.placeVolumes`, ported from upstream's
`ScenePoller` verbatim). A real position, should this producer ever earn one,
replaces `placed:false` outright rather than needing that host code touched.
`kind` is `"disk"`, spelled exactly that way — upstream's own renderer
already keys on it and fell through to a generic document icon the one time
this was spelled `"volume"` instead (`mirror/3b76f79`).

`desktopItems` is present the moment **either** walk opens it and absent only
when neither does — an unresolvable Desktop Folder does not suppress the
volumes walk, and a volume-less machine (not one this project can actually
reach) would not suppress the folder walk either. An **invisible** item
(`fdInvisible`) is refused by assembly itself before it becomes a row, the
same independent-refusal shape `now_scene_add_window` already gives a
disallowed anchor — the Finder does not draw one, and neither does this
scene, regardless of what the walk passed in.

The host side of this crossing needed no new decoder: `NOWSceneDocument`,
`MirrorKit.Scene` and `MirrorSceneAdapter` already carried `desktopItems`
and `SceneGeometry.placeVolumes` from the verbatim MirrorKit port — this
plane was the first guest-side producer to actually fill them. The one wire
this landing added is in the adapter: `kind == "disk"` rows are split out,
run through `placeVolumes` against the scene's own screen size, and
recombined with everything else, mirroring upstream's own call site
(`ScenePoller.swift`, folder items plus `placeVolumes(vols, screen:)`).

| Mutation | Result |
|---|---|
| `now_scene_add_desktop_item`'s `if (invisible) return 0;` guard removed | 2 red — an invisible item reaches the array |
