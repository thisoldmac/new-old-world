...
booted emulator guest, through the lab's baked anchor worker.
    ...
    the pump    -> Macintosh HD:TimBotTu:now-dev:now-pump   (beside the app)
...
THE SECOND GAP, closed the same way. `scripts/build-guests` also builds
`now-pump` (the act plane's click-posting context, now-pump/src/main.c),
and until this file staged it too, nothing put it anywhere: the
application finds it by walking its OWN FOLDER for a file whose creator
is 'NWpu' (act_session.c :: session_find_pump), never by name, and a
folder that holds only the application has nothing for that walk to
find. Staging the app WITHOUT the pump is not a smaller deploy — it is a
machine whose click plane can never work, on a stock spin-up or a real
PowerBook alike, and it read as a working deploy right up until someone
tried to click something (measured 2026-08-01: the acceptance pass had
to stage `now-pump.bin` in by hand to get a number at all). So staging
the app now REQUIRES the pump; there is no flag to skip it.
...
# Found by creator, never by name (act_session.c :: session_find_pump), so
# this name is cosmetic - but it still has to be SOMETHING, and matching
# the CMake target now-pump/CMakeLists.txt builds is the least surprising
# choice.
PUMP_NAME = os.environ.get("NOW_PUMP_NAME", "now-pump")
...
# Required WHENEVER APP_BIN is: an application staged without the pump
# beside it is not a smaller deploy, it is one whose click plane can never
# work (see the module docstring). No flag skips this.
PUMP_BIN = os.environ.get("NOW_PUMP_BIN")
...
if APP_BIN:
    ...
    if not PUMP_BIN or not os.path.isfile(PUMP_BIN):
        raise SystemExit(
            "stage-ext: set NOW_PUMP_BIN to the built now-pump.bin "
            "(scripts/build-guests pump builds it) whenever NOW_APP_BIN is "
            "set. The application finds its click plane by walking its OWN "
            "FOLDER for a file whose creator is 'NWpu' (act_session.c :: "
            "session_find_pump) - staging the app without it produces a "
            "machine whose click plane can never work, and nothing about "
            "the app's own staging would say so.")
    print(f"== stage {APP_NAME} ==")
    ensure_dir(DEV)
    push_verified(f"{DEV}:{APP_NAME}", open(APP_BIN, "rb").read(),
                  want_type="APPL", min_data=1024)

    # 3. The act pump, BESIDE the application in the same folder
    #    session_find_pump walks. Identity is creator 'NWpu' / type 'APPL'
    #    (now-pump/CMakeLists.txt) - matched here the same way the
    #    extension's identity is matched above, because the application
    #    finds this file the same way peek.c finds the extension: by
    #    Finder identity, never by name.
    print(f"== stage {PUMP_NAME} (act pump, beside the application) ==")
    #    The pump is a 68K application: its CODE lives in the RESOURCE
    #    fork and its data fork is legitimately empty, so `min_rsrc` is
    #    the load-bearing assertion here — the same one the INIT uses,
    #    not the PPC application's `min_data`. Measured 2026-08-01 on
    #    mac99/os91: a correctly staged pump reads data=0 rsrc=59475, and
    #    a min_data assertion rejected it.
    st = push_verified(f"{DEV}:{PUMP_NAME}", open(PUMP_BIN, "rb").read(),
                       want_type="APPL", min_rsrc=1024)
    if st.get("creator") not in (None, "NWpu"):
        raise SystemExit(
            f"staged pump has creator {st.get('creator')!r}, not 'NWpu' — "
            f"session_find_pump would not find it beside the application")
