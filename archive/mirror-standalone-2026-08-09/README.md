...
| `guest/extensions/portal/` | The Portal `INIT` (68K) — the act half: in-process trap answering |
...
| `docs/` | Start at [ARCHITECTURE.md](docs/ARCHITECTURE.md) (how it works and why each rule exists) and [CAPABILITIES.md](docs/CAPABILITIES.md) (everything shipped, with the commit that proves each row); [STATUS.md](docs/STATUS.md) is the living state |
...
[docs/TEST-DRIVE.md](docs/TEST-DRIVE.md) and the current act-plane state in
[docs/STATUS.md](docs/STATUS.md). (An older warning here said `axdo` wedges
the session — that was retracted 2026-07-30: the wedge was two bugs in the
test client, and `axdo` scrolls a real scrollbar.)
