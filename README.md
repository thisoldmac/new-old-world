...
| Projects and Development: app-owned Git history/workspaces, verified guest candidates, declarative MPW ToolServer jobs, exact-product run and optional CodeKitten handoff | yes | typed unavailable | **host-home build/run/dialog metal-verified on the PB1400c**: MCP preserved source resource forks and `TEXT/MPS ` identity, sealed an inactive candidate, completed MPW compile/link/Rez, measured both `APPL` product forks, launched the exact product, observed its dialog and discarded the candidate; guest-home import/promotion, typed tests and CodeKitten handoff remain unverified — [docs/development.md](docs/development.md) |
...
**Emulate Finder Window Interiors (Experimental)** keeps each guest-observed Finder window and
...
| `now-host/Packages/MirrorKit/` | NOW-owned `MirrorKit` and `MirrorKitUI` libraries: the semantic scene model, hit testing, and renderer. `scripts/test-all` runs their independent suite at stage 2, both with the selected external asset pack and with `NOW_MIRROR_ASSETS=none`. |
| `tools/asset-pack/` | Resource-fork, font, icon, cursor, and pattern parsers used by the supported offline extractor. Generated Apple assets go to an external configurable store, never into this tree ([docs/asset-pack.md](docs/asset-pack.md)). |
| `archive/mirror-standalone-2026-08-09/` | The retired standalone Mirror app, guest, oracle, probes, raw research, and imported assets. Historical/searchable only; no production target builds or stages it ([archive/mirror-lineage.md](archive/mirror-lineage.md)). |
