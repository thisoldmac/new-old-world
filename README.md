...
| Projects and Development: app-owned Git history/workspaces, verified guest candidates, declarative MPW ToolServer jobs, exact-product run and optional CodeKitten handoff | yes | typed unavailable | **tested and builds only**; the headless workflow has not run in an emulator or on hardware, source resource forks are refused rather than lost, and Finder metadata is not yet preserved — [docs/development.md](docs/development.md) |
