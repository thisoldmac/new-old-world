...
static void test_drive_block_bytes_do_not_wrap(void)
{
    CHECK(n68_census_block_bytes(2048UL, 512UL) == 1048576UL,
          "ordinary drive capacity is exact");
    CHECK(n68_census_block_bytes(0xFFFFFFFFUL, 512UL) == 0xFFFFFFFFUL,
          "a drive above the 32-bit byte range saturates instead of wrapping");
    CHECK(n68_census_block_bytes(0UL, 512UL) == 0UL,
...
static void test_a_tight_frame_pages_rather_than_truncating(void)
{
    ...
    for (i = 0; i < kN68CensusRowsMax; ++i) {
        ...
        snprintf(want, sizeof want, "\"unit %d..", i);
...
int main(void)
{
    test_drive_block_bytes_do_not_wrap();
