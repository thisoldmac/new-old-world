...
static void expect_log_retention(const char *text, unsigned short keep)
{
    N68DevSettings s = parse(text);
    g_checks++;
    if (!s.have_log_retention || s.log_retention != keep) {
        fprintf(stderr, "FAIL log-retention from <<%s>>: have=%d keep=%u\n",
                text, s.have_log_retention, s.log_retention);
        assert(0);
    }
}

static void expect_no_log_retention(const char *text)
{
    N68DevSettings s = parse(text);
    g_checks++;
    if (s.have_log_retention || s.log_retention != 0) {
        fprintf(stderr, "FAIL expected no log-retention from <<%s>>\n", text);
        assert(0);
    }
}
...
int main(void)
{
    /* ---- every key, in the file the lab actually writes ---------------- */
    {
        ...
        if (!s.have_host || s.host_addr != 0xC000022FUL
            ...
            || !s.have_log_retention || s.log_retention != 10
            || s.keys_set != 7 || s.bad_lines != 0) {
    ...
    expect_log_retention("log-retention = 1\n", kNowLogRetentionMin);
    expect_log_retention("log_retention = 10\n", 10);
    expect_log_retention("LOG-RETENTION: 100\n", kNowLogRetentionMax);
    expect_no_log_retention("log-retention = 0\n");
    expect_no_log_retention("log-retention = 101\n");
    expect_no_log_retention("log-retention = ten\n");
