 ...
 * disk produces, which is how now-guest-68k/tests/test_putrx.c can exercise a
 * 4 MB stream at all.
 *
 * ---- Never in memory ---------------------------------------------------
 *
 * Bytes go to disk as they arrive. The application partition is 384 KB
 * (now-guest-68k.r) on a 4 MB machine, so there is no version of "stage it in
 ...
 *
 * Published rather than static so now-guest-68k/tests/test_putrx.c can walk
