 ...
 * PowerPC guest (now/now-guest-ppc/src/files/fileshare.c) - the two guests are reached
                                 ...
                                 long len)
{
    while (len > 0) {
        ...
        if (rx->header_have < kN68MacBinaryHeader) {
            ...
            len -= take;
            if (rx->header_have == kN68MacBinaryHeader) {
                if (!now_macbinary_fork_lengths(rx->header,
                                                 rx->offer.bytes,
                                                 &rx->mb_data_len,
                                                 &rx->mb_rsrc_len)
                    || !header_is_macbinary(rx)) {
...
N68PutCode n68_putrx_data(N68PutRx *rx, const void *bytes, long len)
{
    ...
    if (rx->received > rx->offer.bytes
        || len > rx->offer.bytes - rx->received) {
