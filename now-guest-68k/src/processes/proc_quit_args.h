/*
 * Copied verbatim (grammar and identifiers unchanged - no collisions found
 * against the NOW-68K source tree) from the PowerPC guest,
 * now-guest-ppc/src/processes/proc_quit_args.h on branch thread/guest-quit-command, per
 * proc68.h's instruction to mirror that file rather than re-derive a second
 * grammar. Only the .c file's formatting internals changed for this target
 * - see proc_quit_args.c's header comment.
 */
...
/* `quit`'s argument line, parsed once for both callers.
   ...
   PowerBook (now-guest-ppc/tests/proc_quit_args_test.c). */
