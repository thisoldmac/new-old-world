...
static int apply_pair(N68DevSettings *s, const char *key, const char *value)
{
    ...
    if (str_eq(key, "log-retention")) {
        unsigned long keep;

        if (!parse_uint(value, &keep)
            || keep < (unsigned long)kNowLogRetentionMin
            || keep > (unsigned long)kNowLogRetentionMax) {
            return 0;
        }
        s->have_log_retention = 1;
        s->log_retention = (unsigned short)keep;
        return 1;
    }
